 ...
 * Purpose        : Backing model for the VAS_248_DeliveryOrderBottomPanel tab
 *                  panel — the customer-shipment counterpart of
 *                  VAS_240_RequisitionBottomPanel. Provides parent-shipment
 *                  context and existing lines, the dictionary column metadata
 *                  that drives the Additional Info modal, paged Product / Charge
 *                  catalog search (50 rows / scroll), per-row AD_Val_Rule
 *                  lookups, the server-side line callout (unit + converted
 *                  quantity), product attribute (M_AttributeSetInstance) read +
 *                  create, barcode scan lookup and the M_InOutLine insert /
 *                  update / delete write actions (always through the MInOutLine
 *                  business class).
 *
 *                  A shipment line carries NO price, tax or discount — those
 *                  belong to the order and the invoice — so the panel states
 *                  quantities only: a line count and a summed MovementQty.
 *                  Everything money-shaped in VAS_240 is therefore absent here
 *                  rather than stubbed.
 *
 *                  DIRECTION IS A SECURITY BOUNDARY, not a display flag. This
 *                  model serves M_InOut rows with IsSOTrx = 'Y' ONLY: a vendor
 *                  receipt id posted to this panel's endpoint is refused, so the
 *                  delivery panel can never be used to edit a GRN. VAS_249 is the
 *                  standalone twin that serves the other direction.
...
namespace VASLogic.Models
{
    /// <summary>
    /// Module Name : VASLogic
    /// Purpose     : Data + write model behind the Delivery Order Bottom Panel
    ///               (M_InOut with IsSOTrx = 'Y' / M_InOutLine). Every SELECT is
    ///               filtered through MRole.AddAccessSQL on the main physical table
    ///               alias only and uses bind parameters so the same code runs on
    ///               PostgreSQL and Oracle. All inserts go through MInOutLine (never
    ///               a hand-written INSERT) so the standard shipment-line logic runs.
    /// Chronological development:
    ///   VAI154         Created  07-Sep-2026
    ///   VAI154         Rebuilt on the VAS_240 pattern  09-Sep-2026
    /// </summary>
    public class VAS_248_DeliveryOrderBottomPanelModel
    {
        private static VLogger log = VLogger.GetVLogger(typeof(VAS_248_DeliveryOrderBottomPanelModel).FullName);
        ...
        private const string PARENT_TABLE = "M_InOut";

        /// <summary>Parent key column.</summary>
        private const string PARENT_KEY = "M_InOut_ID";

        /// <summary>AD_Message key prefix for this panel.</summary>
        private const string MSG = "VAS_248_";

        /// <summary>
        /// The document direction this panel serves. A customer shipment is IsSOTrx = 'Y';
        /// LoadParentContext refuses any other, which is what stops a receipt being edited
        /// through this panel's route.
        /// </summary>
        private const bool IS_SOTRX = true;

        #region Panel (read) data

        /// <summary>
        /// Builds the panel header context (everything the client-side logic needs from the
        /// parent shipment) plus one page of already-saved lines. Returns an empty object
        /// (M_InOut_ID = 0) when the role has no access OR the document is not a shipment.
        /// </summary>
        ...
        public DeliveryPanelData GetPanelData(Ctx ctx, int M_InOut_ID, int AD_Window_ID, int page = 0)
        {
            DeliveryPanelData data = new DeliveryPanelData();
            data.LinePageSize = LINE_PAGE_SIZE;
            if (M_InOut_ID <= 0) return data;

            LoadParentContext(ctx, M_InOut_ID, data);
            if (data.M_InOut_ID <= 0) return data;   // no access / not found / wrong direction

            data.AD_Window_ID = AD_Window_ID;
            List<int> tabIds = ResolveInOutLineTabs(AD_Window_ID);
            data.AD_Tab_IDs = tabIds;
            data.AD_Tab_ID = tabIds.Count > 0 ? tabIds[0] : 0;

            if (page < 0) page = 0;
            int total;
            data.Lines = LoadLines(ctx, M_InOut_ID, page, out total);
            data.LinesTotal = total;
            data.LinePage = page;
            data.TotalQty = SumQty(ctx, M_InOut_ID);
        ...
        private void LoadLoginContext(Ctx ctx, DeliveryPanelData data)
        {
            Regex rx = new Regex(@"@([#$][A-Za-z0-9_]+)@");
            HashSet<string> tokens = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (DeliveryColumnMeta m in data.Columns)
            {
        ...
        /// <summary>Loads M_InOutLine column metadata once per panel load.</summary>
        /// <param name="ctx">session context</param>
        /// <param name="data">panel data to populate</param>
        /// <param name="AD_Tab_IDs">shipment-line tabs of the source window</param>
        private void LoadColumns(Ctx ctx, DeliveryPanelData data, List<int> AD_Tab_IDs)
        {
        ...
        private void LoadListValues(DeliveryPanelData data)
        {
            foreach (DeliveryColumnMeta m in data.Columns)
            {
                ...
                foreach (DataRow r in ds.Tables[0].Rows)
                    m.RefListValues.Add(new DeliveryRefListItem
                    {
        ...
        private Dictionary<int, List<int>> _iolTabsByWindow;

        /// <summary>Finds every active AD_Tab bound to M_InOutLine inside the given window.</summary>
        /// <param name="AD_Window_ID">source window</param>
        /// <returns>tab ids, in window sequence</returns>
        private List<int> ResolveInOutLineTabs(int AD_Window_ID)
        {
            if (_iolTabsByWindow == null) _iolTabsByWindow = new Dictionary<int, List<int>>();
            List<int> cached;
            if (_iolTabsByWindow.TryGetValue(AD_Window_ID, out cached)) return cached;
            ...
            _iolTabsByWindow[AD_Window_ID] = tabs;
            return tabs;
        }
        ...
        private static readonly HashSet<string> _systemPrefixes = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // everything up to the first underscore, so Ref_InOutLine_ID would read as an
            // optional module, fail Env.IsModuleInstalled and vanish from the payload.
            "Ref_", "Link_"
        };
        ...
        /// <summary>Reads AD_Field -&gt; AD_Column metadata across all the window's shipment-line tabs.</summary>
        /// <param name="ctx">session context</param>
        /// <param name="data">panel data to populate</param>
        /// <param name="AD_Tab_IDs">shipment-line tabs</param>
        /// <returns>whether any column was found</returns>
        private bool LoadColumnsFromTabs(Ctx ctx, DeliveryPanelData data, List<int> AD_Tab_IDs)
        {
        ...
        /// <summary>Merges every active M_InOutLine column not already loaded from a tab.</summary>
        /// <param name="ctx">session context</param>
        /// <param name="data">panel data to populate</param>
        private void MergeAllColumns(Ctx ctx, DeliveryPanelData data)
        {
            HashSet<string> have = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (DeliveryColumnMeta cm in data.Columns) have.Add(cm.ColumnName);
        ...
        private DeliveryColumnMeta MapColumnMeta(DataRow r, bool fromField)
        {
            DeliveryColumnMeta m = new DeliveryColumnMeta
            {
        ...
        /// catalog: a shipment line carries neither.
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="data">panel data to populate</param>
        ...
        private List<DeliveryUomItem> LoadUomList(Ctx ctx, int M_InOut_ID, Dictionary<string, string> rowVars)
        {
            ...
            string pred = GetValRulePredicate(ctx, "C_UOM_ID", "C_UOM", "u", M_InOut_ID, rowVars);
        ...
        private List<DeliveryLocatorItem> LoadLocatorList(Ctx ctx, int M_InOut_ID, int M_Warehouse_ID,
            Dictionary<string, string> rowVars)
        {
            ...
            string pred = GetValRulePredicate(ctx, "M_Locator_ID", "M_Locator", "l", M_InOut_ID, rowVars);
            if (pred.Length > 0) sql += " AND (" + pred + ")";
        ...
        /// <summary>
        /// Re-fetches the per-row filtered UOM and locator lists for one shipment line,
        ...
        public DeliveryLookupData GetLookupData(Ctx ctx, DeliveryLookupRequest req)
        {
            DeliveryLookupData data = new DeliveryLookupData();
            if (req == null || req.M_InOut_ID <= 0) return data;

            DeliveryPanelData parent = new DeliveryPanelData();
            LoadParentContext(ctx, req.M_InOut_ID, parent);
            if (parent.M_InOut_ID <= 0) return data;

            data.M_InOut_ID = req.M_InOut_ID;
            Dictionary<string, string> rowVars = BuildRowVars(req.RowValues);
            data.UomList = LoadUomList(ctx, req.M_InOut_ID, rowVars);
            data.LocatorList = LoadLocatorList(ctx, req.M_InOut_ID, parent.M_Warehouse_ID, rowVars);
            return data;
        }
        ...
        /// <summary>
        /// Generic FK lookup for a dynamic M_InOutLine field (Table / TableDir / Search):
        ...
        public List<DeliveryRefItem> GetRefLookup(Ctx ctx, DeliveryRefLookupRequest req)
        {
            List<DeliveryRefItem> items = new List<DeliveryRefItem>();
            if (req == null || req.M_InOut_ID <= 0 || string.IsNullOrEmpty(req.ColumnName)) return items;

            DeliveryPanelData parent = new DeliveryPanelData();
            LoadParentContext(ctx, req.M_InOut_ID, parent);
            if (parent.M_InOut_ID <= 0) return items;
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
                items.Add(new DeliveryRefItem { Id = Util.GetValueOfInt(r["Id"]), Name = Util.GetValueOfString(r["Name"]) });
            return items;
        }
        ...
        private static readonly Dictionary<string, string[]> TableDirOverrides =
            new Dictionary<string, string[]>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                // The order line this shipment line delivers, and the RMA line a return
                // ships against. Both follow the TableDir convention already, but are named
                // explicitly because their display expression is overridden below.
                { "C_OrderLine_ID", new string[] { "C_OrderLine", "C_OrderLine_ID" } },
                { "Ref_InOutLine_ID", new string[] { "M_InOutLine", "M_InOutLine_ID" } }
            };
        ...
        private string LineTableDisplayExpr(string table)
        {
            ...
            if ("C_InvoiceLine".Equals(table, StringComparison.OrdinalIgnoreCase))
                return DocLineDisplayExpr("C_Invoice", "C_Invoice_ID");
            return null;
        }

        private Dictionary<string, RefLookupDef> _refDefByColumn;

        /// <summary>Resolves a M_InOutLine FK column to its lookup table, key and display expression.</summary>
        ...
        private RefLookupDef ResolveRefLookup(string columnName)
        {
            ...
            // Fallback: derive the lookup table from the column-name TableDir convention for
            // a column that is absent or inactive in AD_Column.
        ...
        /// <summary>
        /// SELECT-list item for one optional M_InOut column that a shipment-line field's
        /// DisplayLogic reads as a token. The column where the dictionary has it, NULL where
        /// it does not, always under its own name — NULL rather than a literal default on
        /// purpose, so the logic can tell "not set" from a real value.
        /// </summary>
        /// <param name="column">M_InOut column named by a DisplayLogic token</param>
        ...
        /// <summary>
        /// M_InOut columns that M_InOutLine field DisplayLogic / ReadOnlyLogic refer to by
        /// token, and that the panel therefore has to carry on the header for the logic to
        /// evaluate at all. Each is guarded, so a schema without one simply reports it as
        /// absent.
        /// </summary>
        ...
        /// at 0 — which every caller reads as "no access" — when the role cannot see the
        /// record OR the document's direction is not the one this panel serves.
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="M_InOut_ID">parent shipment</param>
        /// <param name="data">object to populate</param>
        private void LoadParentContext(Ctx ctx, int M_InOut_ID, DeliveryPanelData data)
        {
            ...
            bool isSO = Util.GetValueOfString(r["IsSOTrx"]) == "Y";
            // Direction gate. This panel and VAS_249 edit the same table and their endpoints
            // differ only by route, so without this a receipt id posted here would be edited
            // under the delivery panel's rules. Refusing leaves M_InOut_ID at 0.
            if (isSO != IS_SOTRX)
            {
                log.Warning(MSG + "GetPanelData: M_InOut " + M_InOut_ID
                    + " has IsSOTrx=" + (isSO ? "Y" : "N") + ", which this panel does not serve.");
                return;
            }

            data.M_InOut_ID = Util.GetValueOfInt(r["M_InOut_ID"]);
            ...
            // Header values named by shipment-line DisplayLogic tokens. A DBNull stays absent
            // from the bag rather than becoming "" or 0, so the client can tell "not set"
            // from a real value and "@token@=null" evaluates as intended.
        ...
        private static string LogicValue(DeliveryPanelData data, string column)
        {
            string v;
        ...
        /// <summary>Loads one page of saved shipment lines, with the full column bag and display labels.</summary>
        /// <param name="ctx">session context</param>
        /// <param name="M_InOut_ID">parent shipment</param>
        ...
        private List<DeliveryLineRow> LoadLines(Ctx ctx, int M_InOut_ID, int page, out int total)
        {
            List<DeliveryLineRow> rows = new List<DeliveryLineRow>();

            string countSql = "SELECT COUNT(*) FROM M_InOutLine iol"
                + " WHERE iol.M_InOut_ID = @M_InOut_ID AND iol.IsActive = 'Y'";
            countSql = MRole.GetDefault(ctx).AddAccessSQL(countSql, "iol", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO);
            total = Util.GetValueOfInt(DB.ExecuteScalar(countSql,
                new SqlParameter[] { new SqlParameter("@M_InOut_ID", M_InOut_ID) }, null));

            StringBuilder cols = new StringBuilder();
            foreach (string cn in GetInOutLineColumns())
                cols.Append("iol.").Append(cn).Append(", ");
            if (cols.Length == 0)
                cols.Append("iol.M_InOutLine_ID, iol.M_InOut_ID, iol.Line, iol.M_Product_ID, iol.C_Charge_ID, iol.M_Locator_ID, iol.C_UOM_ID, iol.QtyEntered, iol.MovementQty, iol.M_AttributeSetInstance_ID, iol.Description, ");
            ...
            string sql = "SELECT " + cols.ToString() +
               ...
               LEFT JOIN C_Charge ch ON (iol.C_Charge_ID = ch.C_Charge_ID)
            ...
            {
                DeliveryLineRow row = new DeliveryLineRow();
                foreach (DataColumn dc in dt.Columns)
                {
                    ...
                    // M_InOutLine columns are sent back on save.
                    if (dc.ColumnName.StartsWith("VASDOLDISP_", StringComparison.OrdinalIgnoreCase)) continue;
                    row.Values[dc.ColumnName] = (r[dc] == DBNull.Value) ? null : r[dc];
                }
                row.M_InOutLine_ID = Util.GetValueOfInt(r["M_InOutLine_ID"]);
                row.Line = Util.GetValueOfInt(r["Line"]);
                row.M_Product_ID = Util.GetValueOfInt(r["M_Product_ID"]);
                row.ProductValue = Util.GetValueOfString(r["VASDOLDISP_ProductValue"]);
                row.ProductName = Util.GetValueOfString(r["VASDOLDISP_ProductName"]);
                row.C_Charge_ID = dt.Columns.Contains("C_Charge_ID") ? Util.GetValueOfInt(r["C_Charge_ID"]) : 0;
                row.ChargeName = Util.GetValueOfString(r["VASDOLDISP_ChargeName"]);
                row.Description = Util.GetValueOfString(r["Description"]);
                row.MovementQty = Util.GetValueOfDecimal(r["MovementQty"]);
                // QtyEntered is the quantity in the line's SELECTED unit. A line created by a
                // process (rather than by the shipment window) can carry MovementQty alone,
                ...
                row.UOMName = Util.GetValueOfString(r["VASDOLDISP_UOMName"]);
                ...
                row.Values["VASDOLDISP_HasAttrSet"] = hasAttrSetRaw;
                row.ProductType = Util.GetValueOfString(r["VASDOLDISP_ProductType"]);
                rows.Add(row);
            }
            return rows;
        }
        ...
        /// <summary>
        /// Summed MovementQty across every saved line — the quantity figure the totals strip
        /// states, read from the lines so it is right the moment one is saved.
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="M_InOut_ID">parent shipment</param>
        /// <returns>total shipped quantity</returns>
        private decimal SumQty(Ctx ctx, int M_InOut_ID)
        {
            string sql = "SELECT COALESCE(SUM(iol.MovementQty), 0) AS Qty FROM M_InOutLine iol"
                + " WHERE iol.M_InOut_ID = @M_InOut_ID AND iol.IsActive = 'Y'";
            sql = MRole.GetDefault(ctx).AddAccessSQL(sql, "iol", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO);
            DataSet ds = DB.ExecuteDataset(sql,
                new SqlParameter[] { new SqlParameter("@M_InOut_ID", M_InOut_ID) }, null);
        ...
        private List<string> _iolColumns;

        /// <summary>Returns (and caches) the active M_InOutLine column names from the dictionary.</summary>
        /// <returns>column names</returns>
        private List<string> GetInOutLineColumns()
        {
            if (_iolColumns != null) return _iolColumns;
            _iolColumns = new List<string>();
            DataSet ds = DB.ExecuteDataset(
                @"SELECT c.ColumnName
                  FROM AD_Column c
                  INNER JOIN AD_Table t ON (c.AD_Table_ID = t.AD_Table_ID)
            ...
            if (ds != null && ds.Tables.Count > 0)
                foreach (DataRow r in ds.Tables[0].Rows)
                    _iolColumns.Add(Util.GetValueOfString(r["ColumnName"]));
            return _iolColumns;
        }
        ...
        /// <summary>
        /// Paged Product / Charge catalog search for the line picker. M_InOutLine carries
        /// C_Charge_ID, so charges are offered alongside products.
        /// </summary>
        /// <param name="ctx">session context</param>
        ...
        /// <param name="rowValues">compact line context for the lookup AD_Val_Rules</param>
        /// <returns>matching catalog rows</returns>
        public List<DeliveryCatalogItem> SearchProductsCharges(Ctx ctx, int M_InOut_ID,
            string query, int pageSize, int offset, Dictionary<string, object> rowValues = null)
        {
            List<DeliveryCatalogItem> items = new List<DeliveryCatalogItem>();
            if (M_InOut_ID <= 0) return items;
            ...
            // parameter array no longer lines up.
            ...
            Dictionary<int, string> uomNames = LoadUomNames(ctx, M_InOut_ID);
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                DeliveryCatalogItem it = new DeliveryCatalogItem();
        ...
        public DeliveryCatalogItem ScanLookup(Ctx ctx, int M_InOut_ID, string code)
        {
            ...
            string scanChargePred = GetValRulePredicate(ctx, "C_Charge_ID", "C_Charge", "ch", M_InOut_ID, null);
        ...
        private Dictionary<string, string> _ioVars;

        /// <summary>Returns the SQL validation code linked to a M_InOutLine lookup column.</summary>
        ...
        /// @tokens@ from the line values first, then the shipment header, then the session
        /// context. A rule with an unresolved token is dropped rather than applied wrong.
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="columnName">lookup column on M_InOutLine</param>
        ...
        private string GetValRulePredicate(Ctx ctx, string columnName, string tableName, string alias,
            int M_InOut_ID, Dictionary<string, string> rowVars)
        {
            ...
            Dictionary<string, string> oVars = GetInOutVars(ctx, M_InOut_ID);
            ...
            if (frag.IndexOf('@') >= 0)
            {
                log.Warning("VAS_248 val rule skipped for " + columnName + " (unresolved context): " + code);
                return "";
            }
            // An FK token that resolved to 0 means the document simply does not carry that
            // reference (a shipment raised without an order, say). Substituting it leaves a
            // predicate like "C_Order_ID = 0", which no row can satisfy, so the lookup would
            // come back EMPTY rather than merely narrowed. Dropping the rule and offering
            // the role-secured, warehouse-scoped list is the honest answer.
            if (zeroId)
            {
                log.Fine("VAS_248 val rule skipped for " + columnName
        ...
        /// <summary>
        /// Parent-shipment context values (as SQL literals) for val-rule substitution.
        /// IsSOTrx is stated as 'Y': this panel serves customer shipments, and the standard
        /// product / charge val rules gate on that token.
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="M_InOut_ID">parent shipment</param>
        /// <returns>token to SQL literal map</returns>
        private Dictionary<string, string> GetInOutVars(Ctx ctx, int M_InOut_ID)
        {
            if (_ioVars != null) return _ioVars;
            _ioVars = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
            string orderCol = ColumnExists(PARENT_TABLE, "C_Order_ID") ? "io.C_Order_ID" : "0";
            DataSet ds = DB.ExecuteDataset(
                @"SELECT io.AD_Client_ID, io.AD_Org_ID, io.C_BPartner_ID, io.M_Warehouse_ID,
                         " + orderCol + @" AS C_Order_ID
                  FROM M_InOut io
                  WHERE io.M_InOut_ID = @id",
                new SqlParameter[] { new SqlParameter("@id", M_InOut_ID) }, null);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                DataRow r = ds.Tables[0].Rows[0];
                _ioVars["AD_Client_ID"] = Util.GetValueOfInt(r["AD_Client_ID"]).ToString();
                _ioVars["AD_Org_ID"] = Util.GetValueOfInt(r["AD_Org_ID"]).ToString();
                _ioVars["C_BPartner_ID"] = Util.GetValueOfInt(r["C_BPartner_ID"]).ToString();
                _ioVars["M_Warehouse_ID"] = Util.GetValueOfInt(r["M_Warehouse_ID"]).ToString();
                _ioVars["C_Order_ID"] = Util.GetValueOfInt(r["C_Order_ID"]).ToString();
                _ioVars["M_InOut_ID"] = M_InOut_ID.ToString();
                _ioVars["IsSOTrx"] = "'Y'";
            }
            return _ioVars;
        }
        ...
        ///
        /// For a shipment there is nothing to price: what a product / unit / quantity change
        /// actually determines is the line's unit and the base-unit MovementQty that
        /// MInOutLine stores alongside QtyEntered. No row is written.
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="req">current line state</param>
        /// <returns>changed columns + display labels</returns>
        public DeliveryCalloutResult RunColumnCallout(Ctx ctx, DeliveryLineCalcRequest req)
        {
            DeliveryCalloutResult res = new DeliveryCalloutResult();
            if (req == null || req.M_InOut_ID <= 0) return res;
            ...
            if (uom <= 0 && req.C_Charge_ID > 0) uom = MUOM.GetDefault_UOM_ID(ctx);
            ...
            // Product and charge are mutually exclusive on the line; state the clear so the
            // client never keeps both.
        ...
        /// unit. A product with no conversion defined for that unit returns null from the
        /// framework, in which case the entered figure stands rather than the line silently
        /// collapsing to a zero-quantity shipment.
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="M_Product_ID">product being shipped</param>
        ...
        /// <summary>
        /// Returns the product's attribute-set definition (attributes + allowed values) for
        /// the attribute picker dialog.
        /// </summary>
        /// <param name="ctx">session context</param>
        ...
        public DeliveryAttributeSetInfo GetProductAttributes(Ctx ctx, int M_Product_ID)
        {
            DeliveryAttributeSetInfo info = new DeliveryAttributeSetInfo();
            ...
            Dictionary<int, DeliveryAttributeDef> map = new Dictionary<int, DeliveryAttributeDef>();
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                int attrId = Util.GetValueOfInt(r["M_Attribute_ID"]);
                DeliveryAttributeDef def;
                if (!map.TryGetValue(attrId, out def))
                {
                    def = new DeliveryAttributeDef();
                    ...
                    def.Values = new List<DeliveryAttributeValueDef>();
                    map[attrId] = def;
                ...
                if (valId > 0)
                {
                    DeliveryAttributeValueDef v = new DeliveryAttributeValueDef();
        ...
        public List<DeliveryAttributeInstanceValue> GetInstanceValues(Ctx ctx, int M_AttributeSetInstance_ID)
        {
            List<DeliveryAttributeInstanceValue> list = new List<DeliveryAttributeInstanceValue>();
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                DeliveryAttributeInstanceValue v = new DeliveryAttributeInstanceValue();
        ...
        /// delegating to the framework's PAttributesModel.SaveAttribute, so dedup, mandatory
        /// validation and AttrCode / UPC behaviour stay identical to the standard attribute
        /// control.
        /// </summary>
        /// <param name="ctx">session context</param>
        ...
        public DeliveryAttributeSaveResult SaveAttribute(Ctx ctx, DeliveryAttributeSaveRequest req)
        {
            DeliveryAttributeSaveResult res = new DeliveryAttributeSaveResult();
        ...
        private List<KeyNamePair> BuildAttributeValueList(MAttributeSet aset, List<DeliveryAttributeValueSelection> selections)
        {
            List<KeyNamePair> values = new List<KeyNamePair>();
            if (aset == null) return values;

            Dictionary<int, DeliveryAttributeValueSelection> byAttr = new Dictionary<int, DeliveryAttributeValueSelection>();
            if (selections != null)
            {
                foreach (DeliveryAttributeValueSelection sel in selections)
                {
            ...
            {
                DeliveryAttributeValueSelection sel;
        ...
        #region Write actions (insert / update / delete M_InOutLine)

        /// <summary>
        /// Columns SaveLines writes itself through the business setters. ApplyExtraColumns
        /// must never touch them — in particular QtyEntered / MovementQty, whose stale
        /// client values would otherwise be re-applied AFTER the fresh ones.
        /// </summary>
        private static readonly HashSet<string> CORE_OR_SYSTEM_COLUMNS = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            "M_Product_ID", "C_Charge_ID", "M_AttributeSetInstance_ID", "M_Locator_ID",
        ...
        /// <summary>Persists every non-core, updateable M_InOutLine column through PO.Set_Value.</summary>
        ...
        private void ApplyExtraColumns(MInOutLine line, Dictionary<string, object> values, HashSet<string> touched)
        {
            ...
            foreach (KeyValuePair<string, object> kv in values)
            {
                ...
                try
                {
                    ...
                    if (GetReferenceColumns().Contains(col))
                    {
                        ...
                        if (blank)
                        {
                            // An intentional clear is persisted as NULL; an untouched blank FK is
                            // left alone so a default set elsewhere is not wiped.
        ...
        /// <summary>
        /// Inserts or updates the supplied shipment lines through MInOutLine. All lines
        ...
        public DeliverySaveResult SaveLines(Ctx ctx, int M_InOut_ID, int AD_Window_ID, List<DeliveryLineInput> rows, int page = 0)
        {
            DeliverySaveResult res = new DeliverySaveResult();
            res.LinePageSize = LINE_PAGE_SIZE;
            if (M_InOut_ID <= 0 || rows == null || rows.Count == 0)
            {
                res.ErrorKey = MSG + "NothingToSave";
                return res;
            }

            DeliveryPanelData head = new DeliveryPanelData();
            LoadParentContext(ctx, M_InOut_ID, head);
            if (head.M_InOut_ID <= 0) { res.ErrorKey = MSG + "NoAccess"; return res; }
            if (!head.IsEditable) { res.ErrorKey = MSG + "NotEditable"; return res; }

            Trx trx = Trx.GetTrx(Trx.CreateTrxName("VAS248Save_" + M_InOut_ID));
            try
            {
                ...
                foreach (DeliveryLineInput input in rows)
                {
                    ...
                    MInOutLine line = input.M_InOutLine_ID > 0
                        ? new MInOutLine(ctx, input.M_InOutLine_ID, trx)
                        : new MInOutLine(inout);

                    if (input.M_Product_ID > 0)
                    {
                        // setUOM:true makes the framework default the line's unit from the
                        // product. It also RESETS M_AttributeSetInstance_ID to 0, which is why
                        // the instance is applied after this call and never before.
                    ...
                    // The unit must be in place BEFORE the quantity: MInOutLine.SetQtyEntered
                    // rounds to the unit's precision, so setting it first would round against
                    // the previous unit.
                    int uom = input.C_UOM_ID;
                    if (uom <= 0 && input.M_Product_ID > 0) uom = GetProductUomId(ctx, input.M_Product_ID);
                    ...
                    if (!line.Save())
                    {
                        ...
                        res.LineErrors.Add(new DeliveryLineSaveError
                        {
                            RowKey = input.RowKey,
                            M_InOutLine_ID = input.M_InOutLine_ID,
                            Line = input.Line,
                            Message = err
                        });
                    }
            ...
            {
                trx.Rollback();
                log.Log(Level.SEVERE, "VAS_248 SaveLines failed", ex);
                res.ErrorKey = MSG + "SaveFailed";
        ...
        /// <returns>refreshed page, or the failure</returns>
        public DeliverySaveResult DeleteLines(Ctx ctx, int M_InOut_ID, int AD_Window_ID, List<int> lineIds, int page = 0)
        {
            DeliverySaveResult res = new DeliverySaveResult();
            res.LinePageSize = LINE_PAGE_SIZE;
            if (M_InOut_ID <= 0 || lineIds == null || lineIds.Count == 0)
            {
                res.ErrorKey = MSG + "NothingToSave";
                return res;
            }
            ...
            Trx trx = Trx.GetTrx(Trx.CreateTrxName("VAS248Delete_" + M_InOut_ID));
            try
            {
                foreach (int id in lineIds)
                {
                    if (id <= 0) continue;
                    MInOutLine line = new MInOutLine(ctx, id, trx);
                    // Guard against a stale / forged id: the line must exist AND belong to the
                    // shipment the panel is bound to.
                    if (line.Get_ID() != id) continue;
                    if (line.GetM_InOut_ID() != M_InOut_ID) continue;
                    if (!line.Delete(true, trx))
                    {
                        trx.Rollback();
                        string err = FrameworkError(ctx);
                        if (string.IsNullOrEmpty(err)) err = Msg.GetMsg(ctx, MSG + "DeleteFailed");
                        log.Warning("VAS_248 DeleteLines: delete failed for line " + id);
                        res.ErrorKey = err;
                        return res;
                    }
                }
            ...
            {
                trx.Rollback();
                log.Log(Level.SEVERE, "VAS_248 DeleteLines failed", ex);
        ...
        /// classes report a failed Save()/Delete() this way rather than by throwing, so this
        /// is the only place the real reason can be read.
        /// </summary>
        /// <param name="ctx">session context</param>
    ...
    #region Data contracts — VAS_248 specific

    /// <summary>Parent shipment context + saved lines returned to the panel on load.</summary>
    public class DeliveryPanelData
    {
        public int M_InOut_ID { get; set; }
        public int AD_Client_ID { get; set; }
        public int AD_Org_ID { get; set; }
        /// <summary>M_InOut.DocumentNo — shown in the panel subtitle.</summary>
        public string DocumentNo { get; set; }
        /// <summary>C_DocType name, when the shipment carries one.</summary>
        public string DocTypeName { get; set; }
        /// <summary>Always true here — this panel serves customer shipments only.</summary>
        public bool IsSOTrx { get; set; }
        public int C_BPartner_ID { get; set; }
        /// <summary>The sales order this shipment delivers, where the header names one.</summary>
        ...
        /// resolves to "" — so "@token@=null" matches and "@token@&gt;0" does not.
        /// </summary>
        ...
        public List<DeliveryLineRow> Lines { get; set; }
        public List<DeliveryUomItem> UomList { get; set; }
        public List<DeliveryLocatorItem> LocatorList { get; set; }
        public List<DeliveryColumnMeta> Columns { get; set; }
        public Dictionary<string, string> LoginContext { get; set; }

        public DeliveryPanelData()
        {
            Lines = new List<DeliveryLineRow>();
            UomList = new List<DeliveryUomItem>();
            LocatorList = new List<DeliveryLocatorItem>();
            Columns = new List<DeliveryColumnMeta>();
    ...
    /// <summary>A saved shipment line as the panel grid renders it.</summary>
    public class DeliveryLineRow
    {
        ...
        /// <summary>Quantity in the product's BASE unit — what the document ships.</summary>
        public decimal MovementQty { get; set; }
        public int C_UOM_ID { get; set; }
        public string UOMName { get; set; }
        /// <summary>The bin the goods leave from.</summary>
        ...
        /// <summary>Every M_InOutLine column, so the modal and the save carry them all.</summary>
        public Dictionary<string, object> Values { get; set; }

        public DeliveryLineRow()
        {
    ...
    public class DeliveryLookupRequest
    {
        public int M_InOut_ID { get; set; }
        public string ColumnName { get; set; }
        public Dictionary<string, object> RowValues { get; set; }
        public DeliveryLookupRequest() { RowValues = new Dictionary<string, object>(); }
    }

    /// <summary>Per-row filtered option lists for one shipment line.</summary>
    public class DeliveryLookupData
    {
        public int M_InOut_ID { get; set; }
        public List<DeliveryUomItem> UomList { get; set; }
        public List<DeliveryLocatorItem> LocatorList { get; set; }
        public DeliveryLookupData()
        {
            UomList = new List<DeliveryUomItem>();
            LocatorList = new List<DeliveryLocatorItem>();
        }
    ...
    /// <summary>Request for the generic FK lookup of a dynamic M_InOutLine field.</summary>
    public class DeliveryRefLookupRequest
    {
        public int M_InOut_ID { get; set; }
        public string ColumnName { get; set; }
        ...
        public Dictionary<string, object> RowValues { get; set; }
        public DeliveryRefLookupRequest() { RowValues = new Dictionary<string, object>(); }
    }

    /// <summary>One id + label row of a generic FK lookup.</summary>
    public class DeliveryRefItem
    {
        public int Id { get; set; }
        public string Name { get; set; }
        public DeliveryRefItem() { Name = ""; }
    }

    /// <summary>Inbound callout request from the delivery panel.</summary>
    public class DeliveryLineCalcRequest
    {
        public int M_InOut_ID { get; set; }
    ...
    public class DeliveryCalloutResult
    {
    ...
    /// <summary>One inbound shipment line to insert / update.</summary>
    ...
    public class DeliverySaveLinesRequest
    {
        public int M_InOut_ID { get; set; }
        public int AD_Window_ID { get; set; }
        public int Page { get; set; }
        public List<DeliveryLineInput> Lines { get; set; }
        public DeliverySaveLinesRequest() { Lines = new List<DeliveryLineInput>(); }
    }

    /// <summary>POST body for the DeleteLines action.</summary>
    public class DeliveryDeleteLinesRequest
    {
        public int M_InOut_ID { get; set; }
        public int AD_Window_ID { get; set; }
        public int Page { get; set; }
        public List<int> LineIds { get; set; }
        public DeliveryDeleteLinesRequest() { LineIds = new List<int>(); }
    }

    /// <summary>One line that would not save, reported back against its client row.</summary>
    public class DeliveryLineSaveError
    {
        public string RowKey { get; set; }
        public int M_InOutLine_ID { get; set; }
        public int Line { get; set; }
        public string Message { get; set; }
        public DeliveryLineSaveError() { RowKey = ""; Message = ""; }
    }
    ...
    /// </summary>
    public class DeliverySaveResult
    {
        ...
        public List<DeliveryLineSaveError> LineErrors { get; set; }
        public List<DeliveryLineRow> Lines { get; set; }
        public int LinesTotal { get; set; }
        public int LinePage { get; set; }
        public int LinePageSize { get; set; }
        ...
        public DeliverySaveResult()
        {
            ErrorKey = "";
            ErrorDetail = "";
            LineErrors = new List<DeliveryLineSaveError>();
            Lines = new List<DeliveryLineRow>();
        }
    }

    /// <summary>A UOM option for the quantity cell.</summary>
    public class DeliveryUomItem
    {
        public int C_UOM_ID { get; set; }
        public string Name { get; set; }
    ...
    public class DeliveryCatalogItem
    {
        /// <summary>M_Product_ID when Kind is "P", C_Charge_ID when Kind is "C".</summary>
        public int RecordId { get; set; }
        /// <summary>"P" (product) or "C" (charge).</summary>
    ...
    public class DeliveryRefListItem
    {
        public string Value { get; set; }
        public string Name { get; set; }
        public DeliveryRefListItem() { Value = ""; Name = ""; }
    }

    /// <summary>Dictionary metadata for one M_InOutLine column.</summary>
    public class DeliveryColumnMeta
    {
        public string ColumnName { get; set; }
        ...
        public List<DeliveryRefListItem> RefListValues { get; set; }

        public DeliveryColumnMeta()
        {
            ColumnName = "";
            Callout = "";
            Name = "";
            ...
            IconFont = "";
            ImageUrl = "";
            RefListValues = new List<DeliveryRefListItem>();
        }
    }

    /// <summary>Attribute-set definition behind the attribute picker.</summary>
    public class DeliveryAttributeSetInfo
    {
        ...
        public List<DeliveryAttributeDef> Attributes { get; set; }

        public DeliveryAttributeSetInfo()
        {
            ProductName = "";
            GuaranteeDateDefault = "";
            Attributes = new List<DeliveryAttributeDef>();
        }
    }

    /// <summary>One attribute of a product's attribute set.</summary>
    public class DeliveryAttributeDef
    {
        ...
        public bool IsMandatory { get; set; }
        public List<DeliveryAttributeValueDef> Values { get; set; }

        public DeliveryAttributeDef()
        {
            Name = "";
            ValueType = "";
            Values = new List<DeliveryAttributeValueDef>();
        }
    }

    /// <summary>One allowed value of a list attribute.</summary>
    public class DeliveryAttributeValueDef
    {
        public int M_AttributeValue_ID { get; set; }
        public string Code { get; set; }
        public string Name { get; set; }
        public DeliveryAttributeValueDef() { Code = ""; Name = ""; }
    }

    /// <summary>One typed value stored on an existing attribute-set instance.</summary>
    public class DeliveryAttributeInstanceValue
    {
        public int M_Attribute_ID { get; set; }
        public string ValueType { get; set; }
        public int M_AttributeValue_ID { get; set; }
        ...
        public DeliveryAttributeInstanceValue() { ValueType = ""; StringValue = ""; }
    }

    /// <summary>Attribute-instance create / update request from the delivery panel.</summary>
    public class DeliveryAttributeSaveRequest
    {
        public int M_Product_ID { get; set; }
        public int M_AttributeSetInstance_ID { get; set; }
        ...
        public List<DeliveryAttributeValueSelection> Values { get; set; }
    }

    /// <summary>One entered attribute value in a delivery-panel attribute save request.</summary>
    public class DeliveryAttributeValueSelection
    {
        public int M_Attribute_ID { get; set; }
        public string ValueType { get; set; }
        public int M_AttributeValue_ID { get; set; }
    ...
    public class DeliveryAttributeSaveResult
    {
        public int M_AttributeSetInstance_ID { get; set; }
        public string Description { get; set; }
        public string Error { get; set; }
        public DeliveryAttributeSaveResult() { Description = ""; Error = ""; }
    }
