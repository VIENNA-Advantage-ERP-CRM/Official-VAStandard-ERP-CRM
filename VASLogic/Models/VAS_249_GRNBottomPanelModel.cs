 ...
 * Purpose        : Backing model for the VAS_249_GRNBottomPanel tab
 *                  panel — the vendor-receipt counterpart of
 *                  VAS_240_RequisitionBottomPanel. Provides parent-receipt
 ...
 *                  business class).
 *
 *                  A receipt line carries NO price, tax or discount — those
 ...
 *                  model serves M_InOut rows with IsSOTrx = 'N' ONLY: a customer
 *                  customer shipment id posted to this panel's endpoint is refused, so the
 *                  GRN panel can never be used to edit a GRN. VAS_249 is the
...
namespace VASLogic.Models
{
    /// <summary>
    /// Module Name : VASLogic
    /// Purpose     : Data + write model behind the GRN Bottom Panel
    ///               (M_InOut with IsSOTrx = 'N' / M_InOutLine). Every SELECT is
    ...
    ///               a hand-written INSERT) so the standard receipt-line logic runs.
    /// Chronological development:
    ///   VAI154         Created  07-Sep-2026
    ///   VAI154         Rebuilt on the VAS_240 pattern  09-Sep-2026
    /// </summary>
    public class VAS_249_GRNBottomPanelModel
    {
        private static VLogger log = VLogger.GetVLogger(typeof(VAS_249_GRNBottomPanelModel).FullName);
        ...
        private const string MSG = "VAS_249_";

        /// <summary>
        /// The document direction this panel serves. A vendor receipt is IsSOTrx = 'N';
        /// LoadParentContext refuses any other, which is what stops a receipt being edited
        /// through this panel's route.
        /// </summary>
        private const bool IS_SOTRX = false;

        #region Panel (read) data

        /// <summary>
        /// Builds the panel header context (everything the client-side logic needs from the
        /// parent receipt) plus one page of already-saved lines. Returns an empty object
        /// (M_InOut_ID = 0) when the role has no access OR the document is not a receipt.
        /// </summary>
        ...
        public ReceiptPanelData GetPanelData(Ctx ctx, int M_InOut_ID, int AD_Window_ID, int page = 0)
        {
            ReceiptPanelData data = new ReceiptPanelData();
        ...
        private void LoadLoginContext(Ctx ctx, ReceiptPanelData data)
        {
            Regex rx = new Regex(@"@([#$][A-Za-z0-9_]+)@");
            HashSet<string> tokens = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (ReceiptColumnMeta m in data.Columns)
            {
        ...
        /// <param name="AD_Tab_IDs">receipt-line tabs of the source window</param>
        private void LoadColumns(Ctx ctx, ReceiptPanelData data, List<int> AD_Tab_IDs)
        {
        ...
        private void LoadListValues(ReceiptPanelData data)
        {
            foreach (ReceiptColumnMeta m in data.Columns)
            {
                ...
                foreach (DataRow r in ds.Tables[0].Rows)
                    m.RefListValues.Add(new ReceiptRefListItem
                    {
        ...
        /// <summary>Reads AD_Field -&gt; AD_Column metadata across all the window's receipt-line tabs.</summary>
        /// <param name="ctx">session context</param>
        /// <param name="data">panel data to populate</param>
        /// <param name="AD_Tab_IDs">receipt-line tabs</param>
        /// <returns>whether any column was found</returns>
        private bool LoadColumnsFromTabs(Ctx ctx, ReceiptPanelData data, List<int> AD_Tab_IDs)
        {
        ...
        /// <param name="ctx">session context</param>
        /// <param name="data">panel data to populate</param>
        private void MergeAllColumns(Ctx ctx, ReceiptPanelData data)
        {
            HashSet<string> have = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (ReceiptColumnMeta cm in data.Columns) have.Add(cm.ColumnName);
        ...
        private ReceiptColumnMeta MapColumnMeta(DataRow r, bool fromField)
        {
            ReceiptColumnMeta m = new ReceiptColumnMeta
            {
        ...
        /// catalog: a receipt line carries neither.
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="data">panel data to populate</param>
        ...
        private List<ReceiptUomItem> LoadUomList(Ctx ctx, int M_InOut_ID, Dictionary<string, string> rowVars)
        {
        ...
        private List<ReceiptLocatorItem> LoadLocatorList(Ctx ctx, int M_InOut_ID, int M_Warehouse_ID,
        ...
        /// <summary>
        /// Re-fetches the per-row filtered UOM and locator lists for one receipt line,
        ...
        public ReceiptLookupData GetLookupData(Ctx ctx, ReceiptLookupRequest req)
        {
            ReceiptLookupData data = new ReceiptLookupData();
            if (req == null || req.M_InOut_ID <= 0) return data;

            ReceiptPanelData parent = new ReceiptPanelData();
        ...
        public List<ReceiptRefItem> GetRefLookup(Ctx ctx, ReceiptRefLookupRequest req)
        {
            List<ReceiptRefItem> items = new List<ReceiptRefItem>();
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
                items.Add(new ReceiptRefItem { Id = Util.GetValueOfInt(r["Id"]), Name = Util.GetValueOfString(r["Name"]) });
            return items;
        }
        ...
        private static readonly Dictionary<string, string[]> TableDirOverrides =
            new Dictionary<string, string[]>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                // The order line this receipt line delivers, and the RMA line a return
        ...
        /// <summary>
        /// SELECT-list item for one optional M_InOut column that a receipt-line field's
        ...
        private void LoadParentContext(Ctx ctx, int M_InOut_ID, ReceiptPanelData data)
        {
            ...
            // differ only by route, so without this a shipment id posted here would be edited
            // under the GRN panel's rules. Refusing leaves M_InOut_ID at 0.
            if (isSO != IS_SOTRX)
            {
            ...
            // Header values named by receipt-line DisplayLogic tokens. A DBNull stays absent
        ...
        private static string LogicValue(ReceiptPanelData data, string column)
        {
            string v;
        ...
        /// <summary>Loads one page of saved receipt lines, with the full column bag and display labels.</summary>
        /// <param name="ctx">session context</param>
        /// <param name="M_InOut_ID">parent receipt</param>
        ...
        private List<ReceiptLineRow> LoadLines(Ctx ctx, int M_InOut_ID, int page, out int total)
        {
            List<ReceiptLineRow> rows = new List<ReceiptLineRow>();
            ...
            {
                ReceiptLineRow row = new ReceiptLineRow();
                foreach (DataColumn dc in dt.Columns)
                {
                    ...
                    if (dc.ColumnName.StartsWith("VASGRNDISP_", StringComparison.OrdinalIgnoreCase)) continue;
                ...
                row.ProductValue = Util.GetValueOfString(r["VASGRNDISP_ProductValue"]);
                row.ProductName = Util.GetValueOfString(r["VASGRNDISP_ProductName"]);
                row.C_Charge_ID = dt.Columns.Contains("C_Charge_ID") ? Util.GetValueOfInt(r["C_Charge_ID"]) : 0;
                row.ChargeName = Util.GetValueOfString(r["VASGRNDISP_ChargeName"]);
                ...
                // process (rather than by the receipt window) can carry MovementQty alone,
                ...
                row.UOMName = Util.GetValueOfString(r["VASGRNDISP_UOMName"]);
                ...
                row.Values["VASGRNDISP_HasAttrSet"] = hasAttrSetRaw;
                row.ProductType = Util.GetValueOfString(r["VASGRNDISP_ProductType"]);
                rows.Add(row);
            }
            return rows;
        }
        ...
        /// <param name="ctx">session context</param>
        /// <param name="M_InOut_ID">parent receipt</param>
        /// <returns>total received quantity</returns>
        ...
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="M_InOut_ID">parent receipt (guards the call, and client scope)</param>
        ...
        public List<ReceiptCatalogItem> SearchProductsCharges(Ctx ctx, int M_InOut_ID,
            string query, int pageSize, int offset, Dictionary<string, object> rowValues = null)
        {
            List<ReceiptCatalogItem> items = new List<ReceiptCatalogItem>();
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                ReceiptCatalogItem it = new ReceiptCatalogItem();
        ...
        /// <param name="ctx">session context</param>
        /// <param name="M_InOut_ID">parent receipt (client scope)</param>
        /// <param name="code">scanned code</param>
        /// <returns>the matched catalog row, or an empty item</returns>
        public ReceiptCatalogItem ScanLookup(Ctx ctx, int M_InOut_ID, string code)
        {
        ...
        /// @tokens@ from the line values first, then the receipt header, then the session
            ...
            int M_InOut_ID, Dictionary<string, string> rowVars)
        {
            ...
            if (frag.IndexOf('@') >= 0)
            {
                log.Warning("VAS_249 val rule skipped for " + columnName + " (unresolved context): " + code);
                return "";
            }
            ...
            if (zeroId)
            {
                log.Fine("VAS_249 val rule skipped for " + columnName
        ...
        /// <summary>
        /// Parent-receipt context values (as SQL literals) for val-rule substitution.
        /// IsSOTrx is stated as 'N': this panel serves vendor receipts, and the standard
        ...
        ///
        /// For a receipt there is nothing to price: what a product / unit / quantity change
        ...
        public ReceiptCalloutResult RunColumnCallout(Ctx ctx, ReceiptLineCalcRequest req)
        {
            ReceiptCalloutResult res = new ReceiptCalloutResult();
        ...
        /// collapsing to a zero-quantity receipt.
        /// </summary>
        /// <param name="ctx">session context</param>
        /// <param name="M_Product_ID">product being received</param>
        ...
        public ReceiptAttributeSetInfo GetProductAttributes(Ctx ctx, int M_Product_ID)
        {
            ReceiptAttributeSetInfo info = new ReceiptAttributeSetInfo();
            ...
            Dictionary<int, ReceiptAttributeDef> map = new Dictionary<int, ReceiptAttributeDef>();
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                int attrId = Util.GetValueOfInt(r["M_Attribute_ID"]);
                ReceiptAttributeDef def;
                if (!map.TryGetValue(attrId, out def))
                {
                    def = new ReceiptAttributeDef();
                    ...
                    def.Values = new List<ReceiptAttributeValueDef>();
                    map[attrId] = def;
                ...
                if (valId > 0)
                {
                    ReceiptAttributeValueDef v = new ReceiptAttributeValueDef();
        ...
        public List<ReceiptAttributeInstanceValue> GetInstanceValues(Ctx ctx, int M_AttributeSetInstance_ID)
        {
            List<ReceiptAttributeInstanceValue> list = new List<ReceiptAttributeInstanceValue>();
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                ReceiptAttributeInstanceValue v = new ReceiptAttributeInstanceValue();
        ...
        public ReceiptAttributeSaveResult SaveAttribute(Ctx ctx, ReceiptAttributeSaveRequest req)
        {
            ReceiptAttributeSaveResult res = new ReceiptAttributeSaveResult();
        ...
        private List<KeyNamePair> BuildAttributeValueList(MAttributeSet aset, List<ReceiptAttributeValueSelection> selections)
        {
            List<KeyNamePair> values = new List<KeyNamePair>();
            if (aset == null) return values;

            Dictionary<int, ReceiptAttributeValueSelection> byAttr = new Dictionary<int, ReceiptAttributeValueSelection>();
            if (selections != null)
            {
                foreach (ReceiptAttributeValueSelection sel in selections)
                {
            ...
            {
                ReceiptAttributeValueSelection sel;
        ...
        /// <summary>
        /// Inserts or updates the supplied receipt lines through MInOutLine. All lines
        ...
        public ReceiptSaveResult SaveLines(Ctx ctx, int M_InOut_ID, int AD_Window_ID, List<ReceiptLineInput> rows, int page = 0)
        {
            ReceiptSaveResult res = new ReceiptSaveResult();
            ...
            ReceiptPanelData head = new ReceiptPanelData();
            ...
            Trx trx = Trx.GetTrx(Trx.CreateTrxName("VAS249Save_" + M_InOut_ID));
            try
            {
                MInOut inout = new MInOut(ctx, M_InOut_ID, trx);
                foreach (ReceiptLineInput input in rows)
                {
                    ...
                    if (!line.Save())
                    {
                        ...
                        log.Warning("VAS_249 SaveLines: line save failed (Line " + input.Line + ") - " + err);
                        res.LineErrors.Add(new ReceiptLineSaveError
                        {
                            RowKey = input.RowKey,
            ...
            {
                trx.Rollback();
                log.Log(Level.SEVERE, "VAS_249 SaveLines failed", ex);
                res.ErrorKey = MSG + "SaveFailed";
        ...
        /// <summary>Deletes the supplied saved receipt lines through MInOutLine.</summary>
        /// <param name="ctx">session context</param>
        /// <param name="M_InOut_ID">parent receipt</param>
        /// <param name="AD_Window_ID">source window</param>
        ...
        /// <returns>refreshed page, or the failure</returns>
        public ReceiptSaveResult DeleteLines(Ctx ctx, int M_InOut_ID, int AD_Window_ID, List<int> lineIds, int page = 0)
        {
            ReceiptSaveResult res = new ReceiptSaveResult();
            res.LinePageSize = LINE_PAGE_SIZE;
            ...
            Trx trx = Trx.GetTrx(Trx.CreateTrxName("VAS249Delete_" + M_InOut_ID));
            try
            {
                foreach (int id in lineIds)
                {
                    ...
                    if (!line.Delete(true, trx))
                    {
                        ...
                        log.Warning("VAS_249 DeleteLines: delete failed for line " + id);
                        res.ErrorKey = err;
                        return res;
                    }
                }
            ...
            {
                trx.Rollback();
                log.Log(Level.SEVERE, "VAS_249 DeleteLines failed", ex);
    ...
    #region Data contracts — VAS_249 specific

    /// <summary>Parent receipt context + saved lines returned to the panel on load.</summary>
    public class ReceiptPanelData
    {
        ...
        /// <summary>C_DocType name, when the receipt carries one.</summary>
        public string DocTypeName { get; set; }
        /// <summary>Always false here — this panel serves vendor receipts only.</summary>
        public bool IsSOTrx { get; set; }
        public int C_BPartner_ID { get; set; }
        /// <summary>The purchase order this receipt fulfils, where the header names one.</summary>
        ...
        public List<ReceiptUomItem> UomList { get; set; }
        public List<ReceiptLocatorItem> LocatorList { get; set; }
        public List<ReceiptColumnMeta> Columns { get; set; }
        public Dictionary<string, string> LoginContext { get; set; }

        public ReceiptPanelData()
        {
            Lines = new List<ReceiptLineRow>();
            UomList = new List<ReceiptUomItem>();
            LocatorList = new List<ReceiptLocatorItem>();
            Columns = new List<ReceiptColumnMeta>();
    ...
    /// <summary>A saved receipt line as the panel grid renders it.</summary>
    public class ReceiptLineRow
    {
        ...
        /// <summary>Quantity in the product's BASE unit — what the document receives.</summary>
        public decimal MovementQty { get; set; }
        public int C_UOM_ID { get; set; }
        public string UOMName { get; set; }
        /// <summary>The bin the goods arrive in.</summary>
    ...
    public class ReceiptLookupRequest
    {
        ...
        public ReceiptLookupRequest() { RowValues = new Dictionary<string, object>(); }
    }

    /// <summary>Per-row filtered option lists for one receipt line.</summary>
    public class ReceiptLookupData
    {
        public int M_InOut_ID { get; set; }
        public List<ReceiptUomItem> UomList { get; set; }
        public List<ReceiptLocatorItem> LocatorList { get; set; }
        public ReceiptLookupData()
        {
            UomList = new List<ReceiptUomItem>();
            LocatorList = new List<ReceiptLocatorItem>();
        }
    ...
    public class ReceiptRefLookupRequest
    {
        public int M_InOut_ID { get; set; }
        public string ColumnName { get; set; }
        ...
        public Dictionary<string, object> RowValues { get; set; }
        public ReceiptRefLookupRequest() { RowValues = new Dictionary<string, object>(); }
    }

    /// <summary>One id + label row of a generic FK lookup.</summary>
    public class ReceiptRefItem
    {
        public int Id { get; set; }
        public string Name { get; set; }
        public ReceiptRefItem() { Name = ""; }
    }

    /// <summary>Inbound callout request from the GRN panel.</summary>
    public class ReceiptLineCalcRequest
    {
        public int M_InOut_ID { get; set; }
    ...
    public class ReceiptCalloutResult
    {
    ...
    /// <summary>One inbound receipt line to insert / update.</summary>
    ...
    public class ReceiptSaveLinesRequest
    {
        public int M_InOut_ID { get; set; }
        public int AD_Window_ID { get; set; }
        public int Page { get; set; }
        public List<ReceiptLineInput> Lines { get; set; }
        public ReceiptSaveLinesRequest() { Lines = new List<ReceiptLineInput>(); }
    }

    /// <summary>POST body for the DeleteLines action.</summary>
    public class ReceiptDeleteLinesRequest
    {
        public int M_InOut_ID { get; set; }
        public int AD_Window_ID { get; set; }
        public int Page { get; set; }
        public List<int> LineIds { get; set; }
        public ReceiptDeleteLinesRequest() { LineIds = new List<int>(); }
    }

    /// <summary>One line that would not save, reported back against its client row.</summary>
    public class ReceiptLineSaveError
    {
        public string RowKey { get; set; }
        public int M_InOutLine_ID { get; set; }
        public int Line { get; set; }
        public string Message { get; set; }
        public ReceiptLineSaveError() { RowKey = ""; Message = ""; }
    }
    ...
    public class ReceiptSaveResult
    {
        ...
        public List<ReceiptLineSaveError> LineErrors { get; set; }
        public List<ReceiptLineRow> Lines { get; set; }
        public int LinesTotal { get; set; }
        public int LinePage { get; set; }
        public int LinePageSize { get; set; }
        ...
        public ReceiptSaveResult()
        {
            ErrorKey = "";
            ErrorDetail = "";
            LineErrors = new List<ReceiptLineSaveError>();
            Lines = new List<ReceiptLineRow>();
        }
    }

    /// <summary>A UOM option for the quantity cell.</summary>
    public class ReceiptUomItem
    {
        public int C_UOM_ID { get; set; }
        public string Name { get; set; }
    ...
    public class ReceiptCatalogItem
    {
    ...
    public class ReceiptRefListItem
    {
        public string Value { get; set; }
        public string Name { get; set; }
        public ReceiptRefListItem() { Value = ""; Name = ""; }
    }

    /// <summary>Dictionary metadata for one M_InOutLine column.</summary>
    public class ReceiptColumnMeta
    {
        public string ColumnName { get; set; }
        ...
        public List<ReceiptRefListItem> RefListValues { get; set; }

        public ReceiptColumnMeta()
        {
            ColumnName = "";
            Callout = "";
            Name = "";
            ...
            IconFont = "";
            ImageUrl = "";
            RefListValues = new List<ReceiptRefListItem>();
        }
    }

    /// <summary>Attribute-set definition behind the attribute picker.</summary>
    public class ReceiptAttributeSetInfo
    {
        ...
        public List<ReceiptAttributeDef> Attributes { get; set; }

        public ReceiptAttributeSetInfo()
        {
            ProductName = "";
            GuaranteeDateDefault = "";
            Attributes = new List<ReceiptAttributeDef>();
        }
    }

    /// <summary>One attribute of a product's attribute set.</summary>
    public class ReceiptAttributeDef
    {
        ...
        public bool IsMandatory { get; set; }
        public List<ReceiptAttributeValueDef> Values { get; set; }

        public ReceiptAttributeDef()
        {
            Name = "";
            ValueType = "";
            Values = new List<ReceiptAttributeValueDef>();
        }
    }

    /// <summary>One allowed value of a list attribute.</summary>
    public class ReceiptAttributeValueDef
    {
        public int M_AttributeValue_ID { get; set; }
        public string Code { get; set; }
        public string Name { get; set; }
        public ReceiptAttributeValueDef() { Code = ""; Name = ""; }
    }

    /// <summary>One typed value stored on an existing attribute-set instance.</summary>
    public class ReceiptAttributeInstanceValue
    {
        public int M_Attribute_ID { get; set; }
        public string ValueType { get; set; }
        public int M_AttributeValue_ID { get; set; }
        ...
        public ReceiptAttributeInstanceValue() { ValueType = ""; StringValue = ""; }
    }

    /// <summary>Attribute-instance create / update request from the GRN panel.</summary>
    public class ReceiptAttributeSaveRequest
    {
        public int M_Product_ID { get; set; }
        public int M_AttributeSetInstance_ID { get; set; }
        ...
        public List<ReceiptAttributeValueSelection> Values { get; set; }
    }

    /// <summary>One entered attribute value in a GRN-panel attribute save request.</summary>
    public class ReceiptAttributeValueSelection
    {
        public int M_Attribute_ID { get; set; }
        public string ValueType { get; set; }
        public int M_AttributeValue_ID { get; set; }
    ...
    public class ReceiptAttributeSaveResult
    {
        public int M_AttributeSetInstance_ID { get; set; }
        public string Description { get; set; }
