    ...
    public class VAS_133_UpdatedThisMonthWidgetController : Controller
    {
        ...
        public JsonResult GetPagedUpdates(int offset = 0, int pageSize = MaxPageSize)
        {
            ...
            try
            {
                ...
                // DB.ExecuteReader binds POSITIONALLY (matches this array to
                // the left-to-right order @placeholders appear in the final
                // SQL text, not by name) - a value needed in two places (like
                // @Offset here) needs two distinct placeholders, each
                // supplied once, in the exact order they appear. See the
                // project convention in InvoicesController's @Like1/@Like2.
