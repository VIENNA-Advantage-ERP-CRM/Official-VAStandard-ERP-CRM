/************************************************************
 * Module Name    : VAS
 * Purpose        : Controller for Widget 06: Monthly Purchase Order Trend (VAS_196_MonthlyPOTrendWidget)
...
{
    public class VAS_196_MonthlyPOTrendWidgetController : Controller
    {
        private static readonly VLogger Log = VLogger.GetVLogger(typeof(VAS_196_MonthlyPOTrendWidgetController).FullName);
        ...
        public JsonResult GetTrendData(int? fromYear, int? fromMonth, int? toYear, int? toMonth)
        {
            ...
            {
                Log.Log(Level.SEVERE, "VAS_196_MonthlyPOTrendWidget.GetTrendData", ex);
        ...
        public JsonResult GetMonthPODrilldown(int year, int month)
        {
            ...
            {
                Log.Log(Level.SEVERE, "VAS_196_MonthlyPOTrendWidget.GetMonthPODrilldown", ex);
                return Json(JsonConvert.SerializeObject(new { error = Msg.GetMsg(ctx, "Error") ?? "Error", message = ex.Message }), JsonRequestBehavior.AllowGet);
            }
        }
        ...
        public JsonResult GetPOLineDetails(int orderId)
        {
            ...
            {
                Log.Log(Level.SEVERE, "VAS_196_MonthlyPOTrendWidget.GetPOLineDetails", ex);
                return Json(JsonConvert.SerializeObject(new { error = Msg.GetMsg(ctx, "Error") ?? "Error", message = ex.Message }), JsonRequestBehavior.AllowGet);
            }
        }
