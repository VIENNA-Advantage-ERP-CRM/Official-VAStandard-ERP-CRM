...
{
    /// <summary>
    /// Module Name : VAS_149_ExpectedFulfilmentsWidget (Delivery Order dashboard)
    /// Purpose     : Backend for the 3x2 "Expected Fulfilments" list widget and
    ///               its "Generate Delivery Order" modal. Sibling of Due
    ///               Fulfilments - identical in every respect except the
    ///               promised-date direction: this widget lists UPCOMING open
    ///               sales-order requirements whose promised date - COALESCE(line
    ///               DatePromised, order DatePromised) - is TODAY OR IN THE
    ///               FUTURE (>= today). A fulfilment is a completed sales order
    ///               with at least one active, product line still undelivered
    ///               (QtyOrdered > QtyDelivered). The modal converts one
    ...
    /// </summary>
    public class VAS_149_ExpectedFulfilmentsWidgetController : Controller
    {
        // The promised-date direction that defines this widget. Expected =
        // upcoming (promised date today or later). The sibling "Due Fulfilments"
        // widget is the same query with "<".
        private const string ExpectedDateFilter = "COALESCE(ol.DatePromised, o.DatePromised) >= CURRENT_DATE";

        /// <summary>
        /// One page of expected fulfilments (upcoming open sales orders),
        ...
        public JsonResult GetFulfilments(int pageNo = 1, int pageSize = 4)
        {
            ...
            // shows "Data unavailable" even though rows match).
