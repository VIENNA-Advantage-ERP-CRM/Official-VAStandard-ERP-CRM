...
{
    /// <summary>
    /// Module Name : VAS_191_POGlobalSearchWidget (Purchase Order Dashboard - Widget 01)
    ...
    [AjaxAuthorize]
    [AjaxSessionFilter]
    public class VAS_191_POGlobalSearchWidgetController : Controller
    {
        private static readonly VLogger _log = VLogger.GetVLogger(typeof(VAS_191_POGlobalSearchWidgetController));
        ...
        private JsonResult GetRecentPurchaseOrders(Ctx ctx)
        {
            ...
            try
            {
                ...
                while (dr != null && dr.Read() && count < MAX_RECENT_RESULTS)
                {
                    ...
                    results.Add(new
                    {
                        ...
                        documentNo = docNo,
                        group = Msg.GetMsg(ctx, "VAS_191_RecentAcrossPurchase") ?? "Recent across purchase",
                        groupKey = "Recent",
                        title = docNo,
                        subtitle = subtitle,
            ...
            {
                _log.Severe("VAS_191_POGlobalSearchWidgetController.GetRecentPurchaseOrders: " + ex.Message);
                return Fail(Msg.GetMsg(ctx, "VAS_191_SearchError") ?? "Unable to search purchase orders. Please try again.");
            }
            finally
            {
                if (dr != null)
        ...
        private JsonResult ExecuteSearch(Ctx ctx, string searchTerm)
        {
                ...
                while (dr != null && dr.Read())
                {
                    ...
                    if (!string.IsNullOrEmpty(docNo) && docNo.ToLowerInvariant().Contains(qLower))
                    {
                        ...
                        AddCandidate(candidates, seenKeys, new SearchResultCandidate
                        {
                            ...
                            OrderLineId = null,
                            DocumentNo = docNo,
                            Group = Msg.GetMsg(ctx, "VAS_191_PurchaseOrder") ?? "Purchase order",
                    ...
                    if (!string.IsNullOrEmpty(vendorName) && vendorName.ToLowerInvariant().Contains(qLower))
                    {
                        AddCandidate(candidates, seenKeys, new SearchResultCandidate
                        {
                            ...
                            OrderLineId = null,
                            DocumentNo = docNo,
                            Group = Msg.GetMsg(ctx, "VAS_191_Vendor") ?? "Vendor",
                            GroupKey = "Vendor",
                            Title = vendorName,
                            Subtitle = subtitle,
                            Value = formattedValue,
                    ...
                    if (matchProdName || matchProdCode || matchProdSku)
                    {
                        ...
                        AddCandidate(candidates, seenKeys, new SearchResultCandidate
                        {
                            ...
                            DocumentNo = docNo,
                            Group = Msg.GetMsg(ctx, "VAS_191_Product") ?? "Product",
                            GroupKey = "Product",
                            Title = prodTitle,
                            Subtitle = subtitle,
                            Value = formattedValue,
                    ...
                    if (!string.IsNullOrEmpty(repName) && repName.ToLowerInvariant().Contains(qLower))
                    {
                        AddCandidate(candidates, seenKeys, new SearchResultCandidate
                        {
                            ...
                            OrderLineId = null,
                            DocumentNo = docNo,
                            Group = Msg.GetMsg(ctx, "VAS_191_Representative") ?? "Representative",
                    ...
                    if (!string.IsNullOrEmpty(warehouseName) && warehouseName.ToLowerInvariant().Contains(qLower))
                    {
                        AddCandidate(candidates, seenKeys, new SearchResultCandidate
                        {
                            ...
                            OrderLineId = null,
                            DocumentNo = docNo,
                            Group = Msg.GetMsg(ctx, "VAS_191_Warehouse") ?? "Warehouse",
                            GroupKey = "Warehouse",
                            Title = warehouseName,
                            Subtitle = subtitle,
                            Value = formattedValue,
                    ...
                    if (!string.IsNullOrEmpty(reqNo) && reqNo.ToLowerInvariant().Contains(qLower))
                    {
                        AddCandidate(candidates, seenKeys, new SearchResultCandidate
                        {
                            ...
                            OrderLineId = orderLineId > 0 ? (int?)orderLineId : null,
                            DocumentNo = docNo,
                            Group = Msg.GetMsg(ctx, "VAS_191_Requisition") ?? "Requisition",
                    ...
                    if (!string.IsNullOrEmpty(poRef) && poRef.ToLowerInvariant().Contains(qLower))
                    {
                        AddCandidate(candidates, seenKeys, new SearchResultCandidate
                        {
                            ...
                            OrderLineId = null,
                            DocumentNo = docNo,
                            Group = Msg.GetMsg(ctx, "VAS_191_OrderReference") ?? "Order reference",
                    ...
                    if (!string.IsNullOrEmpty(description) && description.ToLowerInvariant().Contains(qLower))
                    {
                        ...
                        AddCandidate(candidates, seenKeys, new SearchResultCandidate
                        {
                            ...
                            OrderLineId = null,
                            DocumentNo = docNo,
                            Group = Msg.GetMsg(ctx, "VAS_191_Description") ?? "Description",
                    ...
                    if (!string.IsNullOrEmpty(locName) && locName.ToLowerInvariant().Contains(qLower))
                    {
                        AddCandidate(candidates, seenKeys, new SearchResultCandidate
                        {
                            ...
                            OrderLineId = null,
                            DocumentNo = docNo,
                            Group = Msg.GetMsg(ctx, "VAS_191_Location") ?? "Location",
                            GroupKey = "Location",
                            Title = locName,
                            Subtitle = subtitle,
                            Value = formattedValue,
            ...
            {
                _log.Severe("VAS_191_POGlobalSearchWidgetController.ExecuteSearch: " + ex.Message);
                return Fail(Msg.GetMsg(ctx, "VAS_191_SearchError") ?? "Unable to search purchase orders. Please try again.");
            }
            finally
            {
