...
{
    /// <summary>
    /// Controller for VAS_205_POCompletedMTDWidget.
    /// Provides MTD completed/closed PO summary figures and drill-down document list.
    /// </summary>
    public class VAS_205_POCompletedMTDWidgetController : Controller
    {
        private static readonly VLogger _log = VLogger.GetVLogger(typeof(VAS_205_POCompletedMTDWidgetController).FullName);
        ...
        public JsonResult GetPOCompletedMTDData()
        {
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    foreach (DataRow row in ds.Tables[0].Rows)
                    {
                        ...
                        if (rowCurId > 0 && schemaCurrencyId > 0 && rowCurId != schemaCurrencyId)
                        {
                            ...
                            {
                                _log.Warning("VAS_205: Currency conversion failed for PO " + docNo + ": " + ex.Message);
                            }
                        }
            ...
            {
                _log.Log(Level.SEVERE, "VAS_205_POCompletedMTDWidget.GetPOCompletedMTDData", ex);
        ...
        public JsonResult GetPOLines(int C_Order_ID)
        {
            ...
            {
                _log.Log(Level.SEVERE, "VAS_205_POCompletedMTDWidget.GetPOLines", ex);
