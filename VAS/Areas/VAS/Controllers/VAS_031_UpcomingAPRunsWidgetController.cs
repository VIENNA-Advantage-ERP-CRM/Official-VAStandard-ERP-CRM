    ...
    public class VAS_031_UpcomingAPRunsWidgetController : Controller
    {
        ...
        public JsonResult GetUpcomingAPRuns()
        {
            ...
            try
            {
                ...
                while (reader != null && reader.Read() && runs.Count < MaximumRows)
                {
                    DateTime? runDate = GetNullableDateTime(reader, "RunDate");
                    int paymentCount = GetInt(reader, "PaymentCount");
                    string currencyISO = GetString(reader, "CurrencyISO");
                    string currencySymbol = GetString(reader, "CurrencySymbol");
                    ...
                    runs.Add(new
                    {
                        ...
                        paymentMethodName = GetPaymentMethodName(ctx, GetString(reader, "PaymentMethodName")),
                        runDate = FormatDate(runDate),
                        dueDate = FormatDate(runDate),
                        runDateText = FormatRunDate(runDate),
                        dueDateText = FormatRunDate(runDate),
                        ...
                        stdPrecision = NormalizePrecision(GetInt(reader, "StdPrecision", 2)),
                ...
                return Json(new
                {
                    ...
                    periodText = GetMsg(ctx, "VAS_031_MessageNext7Days", "Next 7 days"),
            ...
            {
                VLogger.Get().SaveError("VAS_031_GetUpcomingAPRuns", ex);

                string message = GetMsg(ctx, "VAS_ErrorLoading", "Could not load data");
        ...
        private string BuildUpcomingAPRunsSql(Ctx ctx)
        {
            ...
            string clientIdSql = clientId.ToString(CultureInfo.InvariantCulture);
...
(
    ...
    AND InvoiceDocumentType.IsSOTrx = 'N'
            ...
            invoiceAccessSql = MRole.GetDefault(ctx).AddAccessSQL(
                invoiceAccessSql, "Invoice", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO);
...
(
    SELECT
        ...
        CASE
            ...
            THEN SIGN(COALESCE(Invoice.GrandTotal, 0)) * (
                ABS(COALESCE(Invoice.GrandTotal, 0))
                - ABS(COALESCE(InvoiceAllocation.AllocatedAmt, 0))
            )
            ELSE 0
...
(
    SELECT
        ...
        CASE
            ...
            THEN SIGN(COALESCE(InvoicePaySchedule.DueAmt, 0)) * (
                ABS(COALESCE(InvoicePaySchedule.DueAmt, 0))
                - ABS(COALESCE(ScheduleAllocation.AllocatedAmt, 0))
            )
            ELSE 0
    ...
    LEFT OUTER JOIN ScheduleAllocation ScheduleAllocation ON
        (ScheduleAllocation.C_InvoicePaySchedule_ID = InvoicePaySchedule.C_InvoicePaySchedule_ID)
    ...
    (
        ...
        AND ExistingPayment.C_InvoicePaySchedule_ID = InvoicePaySchedule.C_InvoicePaySchedule_ID
...
(
    SELECT
        ...
        CASE
            WHEN ABS(UnpaidSchedule.ScheduleOpenAmount) < ABS(InvoiceOpenBalance.InvoiceOpenAmount)
    ...
    LEFT OUTER JOIN VA009_PaymentMethod PaymentMethod ON
        (PaymentMethod.VA009_PaymentMethod_ID = Invoice.VA009_PaymentMethod_ID)
    ...
    VendorName";
            ...
            int cBPartnerId = 0)
        {
            ...
            if (!DateTime.TryParseExact(
                    ...
                    out parsedRunDate))
            {
                string invalidDateMessage = GetMsg(ctx, "VAS_031_TransactionDateInvalid", "Invalid due date.");
            ...
            if (currencyId <= 0)
            {
                string currencyMessage = GetMsg(ctx, "VAS_031_MessageCurrencyRequired", "Currency is required.");
            ...
            if (cBPartnerId <= 0)
            {
                string businessPartnerMessage = GetMsg(
                    ctx, "VAS_031_BusinessPartnerRequired", "Business partner is required.");
            ...
            try
            {
                string clientIdSql = ctx.GetAD_Client_ID().ToString(CultureInfo.InvariantCulture);
                string paymentMethodIdSql = Math.Max(0, paymentMethodId).ToString(CultureInfo.InvariantCulture);
                string currencyIdSql = currencyId.ToString(CultureInfo.InvariantCulture);
                string cBPartnerIdSql = cBPartnerId.ToString(CultureInfo.InvariantCulture);
                string runDateText = parsedRunDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);

                string runDateSql = DB.IsOracle()
                    ? "TO_DATE('" + runDateText + "', 'YYYY-MM-DD')"
                    : "DATE '" + runDateText + "'";
                 ...
                 */
                string runDateEndSql = DB.IsOracle()
                    ? runDateSql + " + 1"
...
AND Invoice.C_BPartner_ID = " + cBPartnerIdSql + @"
AND COALESCE(Invoice.VA009_PaymentMethod_ID, 0) = " + paymentMethodIdSql;
...
(
    ...
    AND " + runDateSql + @" >= Period.StartDate
    AND " + runDateSql + @" < " + periodEndExclusiveSql + @"
),
SelectedPeriod AS
(
    SELECT
...
(
    SELECT
        ...
        InvoicePaySchedule.DueDate,
        COALESCE(InvoicePaySchedule.DueAmt, 0) AS DueAmt,
        COALESCE(InvoicePaySchedule.DiscountAmt, 0) AS DiscountAmt,
        InvoicePaySchedule.DiscountDate,
        InvoicePaySchedule.DiscountDays2,
        COALESCE(InvoicePaySchedule.Discount2, 0) AS Discount2,
        0 AS WriteOffAmt,
        COALESCE(ScheduleAllocation.AllocatedAmt, 0) AS AllocatedAmt,
        CASE
            WHEN (
...
(
    COALESCE(UnpaidSchedule.DueDate, Invoice.DateAcct) >= SelectedPeriod.DateFrom
    AND COALESCE(UnpaidSchedule.DueDate, Invoice.DateAcct) < " + (DB.IsOracle()
                        ? "SelectedPeriod.DateTo + 1"
                        : "SelectedPeriod.DateTo + INTERVAL '1 DAY'") + @"
)
LEFT OUTER JOIN AD_Org Organization ON
    (Organization.AD_Org_ID = Invoice.AD_Org_ID)
LEFT OUTER JOIN C_Currency Currency ON
    (Currency.C_Currency_ID = Invoice.C_Currency_ID)
LEFT OUTER JOIN C_ConversionType ConversionType ON
    (ConversionType.C_ConversionType_ID = Invoice.C_ConversionType_ID)
LEFT OUTER JOIN VA009_PaymentMethod PaymentMethod ON
    (PaymentMethod.VA009_PaymentMethod_ID = Invoice.VA009_PaymentMethod_ID)
WHERE BusinessPartner.IsActive = 'Y'
AND BusinessPartner.IsVendor = 'Y'
 ...
 */
AND COALESCE(UnpaidSchedule.DueDate, Invoice.DateAcct) >= " + runDateSql + @"
AND COALESCE(UnpaidSchedule.DueDate, Invoice.DateAcct) < " + runDateEndSql + @"
ORDER BY
    DueDate,
    Invoice.DocumentNo,
    UnpaidSchedule.C_InvoicePaySchedule_ID,
    Invoice.C_Invoice_ID";
                ...
                {
                    DateTime? dueDate = GetNullableDateTime(reader, "DueDate");
                    DateTime? dateInvoiced = GetNullableDateTime(reader, "DateInvoiced");

                    if (!periodDateFrom.HasValue)
                    {
                        periodDateFrom = GetNullableDateTime(reader, "PeriodDateFrom");
                    }

                    if (!periodDateTo.HasValue)
                    {
                        periodDateTo = GetNullableDateTime(reader, "PeriodDateTo");
                    }

                    if (periodId <= 0)
                    {
                        periodId = GetInt(reader, "C_Period_ID");
                    }
                    ...
                    int invoiceId = GetInt(reader, "C_Invoice_ID");
                    int vendorId = GetInt(reader, "C_BPartner_ID");
                    int invoicePayScheduleId = GetInt(reader, "C_InvoicePaySchedule_ID");

                    decimal scheduleDueAmount = GetDecimal(reader, "ScheduleDueAmount");
                    decimal scheduleAllocatedAmount = GetDecimal(reader, "ScheduleAllocatedAmount");
                    decimal scheduleDiscountAmt = GetDecimal(reader, "DiscountAmt");
                    decimal scheduleDiscountAmt2 = GetDecimal(reader, "Discount2");
                    decimal writeOffAmt = GetDecimal(reader, "WriteOffAmt");
                    decimal scheduleOpenAmount = GetDecimal(reader, "ScheduleOpenAmount");
                    decimal initialOpenAmount = GetDecimal(reader, "InitialOpenAmount");
                    decimal appliedDiscountAmt = GetDecimal(reader, "InitialDiscountAmount");
                    ...
                    {
                        initialOpenAmount = scheduleOpenAmount;
                    }

                    if (Math.Abs(appliedDiscountAmt) > Math.Abs(initialOpenAmount))
                    {
                        appliedDiscountAmt = initialOpenAmount;
                    }

                    decimal schedulePayMagnitude = Math.Max(
                        Math.Abs(initialOpenAmount) - Math.Abs(appliedDiscountAmt), 0);

                    decimal schedulePayAmt = initialOpenAmount < 0
                        ? -schedulePayMagnitude
                        : schedulePayMagnitude;

                    if (invoicePayScheduleId <= 0 || initialOpenAmount == 0)
                    {
                        continue;
                    }
                    ...
                    rows.Add(new
                    {
                        ...
                        cInvoicePayScheduleId = invoicePayScheduleId,
                        invoiceDocumentNo = GetString(reader, "DocumentNo"),
                        documentNo = GetString(reader, "DocumentNo"),
                        description = GetString(reader, "Description"),
                        invoiceDescription = GetString(reader, "Description"),
                        ...
                        bPartnerLocationId = GetInt(reader, "C_BPartner_Location_ID"),
                        cBPartnerLocationId = GetInt(reader, "C_BPartner_Location_ID"),
                        vendorName = GetString(reader, "VendorName"),
                        isVendor = string.Equals(
                            GetString(reader, "IsVendor"), "Y", StringComparison.OrdinalIgnoreCase),
                        cCurrencyId = GetInt(reader, "C_Currency_ID"),
                        currencyId = GetInt(reader, "C_Currency_ID"),
                        currencyISO = currencyISO,
                        currencySymbol = currencySymbol,
                        ...
                        dateInvoiced = FormatDate(dateInvoiced),
                        dueDate = FormatDate(dueDate),
                        transactionDate = FormatDate(parsedRunDate),
                        dateTrx = FormatDate(parsedRunDate),
                        grandTotal = GetDecimal(reader, "GrandTotal"),
                        scheduleDueAmount = scheduleDueAmount,
                        scheduleAllocatedAmount = scheduleAllocatedAmount,
                        discountAmt = appliedDiscountAmt,
                        discountAmount = appliedDiscountAmt,
                        writeOffAmt = writeOffAmt,
                        writeOffAmount = writeOffAmt,
                        scheduleOpenAmount = scheduleOpenAmount,
                        openAmount = initialOpenAmount,
                        amount = schedulePayAmt,
                        payAmt = schedulePayAmt,
                        paymentMethodId = GetInt(reader, "VA009_PaymentMethod_ID"),
                        paymentMethodName = GetPaymentMethodName(ctx, GetString(reader, "PaymentMethodName")),
                        bankAccountId = 0,
                        docTypeId = 0,
                ...
                return Json(new
                {
                    ...
                    periodId = periodId,
                    dateFrom = FormatDate(periodDateFrom),
                    dateTo = FormatDate(periodDateTo),
                    rows = rows
                }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                VLogger.Get().SaveError("VAS_031_GetUpcomingAPRunDetails", ex);

                string message = GetMsg(ctx, "VAS_ErrorLoading", "Could not load invoice details.");
...
AND ConversionType.AD_Client_ID IN (0, " + clientIdSql + @")",
                ...
                List<object> documentTypes = ReadDocTypeLookupRows(ctx, adOrgId);
                ...
                List<object> paymentMethods = ReadPaymentMethodLookupRows(ctx, adOrgId);
            ...
            {
                VLogger.Get().SaveError("VAS_031_GetPaymentPopupLookups_" + currentLookup, ex);

                string message = GetMsg(ctx, "VAS_ErrorLoading", "Could not load lookup data.");
        ...
        /// <summary>
        /// Bank accounts for the payment popup, optionally narrowed to the
        /// currency and to the organization of the selected invoice.
        /// Organization 0 (*) is always included because those bank accounts
...
AND BankAccount.AD_Client_ID = " + clientId
            ...
            try
            {
                ...
                {
                    int id = GetInt(reader, "ID");
                    string bankName = GetString(reader, "BankName");
                    string accountName = GetString(reader, "BankAccountName");
        ...
        /// <summary>
        /// AP Payment document types for the popup, narrowed to the organization of
        /// the selected invoice. Organization 0 (*) is always included because those
        /// document types are shared by every organization of the client;
        /// adOrgId = 0 leaves the list unfiltered by organization.
        /// </summary>
        private List<object> ReadDocTypeLookupRows(Ctx ctx, int adOrgId = 0)
        {
            int clientId = ctx.GetAD_Client_ID();
...
AND DocumentType.AD_Client_ID IN (0, " + clientId + @")"
        ...
        private List<object> ReadTenderTypeLookupRows(Ctx ctx)
        {
            ...
            string languageSql = ToSqlString(ctx.GetAD_Language());
            string valueSql = GetTextCastSql("RefList.Value");
            string baseNameSql = GetTextCastSql("RefList.Name");
            string translatedNameSql = GetTextCastSql("RefListTrl.Name");
...
(
    ...
    AND RefListTrl.AD_Language = " + languageSql + @"
)
...
ORDER BY
    RefList.Name";
            ...
            try
            {
                ...
                while (reader != null && reader.Read())
                {
                    string value = GetString(reader, "TenderValue");

                    string displayName = FirstNotEmpty(
                        GetString(reader, "TranslatedName"),
                        GetString(reader, "BaseName"),
                        value);
        ...
        /// <summary>
        /// Payment methods for the popup, narrowed to the organization of the
        /// selected invoice. Organization 0 (*) is always included because those
        /// payment methods are shared by every organization of the client;
        /// adOrgId = 0 leaves the list unfiltered by organization.
        /// </summary>
        private List<object> ReadPaymentMethodLookupRows(Ctx ctx, int adOrgId = 0)
        {
            List<object> rows = new List<object>();
            IDataReader reader = null;
...
AND PaymentMethod.VA009_PAYMENTBASETYPE NOT IN ('B','C','P')"
        ...
        private bool IsCheckPaymentMethod(string paymentMethodName)
        {
            ...
            decimal payAmt)
        {
            ...
            if (invoiceId <= 0)
            {
                return GetValidationError(ctx, "VAS_031_SourceInvoiceRequired", "Source invoice is required.");
            }

            if (invoicePayScheduleId <= 0)
            {
                return GetValidationError(
                    ctx, "VAS_031_InvoicePayScheduleRequired", "Invoice payment schedule is required.");
            }

            if (paymentMethodId <= 0)
            {
                return GetValidationError(ctx, "VAS_031_PaymentMethodRequired", "Payment method is required.");
            }
            ...
            {
                return GetValidationError(
                    ctx, "VAS_031_TransactionDateInvalid", "Transaction date must be in yyyy-MM-dd format.");
            }
            ...
            try
            {
                ...
                if (invoice.GetC_Invoice_ID() <= 0)
                {
                    throw new InvalidOperationException(
                        GetMsg(ctx, "VAS_031_SourceInvoiceNotFound", "The source invoice was not found."));
                }
                ...
                if (!string.Equals(invoiceDocStatus, "CO", StringComparison.OrdinalIgnoreCase) &&
                    !string.Equals(invoiceDocStatus, "CL", StringComparison.OrdinalIgnoreCase))
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
                ...
                string invoiceDocBaseType = GetInvoiceDocBaseType(invoiceDocTypeId, trx);

                if (!string.Equals(invoiceDocBaseType, "API", StringComparison.OrdinalIgnoreCase))
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
                ...
                int validatedInvoicePayScheduleId = ValidateInvoicePaySchedule(
                    invoiceId, invoicePayScheduleId, trx);
                ...
                if (HasExistingPaymentForSchedule(invoiceId, validatedInvoicePayScheduleId, trx))
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
                 ...
                 */
                decimal openAmountBeforePayment = GetInvoicePayScheduleOpenAmount(
                    invoiceId, validatedInvoicePayScheduleId, trx);
                 ...
                 */
                MOrg organization = new MOrg(ctx, adOrgId, trx);

                if (organization.GetAD_Org_ID() <= 0 || !organization.IsActive())
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
                 ...
                 */
                MBankAccount bankAccount = new MBankAccount(ctx, bankAccountId, trx);

                if (bankAccount.GetC_BankAccount_ID() <= 0 || !bankAccount.IsActive())
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
                ...
                /*
                 * The popup only offers bank accounts of the invoice
                 * organization (plus organization 0, which every organization
                 * shares), so the same restriction is enforced here.
                 */
                if (bankAccount.GetAD_Org_ID() != 0 &&
                    bankAccount.GetAD_Org_ID() != adOrgId)
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
                            "VAS_031_BankAccountInvalidOrganization",
                            "Bank account does not belong to the selected organization."));
                }

                /*
                 * Validate vendor.
                 */
                MBPartner vendor = new MBPartner(ctx, vendorId, trx);
                ...
                int bPartnerLocationId = ResolveBPartnerLocationId(invoice, vendorId, trx);
                 ...
                 * Validate currency.
                 */
                MCurrency currency = new MCurrency(ctx, currencyId, trx);

                if (currency.GetC_Currency_ID() <= 0 || !currency.IsActive())
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
                 ...
                 */
                MConversionType conversionType = new MConversionType(ctx, conversionTypeId, trx);

                if (conversionType.GetC_ConversionType_ID() <= 0 || !conversionType.IsActive())
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
                 ...
                 */
                int resolvedDocTypeId = ResolveAPPaymentDocTypeId(ctx, docTypeId, adOrgId, trx);
                ...
                MDocType documentType = new MDocType(ctx, resolvedDocTypeId, trx);

                if (documentType.GetC_DocType_ID() <= 0 ||
                    !documentType.IsActive() ||
                    !string.Equals(documentType.GetDocBaseType(), "APP", StringComparison.OrdinalIgnoreCase))
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
                 ...
                 */
                string normalizedTenderType = (tenderType ?? string.Empty).Trim();

                if (!IsValidTenderType(ctx, normalizedTenderType))
                {
                    throw new InvalidOperationException(
                        GetMsg(
                            ctx,
...
FROM VA009_PaymentMethod PaymentMethod
WHERE PaymentMethod.IsActive = 'Y'
AND PaymentMethod.VA009_PaymentMethod_ID = " + paymentMethodId.ToString(CultureInfo.InvariantCulture) + @"
AND PaymentMethod.AD_Client_ID IN (0, " + ctx.GetAD_Client_ID().ToString(CultureInfo.InvariantCulture) + @")",
                        null, trx));
                ...
                bool isCheckPayment = IsCheckPaymentMethod(paymentMethodName);
                ...
                if (isCheckPayment)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            GetMsg(ctx, "VAS_031_MessageCheckNoRequired", "Check number is required."));
                    }
                ...
                decimal scheduleOpenAmountInPaymentCurrency = openAmountBeforePayment;

                if (invoice.GetC_Currency_ID() != currencyId)
                {
                    scheduleOpenAmountInPaymentCurrency = MConversionRate.Convert(
                        ctx,
                ...
                if (Math.Abs(payAmt) + Math.Abs(discountAmt) + Math.Abs(writeOffAmt) >
                ...
                payment.SetC_DocType_ID(resolvedDocTypeId);
                payment.SetC_BankAccount_ID(bankAccountId);
                payment.SetC_BPartner_ID(invoice.GetC_BPartner_ID());
                payment.SetC_BPartner_Location_ID(bPartnerLocationId);
                ...
                payment.Set_Value("C_InvoicePaySchedule_ID", validatedInvoicePayScheduleId);

                payment.SetDateTrx(DateTime.Now);
                payment.SetDateAcct(DateTime.Now);

                payment.SetTenderType(normalizedTenderType);
                payment.Set_Value("VA009_PaymentMethod_ID", paymentMethodId);

                if (isCheckPayment)
                {
                    payment.Set_Value("CheckNo", checkNo.Trim());
                    payment.Set_Value("CheckDate", parsedCheckDate);
                }

                if (!string.IsNullOrWhiteSpace(paymentDescription))
                {
                    payment.Set_Value("Description", paymentDescription.Trim());
                }
                ...
                if (writeOffAmt != 0 && payment.Get_ColumnIndex("WriteOffAmt") >= 0)
                {
                ...
                payment.SetIsOverUnderPayment(overUnderAmt != 0);
                ...
                {
                    throw new InvalidOperationException(
                        GetLastModelError(ctx, "VAS_031_CouldNotSaveAPPayment", "Could not save AP payment."));
                }

                int createdPaymentId = payment.GetC_Payment_ID();
                ...
                string completionMessage = DocumentEngine.CompleteOrReverse(
                    ctx,
                    "C_Payment",
                    PaymentTableId,
                    createdPaymentId,
                 ...
                 */
                payment = new MPayment(ctx, createdPaymentId, null);
                ...
                bool isCompleted =
                    string.Equals(completedDocStatus, "CO", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(completedDocStatus, "CL", StringComparison.OrdinalIgnoreCase);
                ...
                if (!isCompleted)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(processMessage))
                    {
                        ...
                        ).Replace("{0}", completedDocStatus);
                    }

                    throw new InvalidOperationException(processMessage);
                }
                 ...
                 */
                decimal remainingOpenAmount = GetInvoiceOpenAmount(invoiceId, null);

                bool invoiceFullyPaid = remainingOpenAmount == 0;

                string createdPaymentDocumentNo = payment.GetDocumentNo();
                ...
                return Json(new
                {
                    ...
                    invoicePayScheduleId = validatedInvoicePayScheduleId,
                    bPartnerLocationId = bPartnerLocationId,
                    paymentId = payment.GetC_Payment_ID(),
                    documentNo = createdPaymentDocumentNo,
                    paymentDocumentNo = createdPaymentDocumentNo,
                    paymentNo = createdPaymentDocumentNo,
                    docStatus = payment.GetDocStatus(),
                    docAction = payment.GetDocAction(),
                    processed = payment.IsProcessed(),
                    docTypeId = resolvedDocTypeId,
                    docTypeName = documentType.GetName(),
                    paidAmount = payAmt,
                    openAmountBeforePayment = openAmountBeforePayment,
                    remainingOpenAmount = remainingOpenAmount,
                    invoiceFullyPaid = invoiceFullyPaid,
                    removeInvoiceFromWidget = invoiceFullyPaid,
                    refreshUpcomingAPRuns = true,
                    message = msg
                });
            }
            ...
            catch (Exception ex)
            {
                ...
                VLogger.Get().SaveError("VAS_031_CreateUpcomingAPPayment", ex);

                string message = GetMsg(
                    ctx, "VAS_031_CouldNotSaveAPPayment", "Could not save or complete AP payment.");
...
FROM C_DocType DocumentType
WHERE DocumentType.C_DocType_ID = " + invoiceDocTypeId.ToString(CultureInfo.InvariantCulture);

            return Util.GetValueOfString(DB.ExecuteScalar(sql, null, trx));
        }
...
AND COALESCE(InvoicePaySchedule.C_Payment_ID, 0) = 0
AND COALESCE(InvoicePaySchedule.C_CashLine_ID, 0) = 0
AND InvoicePaySchedule.C_Invoice_ID = " + invoiceId.ToString(CultureInfo.InvariantCulture) + @"
AND InvoicePaySchedule.C_InvoicePaySchedule_ID = " +
                invoicePayScheduleId.ToString(CultureInfo.InvariantCulture);

            return Util.GetValueOfInt(DB.ExecuteScalar(sql, null, trx));
        }
            ...
            Trx trx)
        {
            ...
            string sql = @"
SELECT
    COALESCE(InvoicePaySchedule.VA009_IsPaid, 'N')
FROM C_InvoicePaySchedule InvoicePaySchedule
WHERE InvoicePaySchedule.IsActive = 'Y'
AND InvoicePaySchedule.C_Invoice_ID = " + invoiceId.ToString(CultureInfo.InvariantCulture) + @"
AND InvoicePaySchedule.C_InvoicePaySchedule_ID = " +
            ...
            string isPaid = Util.GetValueOfString(DB.ExecuteScalar(sql, null, trx));

            return string.Equals(isPaid, "Y", StringComparison.OrdinalIgnoreCase);
        }
...
FROM C_Payment ExistingPayment
WHERE ExistingPayment.IsActive = 'Y'
AND ExistingPayment.C_Invoice_ID = " + invoiceId.ToString(CultureInfo.InvariantCulture) + @"
AND ExistingPayment.C_InvoicePaySchedule_ID = " +
                invoicePayScheduleId.ToString(CultureInfo.InvariantCulture) + @"
AND ExistingPayment.DocStatus NOT IN ('VO', 'RE')";

            int paymentCount = Util.GetValueOfInt(DB.ExecuteScalar(sql, null, trx));
            ...
            Trx trx)
        {
            ...
            int invoiceLocationId = invoice.GetC_BPartner_Location_ID();
...
AND BPartnerLocation.C_BPartner_ID = " +
                ...
                int validLocationId = Util.GetValueOfInt(DB.ExecuteScalar(validationSql, null, trx));
...
LIMIT 1";
            ...
            return Util.GetValueOfInt(DB.ExecuteScalar(fallbackSql, null, trx));
        }
            ...
            Trx trx)
        {
            ...
            string invoiceIdSql = invoiceId.ToString(CultureInfo.InvariantCulture);
            string scheduleIdSql = invoicePayScheduleId.ToString(CultureInfo.InvariantCulture);

            string sql = @"
SELECT
    CASE
        WHEN ABS(ScheduleBalance.ScheduleOpenAmount) < ABS(InvoiceBalance.InvoiceOpenAmount)
...
(
    ...
    (Allocation.C_Invoice_ID = Invoice.C_Invoice_ID)
WHERE Invoice.C_Invoice_ID = " + invoiceIdSql;
            ...
            try
            {
                reader = DB.ExecuteReader(sql, null, trx);

                if (reader != null && reader.Read())
                {
                    return GetDecimal(reader, "OpenAmount");
                }
            }
            finally
            {
            ...
            decimal payAmt)
        {
            if (adOrgId <= 0)
            {
                return GetValidationError(ctx, "VAS_031_OrganizationRequired", "Organization is required.");
            }

            if (bankAccountId <= 0)
            {
                return GetValidationError(ctx, "VAS_031_MessageBankAccountRequired", "Bank account is required.");
            }

            if (vendorId <= 0)
            {
                return GetValidationError(ctx, "VAS_031_MessageVendorRequired", "Vendor is required.");
            }

            if (currencyId <= 0)
            {
                return GetValidationError(ctx, "VAS_031_MessageCurrencyRequired", "Currency is required.");
            }

            if (conversionTypeId <= 0)
            {
                return GetValidationError(ctx, "VAS_031_ConversionTypeRequired", "Currency type is required.");
            }

            if (docTypeId <= 0)
            {
                return GetValidationError(ctx, "VAS_031_DocumentTypeRequired", "Document type is required.");
            }

            if (string.IsNullOrWhiteSpace(tenderType))
            {
                return GetValidationError(ctx, "VAS_031_MessageTenderTypeRequired", "Tender type is required.");
            }

            if (string.IsNullOrWhiteSpace(transactionDate))
            {
                return GetValidationError(ctx, "VAS_031_TransactionDateRequired", "Transaction date is required.");
            }
            ...
            {
                return GetValidationError(
                    ctx, "VAS_031_PaymentAmountRequired", "Payment amount must not be zero.");
            }
        ...
        private JsonResult GetValidationError(
            ...
            string fallback)
        {
            string message = GetMsg(ctx, messageKey, fallback);
        ...
        private int ResolveAPPaymentDocTypeId(
            ...
            Trx trx)
        {
            if (ctx == null || requestedDocTypeId <= 0 || adOrgId <= 0)
            {
                return 0;
            }
...
(
    ...
    " + adOrgId.ToString(CultureInfo.InvariantCulture) + @"
            ...
            string tenderType)
        {
            ...
            string tenderTypeSql = ToSqlString(tenderType.Trim());
...
AND ReferenceInfo.IsActive = 'Y'
AND RefList.IsActive = 'Y'
AND " + GetTextCastSql("RefList.Value") + @" = " + tenderTypeSql;
        ...
        private string GetDateFilter(
            ...
            DateTime dateTo)
        {
            string dateFromText = dateFrom.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
            string dateToText = dateTo.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
...
AND " + columnName + @" < DATE '" + dateToText + @"'";
        ...
        private int GetInt(
            ...
            int fallback = 0)
        {
            object value = GetReaderValue(record, columnName);
        ...
        {
            return count == 1
                ? GetMsg(ctx, "VAS_031_MessagePayment", "invoice")
                : GetMsg(ctx, "VAS_031_MessagePayments", "invoices");
        }
            ...
            string name)
        {
            return string.IsNullOrWhiteSpace(name)
                ? GetMsg(ctx, "VAS_031_MessageNotSpecified", "Not Specified")
                : name;
        }
        ...
        private string GetLastModelError(
            ...
            string fallback)
        {
            string modelError = GetMsg(ctx, messageKey, fallback);

            try
            {
                ValueNamePair loggerError = VLogger.RetrieveError();

                if (loggerError != null && !string.IsNullOrWhiteSpace(loggerError.GetName()))
                {
        ...
        private void RollbackTransaction(Trx trx)
        {
            if (trx != null)
            {
                trx.Rollback();
