...
SELECT CurrentPeriodData.C_Year_ID,
...
(
    ...
    INNER JOIN (SELECT C_Period.C_Year_ID,
                       MIN(C_Period.StartDate) AS YearStart
                FROM C_Period
                WHERE C_Period.IsActive='Y'
                GROUP BY C_Period.C_Year_ID) YearRange ON (YearRange.C_Year_ID=PeriodData.C_Year_ID)
    WHERE ClientInfo.IsActive='Y'
    AND YearData.IsActive='Y'
    AND PeriodData.IsActive='Y'
    AND CURRENT_DATE >= PeriodData.StartDate
    AND CURRENT_DATE < " + currentDateEndExpression + @"
) CurrentPeriodData
...
SELECT C_ElementValue.Value AS AccountCode,
       C_ElementValue.Name AS AccountName,
       ROUND(COALESCE(SUM(COALESCE(GL_JournalLine.AmtAcctDr,0)),0), COALESCE(SchemaCurrency.StdPrecision,2)) AS TotalDebit,
       ROUND(COALESCE(SUM(COALESCE(GL_JournalLine.AmtAcctCr,0)),0), COALESCE(SchemaCurrency.StdPrecision,2)) AS TotalCredit,
       ABS(ROUND(COALESCE(SUM(COALESCE(GL_JournalLine.AmtAcctDr,0)),0) - COALESCE(SUM(COALESCE(GL_JournalLine.AmtAcctCr,0)),0), COALESCE(SchemaCurrency.StdPrecision,2))) AS NetMovement,
       CASE
           WHEN COALESCE(SUM(COALESCE(GL_JournalLine.AmtAcctDr,0)),0) - COALESCE(SUM(COALESCE(GL_JournalLine.AmtAcctCr,0)),0) < 0 THEN 'Y'
           ELSE 'N'
       END AS IsCredit,
...
AND GL_Journal.DateAcct >= " + periodStartExpression + @"
AND GL_Journal.DateAcct < " + periodEndExclusiveExpression + @"
...
RankedMovement AS
(
SELECT LedgerMovement.*,
       ROW_NUMBER() OVER (ORDER BY LedgerMovement.NetMovement DESC) AS RowNo
...
SELECT TopMovement.AccountCode,
...
ORDER BY TopMovement.RowNo";
                ...
                {
                    rows[i].BarPct = maxAbs > 0m
                        ? (int)Math.Round(rows[i].NetMovement / maxAbs * 100m, MidpointRounding.AwayFromZero)
                        : 0;
                }
