...
namespace VAS.Controllers
{
    ...
    public class VAS_148_DueFulfilmentsWidgetController : Controller
    {
        ...
        {
            // DocBaseType 'MMS' alone is not "outbound shipment": on this data it also matches
            // "MM Customer Return" and "MM Customer Return with Confirmation", which are inbound
            // return documents (IsReturnTrx = 'Y'). Because the list is ordered by Name, the return
            // sorted to the top and became the DEFAULT selection in the modal - a delivery was being
            // offered a Customer Return document type. IsReturnTrx is what separates them here;
            // DocSubTypeSO is NULL on every one of these rows, so it cannot be used.
            string sql = @"
