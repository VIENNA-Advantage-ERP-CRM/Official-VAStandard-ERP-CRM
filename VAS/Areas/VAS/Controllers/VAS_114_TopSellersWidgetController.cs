...
{
    /// <summary>
    /// Module Name : VAS_114_TopSellersWidget
    /// Purpose     : Supplies the "Top Sellers" spotlight card for the Product
    ///               section - highest AND lowest selling products for the current
    ///               accounting year (with last-year comparison and units), so one
    ///               card can flip between a High and a Low series. Same sales
    ///               ranking, accounting-year, currency and product-image logic as
    ///               VAS_094; grouped per product (not per attribute). Placeholder
    ///               number 000 - reassign on hand-off.
    /// Chronological development:
    ///   114         2026-07-16 Created
    /// </summary>
    public class VAS_114_TopSellersWidgetController : Controller
    {
        private static readonly VLogger Log = VLogger.GetVLogger(typeof(VAS_114_TopSellersWidgetController).FullName);

        // Both series spotlight the same number of ranks as VAS_094.
        private const int TopRecordsLimit = 10;

        /// <summary>
        /// Returns the highest- and lowest-selling products for the current
        ...
        public JsonResult GetTopSellers()
        {
            ...
            try
            {
                TopSellersResult result = GetTopSellersData(ctx);
                return Json(JsonConvert.SerializeObject(result), JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                Log.Log(Level.SEVERE, "VAS_114_TopSellersWidget.GetTopSellers", ex);
        ...
        /// <summary>
        /// Aggregates completed AR-invoice sales per product across the current and
        /// previous accounting years, then takes the top 10 by current-year value
        /// descending (high series) and ascending (low series).
        /// </summary>
        private TopSellersResult GetTopSellersData(Ctx ctx)
        {
            TopSellersResult result = new TopSellersResult
            {
                high = new List<TopSellerProduct>(),
                low = new List<TopSellerProduct>()
            };
            ...
            string productSql = @"
                ...
                LEFT OUTER JOIN AD_Image ProductImage ON (ProductImage.AD_Image_ID=Product.AD_Image_ID AND ProductImage.IsActive='Y')
            ...
            List<TopSellerProduct> all = new List<TopSellerProduct>();
            ...
            // High = best sellers (already sorted desc); low = worst sellers that
            // still had sales, ordered ascending. Both capped at the top 10.
            result.high = all.Take(TopRecordsLimit).ToList();
            result.low = all.OrderBy(row => row.current_year_value).Take(TopRecordsLimit).ToList();
            return result;
        }
        ...
        /// <summary>Resolves the organization calendar, falling back to the tenant calendar.</summary>
        ...
        /// <summary>Finds the accounting year containing the current active period.</summary>
        ...
        /// <summary>Gets the first and last active period dates for one accounting year.</summary>
        ...
        /// <summary>Gets the latest configured accounting year ending before the current year.</summary>
        private YearBounds GetPreviousYearBounds(Ctx ctx, int calendarId, DateTime currentYearStart)
        {
            string sql = @"
                ...
                INNER JOIN C_Period Period ON (Period.C_Year_ID=YearInfo.C_Year_ID AND Period.IsActive='Y')
                WHERE YearInfo.IsActive='Y'
                  AND YearInfo.C_Calendar_ID=@Calendar_ID
                  AND YearInfo.AD_Client_ID=@Year_Client_ID
                  AND YearInfo.AD_Org_ID IN (0,COALESCE(NULLIF(@Year_Org_ID,0),YearInfo.AD_Org_ID))
                  AND Period.AD_Client_ID=@Period_Client_ID
        ...
        /// <summary>Reads named start and end date columns into a year-bound value.</summary>
        ...
        /// (same chain as VAS_094): absolute/data URL as-is, then an existing file
        /// under GlobalVariable.ImagePath (original, then thumbnails), then the
        /// AD_Image BLOB as a base64 data URI. Returns null when nothing is
        /// available. The client prepends VIS.Application.contextUrl.
        /// </summary>
        ...
        /// <summary>Top Sellers response: high and low series plus currency.</summary>
        private class TopSellersResult
        {
            public string currency_symbol { get; set; }
            public string currency_iso { get; set; }
            public int std_precision { get; set; }
            public List<TopSellerProduct> high { get; set; }
            public List<TopSellerProduct> low { get; set; }
        }

        /// <summary>One ranked product's current and previous accounting-year totals.</summary>
        private class TopSellerProduct
        {
