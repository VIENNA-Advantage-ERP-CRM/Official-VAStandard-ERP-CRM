...
{
    /// <summary>
    /// Module Name : VAS_130_TopVendorsWidget (Product Master dashboard)
    /// Purpose     : Data endpoints for the 2x3 "Top Vendors" ranked-bar widget -
    ///               ranks vendors by COUNT(DISTINCT product) received through
    ///               completed/closed material receipts (M_InOut MovementType
    ///               'V+', IsSOTrx='N', non-return, DocStatus CO/CL) in a
    ///               selected fiscal year, and drills into the exact products
    ///               and quantities (base UOM) received from one vendor.
    ///               Purchase orders/order lines are intentionally NOT used -
    ///               only completed receipts, per the confirmed business
    ///               correction. The browser only ever sends a YearId; the
    ///               server re-resolves Start/EndDate from C_Period and derives
    ///               EndDateExclusive itself, so no client-supplied date is
    ///               trusted. MRole is applied to the primary fetched table on
    ///               each query (M_InOut for ranking/detail, C_Year for the
    ///               fiscal-year list); all input is parameterized; the SQL
    ///               uses only COALESCE / CASE (no NVL, DECODE, LISTAGG,
    ///               FETCH/LIMIT/OFFSET, DB date formatting or DB-specific
    ///               upsert), so it runs unchanged on Oracle and PostgreSQL.
    ///               Attribute rows are grouped into chips per product in C#,
    ///               never aggregated in SQL. The Top Vendors and Top Customers
    ///               widgets each have their own separate frontend module
    ///               (no shared file); this controller and
    ///               VAS_131_TopCustomersWidgetController stay separate because
    ///               their underlying business definitions (receipt vs.
    ///               delivery movement) differ.
    /// Widget size : 2 columns x 3 rows.
    /// Widget number 130.
    /// Chronological development:
    ///   &lt;EmpCode&gt;   2026-07-20 Created
    /// </summary>
    public class VAS_130_TopVendorsWidgetController : Controller
    {
        /// <summary>
        ...
        /// <summary>
        /// Returns vendors ranked by distinct products received in one fiscal
        /// year, most products first.
        /// </summary>
        ...
        /// <summary>
        /// Returns the products received from one vendor in one fiscal year,
        /// with received quantity (base UOM) and deduplicated attribute chips.
        /// </summary>
        /// <param name="partnerId">C_BPartner_ID of the vendor.</param>
        /// <param name="yearId">C_Year_ID selected by the user.</param>
