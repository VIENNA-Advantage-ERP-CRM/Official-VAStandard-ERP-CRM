...
; (function (VAS, $) {
    ...
    VAS.VAS_206_POsPendingDeliveryWidget = function () {

        this.frame;
        this.windowNo;

        var $self = this;
        var $root = $('<div class="vas-206-root">');
        var $card = null;
        var $kpiVal = null;
        var $kpiMeta = null;
        var $kpiItems = null;
        ...
        function showBusy(show) {
            if (!$busy) { return; }
            $busy.toggleClass('vas-206-hidden', !show);
        }
        ...
        function createWidgetHtml() {
                ...
                '<button type="button" class="vas-206-card vas-206-border-warn" aria-label="' + esc(title) + '">' +
                    ...
                    '<div class="vas-206-body-row">' +
                        ...
                        '<div class="vas-206-kpi-side">' +
                            ...
                            '<div class="vas-206-side-l">' + esc(itemsPendingLbl) + '<br/>' + esc(deliveryLbl) + '</div>' +
                        '</div>' +
                    '</div>' +
                '</button>'
            );

            $kpiVal = $card.find('.vas-206-kpi-val');
            $kpiMeta = $card.find('.vas-206-kpi-meta');
            $kpiItems = $card.find('.vas-206-side-v');
            ...
            $busy = $(
                '<div class="vas-206-busy vas-206-hidden">' +
        ...
        function loadKpiData() {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_206_POsPendingDeliveryWidget/GetPOsPendingDelivery',
                type: 'GET',
                cache: false,
        ...
        function ensureModalHost() {
            ...
            var html =
                '<div class="vas-206-mask" role="dialog" aria-modal="true">' +
                    '<div class="vas-206-modal">' +
                        '<div class="vas-206-modal-header">' +
                            '<div class="vas-206-htxt-wrap">' +
                                '<button type="button" class="vas-206-xbtn vas-206-back-btn" aria-label="' + esc(lbl('VAS_Back', 'Back')) + '" style="display:none;">' +
                                    ICON_BACK +
                                '</button>' +
                                '<div class="vas-206-htxt">' +
                                    '<h2 class="vas-206-mtitle"></h2>' +
                                    '<div class="vas-206-msub"></div>' +
                                '</div>' +
                            '</div>' +
                            '<div class="vas-206-hact">' +
                                '<button type="button" class="vas-206-xbtn vas-206-close-btn" aria-label="' + esc(lbl('VAS_Close', 'Close')) + '">' +
                                    ICON_CLOSE +
                                '</button>' +
                            '</div>' +
                        '</div>' +
                        '<div class="vas-206-modal-body"></div>' +
                        '<div class="vas-206-modal-foot"></div>' +
                    '</div>' +
                '</div>';
            ...
            $modalHost.find('.vas-206-close-btn').on('click', closeModal);
            ...
            $(window).on('resize.vas206', function () {
                if ($modalHost && $modalHost.hasClass('vas-206-open')) {
                    fitAllTables();
                }
            });

            // Delegate table events
            $modalHost.find('.vas-206-modal-body').on('click', function (e) {
        ...
        function openModal(cfg, isBack) {
            ...
            var $backBtn = $modalHost.find('.vas-206-back-btn');
            ...
            var $modal = $modalHost.find('.vas-206-modal');
            $modal.removeClass('vas-206-modal-sm vas-206-modal-md');
            if (cfg.size === 'sm') { $modal.addClass('vas-206-modal-sm'); }
            if (cfg.size === 'md') { $modal.addClass('vas-206-modal-md'); }

            $modalHost.find('.vas-206-mtitle').text(cfg.title || '');
            $modalHost.find('.vas-206-msub').text(cfg.subtitle || '');
            $modalHost.find('.vas-206-modal-body').html(cfg.body || '');
            $modalHost.find('.vas-206-modal-foot').html(cfg.foot || '<span class="vas-206-foot-note"></span><button type="button" class="vas-206-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button>');

            $modalHost.addClass('vas-206-open');
            drawAllTables();
        ...
        function closeModal() {
            if ($modalHost) {
                $modalHost.removeClass('vas-206-open');
            }
            modalStack = [];
        ...
        function pagedTable(cols, rows, opts) {
            ...
            return '<div class="vas-206-mtwrap' + (opts.fixed ? ' vas-206-fixed' : '') + '" id="' + id + '"></div>';
        }

        function cellHTML(cell, col) {
            if (cell && typeof cell === 'object') {
                if (cell.link) {
                    return '<span class="vas-206-cell"><button type="button" class="vas-206-lnk" data-po-id="' + esc(cell.id) + '" data-po-no="' + esc(cell.link) + '" title="' + esc(cell.link) + '">' + esc(cell.link) + '</button></span>';
                }
                if (cell.icon) {
                    return '<span class="vas-206-cell vas-206-center"><button type="button" class="vas-206-iconbtn" data-lines-po-id="' + esc(cell.id) + '" data-lines-po-no="' + esc(cell.icon) + '" title="' + esc(lbl('VAS_Lines', 'Lines')) + '">' + ICON_LINES + '</button></span>';
                }
                if (cell.chip) {
                    return '<span class="vas-206-cell" title="' + esc(cell.text) + '"><span class="vas-206-chip ' + esc(cell.chip) + '">' + esc(cell.text) + '</span></span>';
                }
            }
            var alignClass = (col && col.align === 'right') ? ' vas-206-right' : '';
            var textClass = (col && col.cls) ? (' ' + col.cls) : ' vas-206-c-std';
            return '<span class="vas-206-cell' + alignClass + textClass + '" title="' + esc(cell) + '">' + esc(cell) + '</span>';
        }
        ...
        function drawTable(id) {
            ...
            var h = '<div class="vas-206-mtbl">' +
                        '<div class="vas-206-mrow vas-206-mhead" style="grid-template-columns:' + tpl + '">';
            for (var ci = 0; ci < t.cols.length; ci++) {
                var c = t.cols[ci];
                var cAlign = c.align === 'right' ? ' vas-206-right' : '';
                h += '<span class="vas-206-cell' + cAlign + '" title="' + esc(c.label || '') + '">' + esc(c.label || '') + '</span>';
            }
            h += '</div>' +
            ...
            if (slice.length === 0) {
                ...
                for (var ri = 0; ri < slice.length; ri++) {
                    var r = slice[ri];
                    h += '<div class="vas-206-mrow" style="grid-template-columns:' + tpl + '">';
            ...
            h += '<div class="vas-206-mtfoot">' +
                    '<span class="vas-206-helper">' + esc(showingText) + '</span>';
        ...
        function mstatsHtml(items) {
            var h = '<div class="vas-206-mstats">';
            for (var i = 0; i < items.length; i++) {
                var it = items[i];
                h += '<div class="vas-206-mstat">' +
                        '<div class="vas-206-mstat-l">' + esc(it.l) + '</div>' +
                        '<div class="vas-206-mstat-v" title="' + esc(it.v) + '">' + esc(it.v) + '</div>' +
                     '</div>';
            }
            h += '</div>';
            return h;
        ...
        function openPendingDeliveryModal() {
            ...
            var rows = all.map(function (p) {
                ...
                var delivChip = p.DeliveryStatus === 'Partial' ? 'vas-206-chip-warn' : 'vas-206-chip-neutral';
        ...
        function openLinesModal(poId, poNo) {
            ...
            // Load lines via AJAX
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_206_POsPendingDeliveryWidget/GetPOLines?C_Order_ID=' + poId,
                type: 'GET',
                cache: false,
                success: function (res) {
                    var data = parseResponse(res);
                    ...
                    var lineCols = [
                        ...
                        { label: lbl('VAS_Product', 'Product'), w: 1.5, cls: 'vas-206-c-prim' },
                        ...
                        { label: lbl('VAS_PendingItems', 'Pending'), w: 0.7, align: 'right', cls: 'vas-206-c-prim' },
                        { label: lbl('VAS_Rate', 'Rate'), w: 0.7, align: 'right' },
                        { label: lbl('VAS_Amount', 'Amount'), w: 0.9, align: 'right', cls: 'vas-206-c-emph' },
                    ...
                    var topPolink =
                        '<div class="vas-206-polink">' +
                            esc(lbl('VAS_PurchaseOrder', 'Purchase order')) + ' ' +
                            '<button type="button" class="vas-206-lnk" data-po-id="' + poId + '" data-po-no="' + esc(poNo) + '">' + esc(poNo) + '</button>' +
                    ...
                    var bodyHtml =
                        topPolink +
                        statStrip +
                        '<div class="vas-206-msec">' + esc(lbl('VAS_PurchaseOrderLines', 'Purchase order lines')) + '</div>' +
                    ...
                    openModal({
                        ...
                        body: bodyHtml,
                        foot: '<span class="vas-206-foot-note">' + esc(poNo) + ' · ' + esc(vendor) + '</span>' +
                              '<span><button type="button" class="vas-206-btn vas-206-btn-back">' + esc(lbl('VAS_Back', 'Back')) + '</button> ' +
                              '<button type="button" class="vas-206-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button></span>',
                        after: function () {
                            $modalHost.find('.vas-206-btn-back').on('click', popModal);
                        }
                    });
                }
            });
        ...
        function openRecordModal(poId, poNo) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_206_POsPendingDeliveryWidget/GetPODetail?C_Order_ID=' + poId,
                type: 'GET',
                cache: false,
                success: function (res) {
                    var data = parseResponse(res);
                    ...
                    $.ajax({
                        ...
                        success: function (lineRes) {
                            ...
                            openModal({
                                ...
                                body: bodyHtml,
                                foot: '<span class="vas-206-foot-note">' + esc(footNote) + '</span>' +
                                      '<span><button type="button" class="vas-206-btn vas-206-btn-primary vas-206-btn-open-record">' + esc(lbl('VAS_OpenRecord', 'Open Record')) + '</button> ' +
                                      '<button type="button" class="vas-206-btn vas-206-btn-back">' + esc(lbl('VAS_Back', 'Back')) + '</button> ' +
                                      '<button type="button" class="vas-206-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button></span>',
                                after: function () {
                                    $modalHost.find('.vas-206-btn-back').on('click', popModal);
                                    $modalHost.find('.vas-206-btn-open-record').on('click', function () {
    ...
    VAS.VAS_206_POsPendingDeliveryWidget.prototype.widgetFirevalueChanged = function (value) {
        if (this.listener) { this.listener.widgetFirevalueChanged(value); }
    };

    VAS.VAS_206_POsPendingDeliveryWidget.prototype.addChangeListener = function (listener) {
        this.listener = listener;
    };

    VAS.VAS_206_POsPendingDeliveryWidget.prototype.init = function (windowNo, frame) {
        this.frame = frame;
    ...
    VAS.VAS_206_POsPendingDeliveryWidget.prototype.widgetSizeChange = function (height, width) { };

    VAS.VAS_206_POsPendingDeliveryWidget.prototype.refreshWidget = function () {
        this.refreshWidget();
    };

    VAS.VAS_206_POsPendingDeliveryWidget.prototype.dispose = function () {
