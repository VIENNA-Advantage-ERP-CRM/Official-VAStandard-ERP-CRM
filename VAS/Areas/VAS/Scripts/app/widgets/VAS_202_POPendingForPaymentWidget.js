/**
 * VAS_202_POPendingForPaymentWidget
...
; (function (VAS, $) {
    ...
    VAS.VAS_202_POPendingForPaymentWidget = function () {
        ...
        var $self = this;
        var $root = $('<div class="vas-202-root"></div>');
        var $card = null;
        var $payHead = null;
        var $payBody = null;
        var $payPill = null;
        ...
        function showBusy(show) {
            if (!$busy) { return; }
            $busy.toggleClass('vas-202-hidden', !show);
        }
        ...
        function createWidgetHtml() {
            ...
            $card = $('<div class="vas-202-card"></div>');

            var $head = $(
                '<div class="vas-202-head">' +
                    '<div class="vas-202-head-txt">' +
                        '<p class="vas-202-title">' + esc(title) + '</p>' +
                        '<p class="vas-202-sub">' + esc(subtitle) + '</p>' +
                    '</div>' +
                    '<span class="vas-202-hpill vas-202-hpill-danger">—</span>' +
                '</div>'
            );
            $payPill = $head.find('.vas-202-hpill');
            $card.append($head);

            var $tbl = $('<div class="vas-202-tbl"></div>');
            $payHead = $('<div class="vas-202-thead" style="grid-template-columns:' + GRID_COLUMNS + ';"></div>');
            $payBody = $('<div class="vas-202-tbody"></div>');
            ...
            var $foot = $(
                '<div class="vas-202-wfoot">' +
                    '<span class="vas-202-helper"></span>' +
                    '<div class="vas-202-pager">' +
                        '<button type="button" class="vas-202-pbtn vas-202-prev-btn" aria-label="' + esc(lbl('VAS_Previous', 'Previous')) + '" disabled>' +
                            ICON_PREV +
                        '</button>' +
                        '<span class="vas-202-ptxt">1 of 1</span>' +
                        '<button type="button" class="vas-202-pbtn vas-202-next-btn" aria-label="' + esc(lbl('VAS_Next', 'Next')) + '" disabled>' +
                            ICON_NEXT +
                        '</button>' +
                    '</div>' +
                '</div>'
            ...
            $payHelper = $foot.find('.vas-202-helper');
            $payPage = $foot.find('.vas-202-ptxt');
            $prevBtn = $foot.find('.vas-202-prev-btn');
            $nextBtn = $foot.find('.vas-202-next-btn');
            ...
            $payBody.on('click', '.vas-202-trow', function (e) {
            ...
            $busy = $(
                '<div class="vas-202-busy vas-202-hidden">' +
        ...
        function renderTableHeaders() {
            ...
            for (var i = 0; i < colHeaders.length; i++) {
                ...
                var rightClass = col.align === 'right' ? ' vas-202-right' : '';
                h += '<span class="vas-202-cell' + rightClass + '" title="' + esc(col.label) + '">' + esc(col.label) + '</span>';
            }
            $payHead.html(h);
        }
        ...
        function loadQueueData() {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_202_POPendingForPaymentWidget/GetPOPendingForPayment',
                type: 'GET',
                cache: false,
        ...
        function renderWidgetPage() {
            ...
            if (total === 0) {
                $payBody.html('<div class="vas-202-empty-state">' + esc(lbl('VAS_NoPOsPendingPayment', 'No POs pending for payment found')) + '</div>');
            ...
            for (var i = 0; i < pageRecords.length; i++) {
                ...
                if (p.IsOverdue) {
                    ...
                    dueCellHtml = '<span class="vas-202-chip vas-202-chip-risk" title="' + esc(overdueLabel) + '">' + esc(overdueLabel) + '</span>';
                } else {
                    var dueText = p.PaymentDueShort || p.PaymentDueDisplay || p.PaymentDue || '—';
                    dueCellHtml = '<span class="vas-202-c-std" title="' + esc(dueText) + '">' + esc(dueText) + '</span>';
                }
                ...
                rowsHtml +=
                    '<div class="vas-202-trow" data-po-id="' + p.PurchaseOrderID + '" data-po-no="' + esc(poNo) + '" style="grid-template-columns:' + GRID_COLUMNS + ';">' +
                        '<span class="vas-202-cell vas-202-c-link" title="' + esc(poNo) + '">' +
                            '<button type="button" class="vas-202-lnk" data-po-id="' + p.PurchaseOrderID + '" data-po-no="' + esc(poNo) + '" title="' + esc(poNo) + '">' +
                                esc(poNo) +
                            '</button>' +
                        '</span>' +
                        '<span class="vas-202-cell vas-202-c-std" title="' + esc(p.OrderDateDisplay || poDateDisplay) + '">' + esc(poDateDisplay) + '</span>' +
                        '<span class="vas-202-cell vas-202-c-std" title="' + esc(vendor) + '">' + esc(vendor) + '</span>' +
                        '<span class="vas-202-cell vas-202-c-dark" title="' + esc(wh) + '">' + esc(wh) + '</span>' +
                        '<span class="vas-202-cell vas-202-c-std" title="' + esc(p.ReceivedOnDisplay || recdOn) + '">' + esc(recdOn) + '</span>' +
                        '<span class="vas-202-cell" title="' + esc(p.PaymentDueDisplay || '') + '">' + dueCellHtml + '</span>' +
                        '<div class="vas-202-cell vas-202-right">' +
                            '<span class="vas-202-c-emph" title="' + esc(balFormatted) + '">' + esc(balFormatted) + '</span>' +
                            '<div class="vas-202-sub-txt" title="' + esc(paidText) + '">' + esc(paidText) + '</div>' +
                        '</div>' +
                    '</div>';
            }
        ...
        function renderErrorState(msg) {
            $payBody.html(
                '<div class="vas-202-error-state">' +
                    '<span>' + esc(msg) + '</span>' +
                    '<button type="button" class="vas-202-retry-btn">' + esc(lbl('VAS_Retry', 'Retry')) + '</button>' +
                '</div>'
            );
            $payBody.find('.vas-202-retry-btn').on('click', function () {
                loadQueueData();
            });
        }
        ...
        function ensureModalHost() {
            ...
            var html =
                '<div class="vas-202-mask" role="dialog" aria-modal="true">' +
                    '<div class="vas-202-modal">' +
                        '<div class="vas-202-modal-header">' +
                            '<div class="vas-202-htxt-wrap">' +
                                '<button type="button" class="vas-202-xbtn vas-202-back-btn" aria-label="' + esc(lbl('VAS_Back', 'Back')) + '" style="display:none;">' +
                                    ICON_BACK +
                                '</button>' +
                                '<div class="vas-202-htxt">' +
                                    '<h2 class="vas-202-mtitle"></h2>' +
                                    '<div class="vas-202-msub"></div>' +
                                '</div>' +
                            '</div>' +
                            '<div class="vas-202-hact">' +
                                '<button type="button" class="vas-202-xbtn vas-202-close-btn" aria-label="' + esc(lbl('VAS_Close', 'Close')) + '">' +
                                    ICON_CLOSE +
                                '</button>' +
                            '</div>' +
                        '</div>' +
            ...
            $modalHost.find('.vas-202-close-btn').on('click', closeModal);
            $modalHost.find('.vas-202-back-btn').on('click', popModal);
            ...
            $(document).on('keydown.vas202', function (e) {
                if (e.key === 'Escape') { closeModal(); }
            });

            $(window).on('resize.vas202', function () {
                if ($modalHost && $modalHost.hasClass('vas-202-open')) {
                    fitAllTables();
                }
            });

            // Delegate table pagination & links in modal
            $modalHost.find('.vas-202-modal-body').on('click', function (e) {
        ...
        function openModal(cfg, isBack) {
            ...
            var $backBtn = $modalHost.find('.vas-202-back-btn');
            ...
            var $modal = $modalHost.find('.vas-202-modal');
            $modal.removeClass('vas-202-modal-sm vas-202-modal-md');
            if (cfg.size === 'sm') { $modal.addClass('vas-202-modal-sm'); }
            if (cfg.size === 'md') { $modal.addClass('vas-202-modal-md'); }

            $modalHost.find('.vas-202-mtitle').text(cfg.title || '');
            $modalHost.find('.vas-202-msub').text(cfg.subtitle || '');
            $modalHost.find('.vas-202-modal-body').html(cfg.body || '');
            $modalHost.find('.vas-202-modal-foot').html(cfg.foot || '<span class="vas-202-foot-note"></span><button type="button" class="vas-202-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button>');

            $modalHost.addClass('vas-202-open');
            drawAllTables();
        ...
        function closeModal() {
            if ($modalHost) {
                $modalHost.removeClass('vas-202-open');
            }
            modalStack = [];
        ...
        function pagedTable(cols, rows, opts) {
            ...
            return '<div class="vas-202-mtwrap' + (opts.fixed ? ' vas-202-fixed' : '') + '" id="' + id + '"></div>';
        }

        function cellHTML(cell, col) {
            if (cell && typeof cell === 'object') {
                if (cell.link) {
                    return '<span class="vas-202-cell"><button type="button" class="vas-202-lnk" data-po-id="' + esc(cell.id) + '" data-po-no="' + esc(cell.link) + '" title="' + esc(cell.link) + '">' + esc(cell.link) + '</button></span>';
                }
                if (cell.icon) {
                    return '<span class="vas-202-cell vas-202-center"><button type="button" class="vas-202-iconbtn" data-lines-po-id="' + esc(cell.id) + '" data-lines-po-no="' + esc(cell.icon) + '" title="' + esc(lbl('VAS_Lines', 'Lines')) + '">' + ICON_LINES + '</button></span>';
                }
                if (cell.chip) {
                    return '<span class="vas-202-cell" title="' + esc(cell.text) + '"><span class="vas-202-chip ' + esc(cell.chip) + '">' + esc(cell.text) + '</span></span>';
                }
            }
            var alignClass = (col && col.align === 'right') ? ' vas-202-right' : '';
            var textClass = (col && col.cls) ? (' ' + col.cls) : ' vas-202-c-std';
            return '<span class="vas-202-cell' + alignClass + textClass + '" title="' + esc(cell) + '">' + esc(cell) + '</span>';
        }
        ...
        function drawTable(id) {
            ...
            var h = '<div class="vas-202-mtbl">' +
                        '<div class="vas-202-mrow vas-202-mhead" style="grid-template-columns:' + tpl + '">';
            for (var ci = 0; ci < t.cols.length; ci++) {
                var c = t.cols[ci];
                var cAlign = c.align === 'right' ? ' vas-202-right' : '';
                h += '<span class="vas-202-cell' + cAlign + '" title="' + esc(c.label || '') + '">' + esc(c.label || '') + '</span>';
            }
            h += '</div>' +
                 '<div class="vas-202-mbody">';

            if (slice.length === 0) {
                h += '<div class="vas-202-empty-state">' + esc(lbl('VAS_NoPOsPendingPayment', 'No records found')) + '</div>';
            } else {
                for (var ri = 0; ri < slice.length; ri++) {
                    var r = slice[ri];
                    h += '<div class="vas-202-mrow" style="grid-template-columns:' + tpl + '">';
            ...
            h += '<div class="vas-202-mtfoot">' +
                    '<span class="vas-202-helper">' + esc(showingText) + '</span>';

            if (pages > 1) {
                h += '<span class="vas-202-pager">' +
                        '<button type="button" class="vas-202-pbtn" data-mt="' + id + '" data-dir="-1"' + (t.page === 0 ? ' disabled' : '') + ' aria-label="' + esc(lbl('VAS_Previous', 'Previous')) + '">' + ICON_PREV + '</button>' +
                        '<span class="vas-202-ptxt">' + (t.page + 1) + ' ' + lbl('VAS_Of', 'of') + ' ' + pages + '</span>' +
                        '<button type="button" class="vas-202-pbtn" data-mt="' + id + '" data-dir="1"' + (t.page >= pages - 1 ? ' disabled' : '') + ' aria-label="' + esc(lbl('VAS_Next', 'Next')) + '">' + ICON_NEXT + '</button>' +
                     '</span>';
            } else {
                h += '<span></span>';
            }
        ...
        function fitTable(id) {
            ...
            var head = el.querySelector('.vas-202-mhead');
            var foot = el.querySelector('.vas-202-mtfoot');
            var row = el.querySelector('.vas-202-mbody .vas-202-mrow');
            if (!head || !row) { return; }
        ...
        function mstatsHtml(items) {
            var h = '<div class="vas-202-mstats">';
            for (var i = 0; i < items.length; i++) {
                var it = items[i];
                h += '<div class="vas-202-mstat">' +
                        '<div class="vas-202-mstat-l">' + esc(it.l) + '</div>' +
                        '<div class="vas-202-mstat-v" title="' + esc(it.v) + '">' + esc(it.v) + '</div>' +
                     '</div>';
            }
            h += '</div>';
            return h;
        ...
        function openRecordModal(poId, poNo) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_202_POPendingForPaymentWidget/GetPODetail?C_Order_ID=' + poId,
                type: 'GET',
                cache: false,
                success: function (res) {
                    var data = parseResponse(res);
                    ...
                    // Fetch PO line items
                    $.ajax({
                        url: VIS.Application.contextUrl + 'VAS_202_POPendingForPaymentWidget/GetPOLines?C_Order_ID=' + poId,
                        type: 'GET',
                        cache: false,
                        success: function (lineRes) {
                            ...
                            var lineCols = [
                                ...
                                { label: lbl('VAS_Product', 'Product'), w: 1.5, cls: 'vas-202-c-prim' },
                                ...
                                { label: lbl('VAS_Pending', 'Pending'), w: 0.7, align: 'right', cls: 'vas-202-c-prim' },
                                { label: lbl('VAS_Rate', 'Rate'), w: 0.7, align: 'right' },
                                { label: lbl('VAS_Amount', 'Amount'), w: 0.9, align: 'right', cls: 'vas-202-c-emph' },
                            ...
                            var bodyHtml =
                                headerStatsHtml +
                                '<div class="vas-202-msec">' + esc(lbl('VAS_PurchaseOrderLines', 'Purchase order lines')) + '</div>' +
                            ...
                            openModal({
                                ...
                                body: bodyHtml,
                                foot: '<span class="vas-202-foot-note">' + esc(footNote) + '</span>' +
                                      '<span><button type="button" class="vas-202-btn vas-202-btn-primary vas-202-btn-open-record">' + esc(lbl('VAS_OpenRecord', 'Open Record')) + '</button> ' +
                                      '<button type="button" class="vas-202-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button></span>',
                                after: function () {
                                    $modalHost.find('.vas-202-btn-open-record').on('click', function () {
        ...
        this.disposeComponent = function () {
            $(document).off('keydown.vas202');
            $(window).off('resize.vas202');
            if ($modalHost) {
                $modalHost.remove();
                $modalHost = null;
            }
    ...
    VAS.VAS_202_POPendingForPaymentWidget.prototype.widgetFirevalueChanged = function (value) {
        if (this.listener) { this.listener.widgetFirevalueChanged(value); }
    };

    VAS.VAS_202_POPendingForPaymentWidget.prototype.addChangeListener = function (listener) {
        this.listener = listener;
    };

    VAS.VAS_202_POPendingForPaymentWidget.prototype.init = function (windowNo, frame) {
        this.frame = frame;
    ...
    VAS.VAS_202_POPendingForPaymentWidget.prototype.widgetSizeChange = function (height, width) { };

    VAS.VAS_202_POPendingForPaymentWidget.prototype.refreshWidget = function () {
        this.refreshWidget();
    };

    VAS.VAS_202_POPendingForPaymentWidget.prototype.dispose = function () {
