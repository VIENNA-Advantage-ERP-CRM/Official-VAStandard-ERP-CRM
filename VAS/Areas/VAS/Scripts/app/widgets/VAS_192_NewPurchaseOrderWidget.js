/**
 * VAS_192_NewPurchaseOrderWidget
...
; (function (VAS, $) {
    ...
    VAS.VAS_192_NewPurchaseOrderWidget = function () {

        this.frame;
        this.windowNo;

        var $self = this;
        var $root = $('<div class="vas-192-npo-root">');
        var $card;
        var poWindowId = 0;
        var isBusy = false;
        ...
        function toast(msg) {
            var $t = $('#vas_192_toast');
            if (!$t.length) {
                $t = $('<div id="vas_192_toast" class="vas-192-toast"></div>');
                $('body').append($t);
            }
        ...
        function loadPOWindowId() {
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_192_NewPurchaseOrderWidget/GetPurchaseOrderWindowId',
                type: 'GET',
                cache: false,
        ...
        function loadLookups() {
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_192_NewPurchaseOrderWidget/GetFormLookups',
                type: 'GET',
                cache: false,
                success: function (response) {
                    var res = parseResponse(response);
        ...
        function createWidget() {
            ...
            $card = $(
                '<button type="button" class="vas-192-npo-card" aria-label="' + escapeHtml(title) + '">' +
                '<span class="vas-192-npo-well" aria-hidden="true">' +
                '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">' +
                '<path d="M12 5v14M5 12h14"/>' +
                '</svg>' +
                '</span>' +
                '<span class="vas-192-npo-title">' + escapeHtml(title) + '</span>' +
                '</button>'
            );
        ...
        function ensureModalHost() {
            ...
            var modalHtml =
                '<div class="vas-192-mask" role="dialog" aria-modal="true">' +
                '  <div class="vas-192-modal">' +
                '    <div class="vas-192-modal-header">' +
                '      <div class="vas-192-htxt-wrap">' +
                '        <button type="button" class="vas-192-xbtn vas-192-mback" aria-label="' + escapeHtml(lbl('VAS_Back', 'Back')) + '" style="display:none;">' +
                '          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>' +
                '        </button>' +
                '        <div class="vas-192-htxt"><h2 class="vas-192-mtitle"></h2><div class="vas-192-msub"></div></div>' +
                '      </div>' +
                '      <div class="vas-192-hact">' +
                '        <button type="button" class="vas-192-xbtn vas-192-mclose" aria-label="' + escapeHtml(lbl('VAS_Close', 'Close')) + '">' +
                '          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg>' +
                '        </button>' +
                '      </div>' +
                '    </div>' +
                '    <div class="vas-192-modal-body"></div>' +
                '    <div class="vas-192-modal-foot"></div>' +
                '  </div>' +
                '</div>';

            $mask = $(modalHtml);
            $modal = $mask.find('.vas-192-modal');
            $mTitle = $mask.find('.vas-192-mtitle');
            $mSub = $mask.find('.vas-192-msub');
            $mBack = $mask.find('.vas-192-mback');
            $mClose = $mask.find('.vas-192-mclose');
            $mBody = $mask.find('.vas-192-modal-body');
            $mFoot = $mask.find('.vas-192-modal-foot');
            ...
            $(document).on('keydown.vas192', function (e) {
                if (e.key === 'Escape' && $mask.hasClass('open')) {
                    closeModal();
                }
            });

            $(window).on('resize.vas192', function () {
        ...
        function openModal(cfg, isBack) {
            ...
            $mBody.attr('class', 'vas-192-modal-body' + (cfg.bodyClass ? ' ' + cfg.bodyClass : ''));
            $modal.attr('class', 'vas-192-modal' + (cfg.size ? ' ' + cfg.size : ''));

            $mTitle.text(cfg.title || '');
            $mSub.text(cfg.subtitle || '');
            $mBody.html(cfg.body || '');
            $mFoot.html(cfg.foot || '<span class="vas-192-foot-note"></span><button type="button" class="vas-192-btn" data-close="1">' + escapeHtml(lbl('VAS_Close', 'Close')) + '</button>');
        ...
        function pagedTable(cols, rows, opts) {
            ...
            return '<div class="vas-192-mtwrap' + (opts.fixed ? ' fixed' : '') + '" id="' + id + '"></div>';
        }

        function cellHTML(cell, c) {
            if (cell && typeof cell === 'object') {
                if (cell.link) {
                    return '<span class="vas-192-cell"><button type="button" class="vas-192-lnk" data-po="' + escapeHtml(cell.link) + '" title="' + escapeHtml(cell.link) + '">' + escapeHtml(cell.link) + '</button></span>';
                }
                if (cell.chip) {
                    return '<span class="vas-192-cell" title="' + escapeHtml(cell.text) + '"><span class="vas-192-chip ' + escapeHtml(cell.chip) + '">' + escapeHtml(cell.text) + '</span></span>';
                }
            }
            var alignCls = (c && c.align === 'right') ? ' right' : '';
            var textCls = (c && c.cls) ? ' ' + c.cls : ' c-std';
            return '<span class="vas-192-cell' + alignCls + textCls + '" title="' + escapeHtml(cell) + '">' + escapeHtml(cell) + '</span>';
        }
        ...
        function mtDraw(id) {
            ...
            var h = '<div class="vas-192-mtbl"><div class="vas-192-mrow vas-192-mhead" style="grid-template-columns:' + tpl + '">';
            h += t.cols.map(function (c) {
                return '<span class="vas-192-cell' + (c.align === 'right' ? ' right' : '') + '" title="' + escapeHtml(c.label || '') + '">' + escapeHtml(c.label || '') + '</span>';
            }).join('');
            h += '</div><div class="vas-192-mbody">';

            h += slice.map(function (r, i) {
                if (t.render) {
                    return '<div class="vas-192-mrow' + (t.rowClass ? ' ' + t.rowClass : '') + (r.sel ? ' sel' : '') + '" style="grid-template-columns:' + tpl + '" data-i="' + (s + i) + '">' + t.render(r, s + i) + '</div>';
                }
                return '<div class="vas-192-mrow" style="grid-template-columns:' + tpl + '">' +
            ...
            h += '<div class="vas-192-mtfoot">';
            h += '<span class="vas-192-helper">' + escapeHtml(lbl('VAS_Showing', 'Showing') + ' ' + showStart + '–' + showEnd + ' ' + lbl('VAS_Of', 'of') + ' ' + t.rows.length + (t.label ? ' · ' + t.label : '')) + '</span>';

            if (pages > 1) {
                h += '<span class="vas-192-pager">' +
                    '<button type="button" class="vas-192-pbtn" data-mt="' + id + '" data-dir="-1"' + (t.page === 0 ? ' disabled' : '') + ' aria-label="' + escapeHtml(lbl('Previous', 'Previous')) + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><polyline points="15 18 9 12 15 6"/></svg></button>' +
                    '<span class="vas-192-ptxt">' + (t.page + 1) + ' ' + escapeHtml(lbl('VAS_Of', 'of')) + ' ' + pages + '</span>' +
                    '<button type="button" class="vas-192-pbtn" data-mt="' + id + '" data-dir="1"' + (t.page >= pages - 1 ? ' disabled' : '') + ' aria-label="' + escapeHtml(lbl('Next', 'Next')) + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><polyline points="9 18 15 12 9 6"/></svg></button>' +
                    '</span>';
            } else {
                h += '<span></span>';
            }
        ...
        function mtFit(id) {
            ...
            var head = el.querySelector('.vas-192-mhead');
            var foot = el.querySelector('.vas-192-mtfoot');
            var row = el.querySelector('.vas-192-mbody .vas-192-mrow');
            if (!head || !row) { return; }
        ...
        $(document).on('click', '.vas-192-pbtn[data-mt]', function (e) {
            e.stopPropagation();
        ...
        function openRequisitionsModal() {
            openModal({
                ...
                body: '<div class="vas-192-loading">' + escapeHtml(lbl('Loading', 'Loading...')) + '</div>',
                foot: '<span class="vas-192-foot-note">' + escapeHtml(lbl('VAS_OneReqAtTime', 'One requisition at a time · its lines can go into one PO')) + '</span>' +
                    '<span><button type="button" class="vas-192-btn vas-192-direct-po">' + escapeHtml(lbl('VAS_DirectNewPO', 'Direct New PO (Blank)')) + '</button> ' +
                    '<button type="button" class="vas-192-btn" data-close="1">' + escapeHtml(lbl('VAS_Close', 'Close')) + '</button></span>',
                after: function () {
                    $mFoot.find('.vas-192-direct-po').on('click', function () {
        ...
        function loadRequisitionsList() {
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_192_NewPurchaseOrderWidget/GetOpenRequisitions',
                type: 'GET',
                cache: false,
                success: function (response) {
                    var res = parseResponse(response);
                ...
                error: function () {
                    $mBody.html('<div class="vas-192-empty-msg">' + escapeHtml(lbl('VAS_ErrorLoadingData', 'Unable to load requisitions.')) + '</div>');
                }
            });
        }
        ...
        function renderRequisitionsStep(reqs, summary) {
            ...
            var statsHtml =
                '<div class="vas-192-mstats">' +
                '  <div class="vas-192-mstat"><div class="l">' + escapeHtml(lbl('VAS_OpenRequisitionsStat', 'Open requisitions')) + '</div><div class="v">' + fmtNum(summary.openRequisitions) + '</div></div>' +
                '  <div class="vas-192-mstat"><div class="l">' + escapeHtml(lbl('VAS_ReadyToPO', 'Ready to PO')) + '</div><div class="v">' + fmtNum(summary.readyToPO) + '</div></div>' +
                '  <div class="vas-192-mstat"><div class="l">' + escapeHtml(lbl('VAS_PartlyOrdered', 'Partly ordered')) + '</div><div class="v">' + fmtNum(summary.partlyOrdered) + '</div></div>' +
                '  <div class="vas-192-mstat"><div class="l">' + escapeHtml(lbl('VAS_PendingQty', 'Pending qty')) + '</div><div class="v">' + fmtNum(summary.pendingQty) + '</div></div>' +
                '</div>';

            var bodyHtml = statsHtml +
                '<div class="vas-192-msec">' + escapeHtml(lbl('VAS_Requisitions', 'Requisitions')) + '</div>' +
            ...
            $mBody.off('click.reqrow').on('click.reqrow', '.vas-192-mrow[data-i]', function (e) {
        ...
        function stepRequisitionLines(req) {
            ...
            openModal({
                ...
                body: '<div class="vas-192-loading">' + escapeHtml(lbl('Loading', 'Loading...')) + '</div>',
                foot: '<span class="vas-192-foot-note" id="vas192_selNote">' + escapeHtml(lbl('VAS_NoLinesSelected', 'No lines selected')) + '</span>' +
                    '<span><button type="button" class="vas-192-btn vas-192-btn-back">' + escapeHtml(lbl('VAS_Back', 'Back')) + '</button> ' +
                    '<button type="button" class="vas-192-btn vas-192-btn-primary vas-192-btn-continue" disabled>' + escapeHtml(lbl('VAS_Continue', 'Continue')) + '</button></span>',
                after: function () {
                    $mFoot.find('.vas-192-btn-back').on('click', function () { backModal(); });
                    $mFoot.find('.vas-192-btn-continue').on('click', function () { startNewPOForm(); });
        ...
        function loadRequisitionLinesData(reqId) {
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_192_NewPurchaseOrderWidget/GetRequisitionLines',
                type: 'GET',
                ...
                error: function () {
                    $mBody.html('<div class="vas-192-empty-msg">' + escapeHtml(lbl('VAS_ErrorLoadingLines', 'Unable to load requisition lines.')) + '</div>');
                }
            });
        }
        ...
        function renderRequisitionLinesStep(req, lines) {
            ...
            var bodyHtml = summaryHtml +
                '<div class="vas-192-msec">' + escapeHtml(lbl('VAS_ReqLinesSelectHeader', 'Requisition lines · select one or many')) + '</div>' +
                pagedTable(lineCols, lines, {
                    ...
                    render: function (l, i) {
                        ...
                        return '<span class="vas-192-cell"><input type="checkbox" class="vas-192-chk line" data-i="' + i + '"' + (isSel ? ' checked' : '') + ' aria-label="Select line"></span>' +
                            '<span class="vas-192-cell c-dark" title="' + escapeHtml(l.productName) + '">' + escapeHtml(l.productName) + '</span>' +
                            '<span class="vas-192-cell c-std" title="' + escapeHtml(l.attribute || 'Standard') + '">' + escapeHtml(l.attribute || 'Standard') + '</span>' +
                            '<span class="vas-192-cell c-std">' + escapeHtml(l.uom) + '</span>' +
                            '<span class="vas-192-cell right c-std">' + fmtNum(l.reqQty) + '</span>' +
                            '<span class="vas-192-cell right ' + (l.alreadyOrderedQty > 0 ? 'c-dark' : 'c-std') + '">' + (l.alreadyOrderedQty > 0 ? fmtNum(l.alreadyOrderedQty) : '—') + '</span>' +
                            '<span class="vas-192-cell right c-prim">' + fmtNum(l.pendingQty) + '</span>' +
                            '<span class="vas-192-cell"><input class="vas-192-rowin qty" type="number" min="1" max="' + l.pendingQty + '" value="' + (l.orderQty || l.pendingQty) + '" data-i="' + i + '" aria-label="Qty to order"></span>' +
                            '<span class="vas-192-cell"><select class="vas-192-rowsel vend" data-i="' + i + '" aria-label="Vendor">' + vendorOpts + '</select></span>' +
                            '<span class="vas-192-cell right c-std">' + fmtMoney(l.rate) + '</span>';
                    }
                });
            ...
            $mBody.off('change.linechange').on('change.linechange', function (e) {
                ...
                if ($t.hasClass('line')) {
                    ...
                    $t.closest('.vas-192-mrow').toggleClass('sel', line.sel);
                    syncLineSelectionFooter();
        ...
        function syncLineSelectionFooter() {
            ...
            var $note = $('#vas192_selNote');
            var $btn = $mFoot.find('.vas-192-btn-continue');
            if (!$note.length || !$btn.length) { return; }

            if (!sel.length) {
                $note.attr('class', 'vas-192-foot-note').text(lbl('VAS_NoLinesSelected', 'No lines selected'));
            ...
            if (vendors.length > 1) {
                $note.attr('class', 'vas-192-warnnote').text(
            ...
            $note.attr('class', 'vas-192-foot-note').text(
        ...
        function renderPoDetailsPage() {
            ...
            var star = '<span class="vas-192-req-star">*</span>';
            ...
            var formHtml =
                '<div class="vas-192-formwrap">' +
                '  <div class="vas-192-formsec">' + escapeHtml(lbl('VAS_Document', 'Document')) + '</div>' +
                '  <div class="vas-192-form-grid">' +
                '    <div class="vas-192-field"><label>' + escapeHtml(lbl('VAS_TargetDocType', 'Target document type')) + star + '</label><select class="vas-192-fctl" id="vas192_fDocType">' + docTypeOpts + '</select></div>' +
                '    <div class="vas-192-field"><label>' + escapeHtml(lbl('VAS_OrderReference', 'Order reference')) + '</label><input class="vas-192-fctl" id="vas192_fOrderRef" value="' + escapeHtml(poHeadValues.orderRef) + '"></div>' +
                '    <div class="vas-192-field"><label>' + escapeHtml(lbl('VAS_PODate', 'PO date')) + star + '</label><input class="vas-192-fctl" type="date" id="vas192_fPoDate" value="' + escapeHtml(poHeadValues.poDate) + '"></div>' +
                '    <div class="vas-192-field"><label>' + escapeHtml(lbl('VAS_DatePromised', 'Date promised')) + star + '</label><input class="vas-192-fctl" type="date" id="vas192_fPromised" value="' + escapeHtml(poHeadValues.promisedDate) + '"></div>' +
                '  </div>' +
                '  <div class="vas-192-formsec">' + escapeHtml(lbl('VAS_VendorAndPayment', 'Vendor and payment')) + '</div>' +
                '  <div class="vas-192-form-grid">' +
                '    <div class="vas-192-field"><label>' + escapeHtml(lbl('VAS_Vendor', 'Vendor')) + star + '</label><input class="vas-192-fctl" value="' + escapeHtml(poHeadValues.vendorName) + '" disabled></div>' +
                '    <div class="vas-192-field"><label>' + escapeHtml(lbl('VAS_PaymentTerm', 'Payment term')) + star + '</label><select class="vas-192-fctl" id="vas192_fTerm">' + termOpts + '</select></div>' +
                '    <div class="vas-192-field"><label>' + escapeHtml(lbl('VAS_Warehouse', 'Warehouse')) + star + '</label><select class="vas-192-fctl" id="vas192_fWh">' + whOpts + '</select></div>' +
                '    <div class="vas-192-field"><label>' + escapeHtml(lbl('VAS_Priority', 'Priority')) + '</label><select class="vas-192-fctl" id="vas192_fPriority">' + priorityOpts + '</select></div>' +
                '  </div>' +
                '  <div class="vas-192-formsec">' + escapeHtml(lbl('VAS_Description', 'Description')) + '</div>' +
                '  <div class="vas-192-form-grid">' +
                '    <div class="vas-192-field span-2"><label>' + escapeHtml(lbl('VAS_Description', 'Description')) + '</label><input class="vas-192-fctl" id="vas192_fDesc" value="' + escapeHtml(poHeadValues.description) + '"></div>' +
                '    <div class="vas-192-field span-2"><label>' + escapeHtml(lbl('VAS_PrintDescription', 'Print description')) + '</label><input class="vas-192-fctl" id="vas192_fPrintDesc" placeholder="' + escapeHtml(lbl('VAS_PrintDescPlaceholder', 'Text printed on the vendor copy')) + '" value="' + escapeHtml(poHeadValues.printDescription) + '"></div>' +
                '  </div>' +
                '</div>';
            ...
            openModal({
                ...
                foot: '<span class="vas-192-foot-note"><span class="vas-192-req-star">*</span> ' + escapeHtml(lbl('VAS_RequiredNextPage', 'required · line details on the next page')) + '</span>' +
                    '<span><button type="button" class="vas-192-btn vas-192-btn-back">' + escapeHtml(lbl('VAS_Back', 'Back')) + '</button> ' +
                    '<button type="button" class="vas-192-btn vas-192-btn-primary vas-192-to-lines">' + escapeHtml(lbl('VAS_ContinueToLines', 'Continue to lines')) + '</button></span>',
                after: function () {
                    $mFoot.find('.vas-192-btn-back').on('click', function () { backModal(); });
                    $mFoot.find('.vas-192-to-lines').on('click', function () {
                        captureHeadValues();
                        renderPoLinesPage();
                    });
                }
        ...
        function captureHeadValues() {
            poHeadValues.docTypeId = +($('#vas192_fDocType').val() || 0);
            poHeadValues.orderRef = $('#vas192_fOrderRef').val() || '';
            poHeadValues.poDate = $('#vas192_fPoDate').val() || '';
            poHeadValues.promisedDate = $('#vas192_fPromised').val() || '';
            poHeadValues.warehouseId = +($('#vas192_fWh').val() || 0);
            poHeadValues.paymentTermId = +($('#vas192_fTerm').val() || 0);
            poHeadValues.priority = $('#vas192_fPriority').val() || 'Normal';
            poHeadValues.description = $('#vas192_fDesc').val() || '';
            poHeadValues.printDescription = $('#vas192_fPrintDesc').val() || '';
        }
        ...
        function renderPoLinesPage() {
            ...
            var linesTableHtml = pagedTable(poLineCols, selectedPoLines, {
                ...
                render: function (l, i) {
                    return '<span class="vas-192-cell c-dark" title="' + escapeHtml(l.productName) + '">' + escapeHtml(l.productName) + '</span>' +
                        '<span class="vas-192-cell c-std" title="' + escapeHtml(l.attribute || 'Standard') + '">' + escapeHtml(l.attribute || 'Standard') + '</span>' +
                        '<span class="vas-192-cell c-std">' + escapeHtml(l.uom) + '</span>' +
                        '<span class="vas-192-cell right c-dark">' + fmtNum(l.orderQty) + '</span>' +
                        '<span class="vas-192-cell right c-std">' + fmtMoney(l.rate) + '</span>' +
                        '<span class="vas-192-cell right c-emph">' + fmtMoney(l.orderQty * l.rate) + '</span>' +
                        '<span class="vas-192-cell"><input class="vas-192-rowin ldate" type="date" value="' + escapeHtml(l.promised || poHeadValues.promisedDate) + '" data-i="' + i + '" aria-label="Date promised"></span>';
                }
            });

            var totalsBarHtml =
                '<div class="vas-192-totline">' +
            ...
            openModal({
                ...
                body: summaryHtml +
                    '<div class="vas-192-msec">' + escapeHtml(lbl('VAS_ConfirmLinesHeader', 'Purchase order lines')) + '</div>' +
                    linesTableHtml +
                    totalsBarHtml,
                foot: '<span class="vas-192-foot-note" id="vas192_poFootNote">' +
                    ...
                    '</span>' +
                    '<span><button type="button" class="vas-192-btn vas-192-btn-back">' + escapeHtml(lbl('VAS_BackToDetails', 'Back to details')) + '</button> ' +
                    '<button type="button" class="vas-192-btn vas-192-btn-primary vas-192-btn-create-po">' + escapeHtml(lbl('VAS_CreatePO', 'Create PO')) + '</button></span>',
                after: function () {
                    $mFoot.find('.vas-192-btn-back').on('click', function () { backModal(); });
                    $mFoot.find('.vas-192-btn-create-po').on('click', function () {
                        executeCreatePO();
                    });
                }
            });
        ...
        function executeCreatePO() {
            ...
            var $createBtn = $mFoot.find('.vas-192-btn-create-po');
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_192_NewPurchaseOrderWidget/CreatePurchaseOrder',
                type: 'POST',
                data: payload,
                cache: false,
                success: function (response) {
        ...
        this.disposeComponent = function () {
            ...
            $(document).off('keydown.vas192');
            $(window).off('resize.vas192');
            $root.remove();
        };
    };

    VAS.VAS_192_NewPurchaseOrderWidget.prototype.widgetFirevalueChanged = function (value) {
        if (this.listener) { this.listener.widgetFirevalueChanged(value); }
    };

    VAS.VAS_192_NewPurchaseOrderWidget.prototype.addChangeListener = function (listener) {
        this.listener = listener;
    };

    VAS.VAS_192_NewPurchaseOrderWidget.prototype.init = function (windowNo, frame) {
        this.frame = frame;
    ...
    VAS.VAS_192_NewPurchaseOrderWidget.prototype.widgetSizeChange = function (height, width) { };

    VAS.VAS_192_NewPurchaseOrderWidget.prototype.refreshWidget = function () { };

    VAS.VAS_192_NewPurchaseOrderWidget.prototype.dispose = function () {
