...
; (function (VIS, $) {
    ...
    VIS.CollectionEfficiencyWidget = function () {
        ...
        function setNoData() {
            ...
            if ($dsoText) {
                $dsoText.html("<strong>DSO</strong> 0 days · target 22");
            }
        ...
        function updateRing(percent) {
            var safePercent = Math.max(0, Math.min(Number(percent || 0), 100));
            var deg = Math.round(safePercent * 3.6);

            $root.find('.vas-ce-ring').css(
                'background',
                'conic-gradient(#019D89 0deg ' + deg + 'deg, #EDF2F6 ' + deg + 'deg 360deg)'
            );
        }
        ...
        function renderData(data) {
            var percent = Number(data && data.collectionEfficiencyPercent || 0);
            var dsoDays = Number(data && data.dsoDays || 0);
            var targetDays = Number(data && data.dsoTargetDays || 22);
            var overdueAmount = Number(data && data.overdueAmount || 0);
            ...
            if ($overdueText) {
                $overdueText.text(
                    formatCompactAmount(overdueAmount) +
                    " overdue across " +
                    overdueInvoiceCount +
                    " invoices"
                );
        ...
        function createWidget() {
            var $card = $(
                ...
                '<button type="button" class="vas-ce-clear-btn">' +
                lbl("VIS_Clear", "Clear") +
                '</button>' +
                '<button type="button" class="vas-ce-apply-btn">' +
                lbl("VIS_Apply", "Apply") +
                '</button>' +
                '</div>' +
                '</div>' +
            ...
            $card.find('.vas-ce-custom-close').on('click', function (e) {
                e.stopPropagation();
                $customPanel.removeClass('vas-ce-custom-panel-show');
            });
            ...
            $customPanel.on('click', function (e) {
                e.stopPropagation();
            });
