...
; (function (VAS, $) {
    ...
    VAS.VAS_207_DraftedInProcessPOsWidget = function () {
        ...
        var $self = this;
        var $wrapper = $('<div class="vas-207-widget-container"></div>');
        var $card = null;
        var $kpiVal = null;
        var $kpiMeta = null;
        var $kpiSplit = null;
        ...
        function showBusy(show) {
            if ($busy && $busy[0]) {
                $busy.toggleClass('vas-207-hidden', !show);
            }
        }
        ...
        function createWidgetDOM() {
                ...
                '<button type="button" class="vas-207-card vas-207-border-info" aria-label="' + esc(titleText) + '">' +
                    ...
                    '<div class="vas-207-kpi-row">' +
                        '<div class="vas-207-kpi-left">' +
                            '<p class="vas-207-kpi-val vas-207-val-info">—</p>' +
                            '<p class="vas-207-kpi-meta"></p>' +
                        '</div>' +
                        '<div class="vas-207-kpi-side">' +
            ...
            $kpiVal = $card.find('.vas-207-kpi-val');
            $kpiMeta = $card.find('.vas-207-kpi-meta');
            $kpiSplit = $card.find('.vas-207-side-v');
            $kpiSplitLabel = $card.find('.vas-207-side-l');
            ...
            $busy = $('<div class="vas-207-busy vas-207-hidden"><div class="vis-busyindicatorinnerwrap"><i class="vis_widgetloader"></i></div></div>');
            $wrapper.append($busy);
        }
        ...
        function loadWidgetData() {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_207_DraftedInProcessPOsWidget/GetDraftedInProcessPOsData',
                type: 'GET',
                cache: false,
                dataType: 'json',
        ...
        function openDrilldownModal() {
            ...
            var $overlay = $('<div class="vas-207-mask" role="dialog" aria-modal="true"></div>');
            var $dialog = $('<div class="vas-207-modal"></div>');

            // Header
            var $header = $(
                '<div class="vas-207-modal-header">' +
                    '<div class="vas-207-head-left">' +
                        '<button type="button" class="vas-207-xbtn vas-207-mback" aria-label="' + esc(lbl("VAS_Back", "Back")) + '" style="display:none;">' +
                        ...
                        '</div>' +
                    '</div>' +
                    '<div class="vas-207-hact">' +
                        '<button type="button" class="vas-207-xbtn vas-207-mclose" aria-label="' + esc(lbl("VAS_Close", "Close")) + '">' +
            ...
            $(window).off('resize.vas207modal').on('resize.vas207modal', function () {
                if ($activeModalOverlay) {
                    fitModalTableRows($dialog, $body);
                }
            });
        ...
        function renderMainTableView($dialog, $header, $body, $foot) {
            ...
            var $statStrip = $(
                '<div class="vas-207-mstats">' +
                    ...
                    '</div>' +
                    '<div class="vas-207-mstat">' +
                        '<div class="vas-207-stat-l">' + esc(lbl("VAS_Drafted", "Drafted")) + '</div>' +
                    ...
                    '</div>' +
                    '<div class="vas-207-mstat">' +
                        '<div class="vas-207-stat-l">' + esc(lbl("VAS_Value", "Value")) + '</div>' +
                        '<div class="vas-207-stat-v" title="' + esc(formattedTotal) + '">' + esc(formattedTotal) + '</div>' +
                    '</div>' +
                '</div>'
            );
        ...
        function renderMainTableRows($tableWrap, $dialog, $header, $body, $foot) {
            ...
            var $table = $('<div class="vas-207-mtbl"></div>');

            // Table Header
            var $thead = $(
                '<div class="vas-207-mrow vas-207-mhead">' +
                    '<span class="vas-207-cell vas-207-w-icon"></span>' +
                    '<span class="vas-207-cell vas-207-w-pono" title="' + esc(lbl("VAS_PONo", "PO No")) + '">' + esc(lbl("VAS_PONo", "PO No")) + '</span>' +
                    '<span class="vas-207-cell vas-207-w-date" title="' + esc(lbl("VAS_PODate", "PO date")) + '">' + esc(lbl("VAS_PODate", "PO date")) + '</span>' +
                    '<span class="vas-207-cell vas-207-w-vendor" title="' + esc(lbl("VAS_Vendor", "Vendor")) + '">' + esc(lbl("VAS_Vendor", "Vendor")) + '</span>' +
            ...
            // Body Rows
            var $mbody = $('<div class="vas-207-mbody"></div>');
            var sym = cachedData.currencySymbol || '₹';
            var iso = cachedData.currencyIso || 'INR';
                ...
                var $row = $(
                    '<div class="vas-207-mrow vas-207-data-row">' +
                        '<span class="vas-207-cell vas-207-w-icon vas-207-center">' +
                            ...
                            '</button>' +
                        '</span>' +
                        '<span class="vas-207-cell vas-207-w-pono">' +
                            '<button type="button" class="vas-207-lnk vas-207-pono-btn" data-id="' + r.PurchaseOrderId + '" title="' + esc(lbl("VAS_OpenRecord", "Open record") + ' ' + r.PurchaseOrderNumber) + '">' +
                                esc(r.PurchaseOrderNumber) +
                            '</button>' +
                        '</span>' +
                        '<span class="vas-207-cell vas-207-w-date vas-207-c-std" title="' + esc(r.OrderDateFormatted) + '">' + esc(r.OrderDateFormatted) + '</span>' +
                        '<span class="vas-207-cell vas-207-w-vendor vas-207-c-std" title="' + esc(r.VendorName) + '">' + esc(r.VendorName) + '</span>' +
                        '<span class="vas-207-cell vas-207-w-rep vas-207-c-std" title="' + esc(r.RepresentativeName) + '">' + esc(r.RepresentativeName) + '</span>' +
                        '<span class="vas-207-cell vas-207-w-lines vas-207-right vas-207-c-std" title="' + formatNumber(r.LineCount) + '">' + formatNumber(r.LineCount) + '</span>' +
                        '<span class="vas-207-cell vas-207-w-val vas-207-right vas-207-c-emph" title="' + esc(formattedVal) + '">' + esc(formattedVal) + '</span>' +
                        '<span class="vas-207-cell vas-207-w-stage" title="' + esc(stageText) + '">' +
                            '<span class="vas-207-chip ' + stageChipClass + '">' + esc(stageText) + '</span>' +
                        '</span>' +
                    '</div>'
                );

                (function (poRec) {
                    $row.find('.vas-207-pono-btn').on('click', function (e) {
                        e.stopPropagation();
                        openPurchaseOrderRecord(poRec.PurchaseOrderId);
                    });

                    $row.find('.vas-207-lines-btn').on('click', function (e) {
                        e.stopPropagation();
            ...
            $tfoot.find('.vas-207-prev-btn').on('click', function () {
            ...
            $tfoot.find('.vas-207-next-btn').on('click', function () {
        ...
        function openLinesDrilldown(poRec, $dialog, $header, $body, $foot) {
            ...
            $header.find('.vas-207-mtitle').text(lbl("VAS_Lines", "Lines") + ' · ' + poRec.PurchaseOrderNumber);
            $header.find('.vas-207-msub').text(poRec.VendorName + ' · ' + poRec.OrderDateFormatted + ' · ' + poRec.DocStatusName);

            $body.empty();
            $body.html('<div class="vas-207-empty-msg"><div class="vis-busyindicatorinnerwrap"><i class="vis_widgetloader"></i></div></div>');

            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_207_DraftedInProcessPOsWidget/GetOrderLines',
                type: 'GET',
                ...
                success: function (res) {
                    ...
                    if (!data || data.error) {
                        $body.html('<div class="vas-207-empty-msg">' + esc(lbl("VAS_CouldntLoad", "Couldn't load data")) + '</div>');
                        return;
                    }
        ...
        function renderLinesContent(poRec, lines, $dialog, $header, $body, $foot) {
            ...
            var $statStrip = $(
                '<div class="vas-207-mstats">' +
                    '<div class="vas-207-mstat">' +
                        '<div class="vas-207-stat-l">' + esc(lbl("VAS_Lines", "Lines")) + '</div>' +
                        '<div class="vas-207-stat-v" title="' + formatNumber(lines.length) + '">' + formatNumber(lines.length) + '</div>' +
                    '</div>' +
                    '<div class="vas-207-mstat">' +
                        '<div class="vas-207-stat-l">' + esc(lbl("VAS_Value", "PO value")) + '</div>' +
                        '<div class="vas-207-stat-v" title="' + esc(formattedVal) + '">' + esc(formattedVal) + '</div>' +
                    '</div>' +
                    '<div class="vas-207-mstat">' +
                        '<div class="vas-207-stat-l">' + esc(lbl("VAS_QtyOrdered", "Qty ordered")) + '</div>' +
                        '<div class="vas-207-stat-v" title="' + formatNumber(totalOrderedQty) + '">' + formatNumber(totalOrderedQty) + '</div>' +
                    '</div>' +
                    '<div class="vas-207-mstat">' +
                        '<div class="vas-207-stat-l">' + esc(lbl("VAS_QtyPending", "Qty pending")) + '</div>' +
                        '<div class="vas-207-stat-v" title="' + formatNumber(totalPendingQty) + '">' + formatNumber(totalPendingQty) + '</div>' +
                    '</div>' +
                '</div>'
            );
            $body.append($statStrip);
        ...
        function renderLinesTableRows(poRec, lines, $tableWrap) {
            ...
            var $table = $('<div class="vas-207-mtbl vas-207-lines-grid"></div>');

            // Table Header
            var $thead = $(
                '<div class="vas-207-mrow vas-207-mhead vas-207-lines-head">' +
                    '<span class="vas-207-cell vas-207-lh-num vas-207-right" title="#">#</span>' +
                    '<span class="vas-207-cell vas-207-lh-prod" title="' + esc(lbl("VAS_Product", "Product")) + '">' + esc(lbl("VAS_Product", "Product")) + '</span>' +
                    '<span class="vas-207-cell vas-207-lh-attr" title="' + esc(lbl("VAS_Attribute", "Attribute")) + '">' + esc(lbl("VAS_Attribute", "Attribute")) + '</span>' +
                    '<span class="vas-207-cell vas-207-lh-uom" title="' + esc(lbl("VAS_UOM", "UoM")) + '">' + esc(lbl("VAS_UOM", "UoM")) + '</span>' +
                    '<span class="vas-207-cell vas-207-lh-ord vas-207-right" title="' + esc(lbl("VAS_Ordered", "Ordered")) + '">' + esc(lbl("VAS_Ordered", "Ordered")) + '</span>' +
                    '<span class="vas-207-cell vas-207-lh-rec vas-207-right" title="' + esc(lbl("VAS_Received", "Received")) + '">' + esc(lbl("VAS_Received", "Received")) + '</span>' +
                    '<span class="vas-207-cell vas-207-lh-pend vas-207-right" title="' + esc(lbl("VAS_Pending", "Pending")) + '">' + esc(lbl("VAS_Pending", "Pending")) + '</span>' +
                    '<span class="vas-207-cell vas-207-lh-rate vas-207-right" title="' + esc(lbl("VAS_Rate", "Rate")) + '">' + esc(lbl("VAS_Rate", "Rate")) + '</span>' +
                    '<span class="vas-207-cell vas-207-lh-amt vas-207-right" title="' + esc(lbl("VAS_Amount", "Amount")) + '">' + esc(lbl("VAS_Amount", "Amount")) + '</span>' +
                    '<span class="vas-207-cell vas-207-lh-stat" title="' + esc(lbl("VAS_LineStatus", "Line status")) + '">' + esc(lbl("VAS_LineStatus", "Line status")) + '</span>' +
                '</div>'
            );
            $table.append($thead);

            var $mbody = $('<div class="vas-207-mbody vas-207-lines-body"></div>');
            var sym = cachedData.currencySymbol || '₹';
            var iso = cachedData.currencyIso || 'INR';

            for (var i = 0; i < pageRows.length; i++) {
                var l = pageRows[i];
                var chipClass = 'vas-207-chip-neutral';
                if (l.LineStatus === 'Received') { chipClass = 'vas-207-chip-ok'; }
                else if (l.LineStatus === 'Partial received') { chipClass = 'vas-207-chip-warn'; }
        ...
        function fitModalTableRows($dialog, $body) {
            ...
            var $mtwrap = $body.find('.vas-207-mtbl-wrap');
            if (!$mtwrap[0]) { return; }

            var availHeight = $mtwrap.innerHeight() || ($body.innerHeight() - 140);
            if (availHeight < 60) { return; }

            var $head = $mtwrap.find('.vas-207-mhead');
            var $foot = $mtwrap.find('.vas-207-mtfoot');
            var $row = $mtwrap.find('.vas-207-data-row, .vas-207-lines-row').first();
            ...
            if ($mtwrap.find('.vas-207-lines-grid')[0]) {
        ...
        this.disposeComponent = function () {
            ...
            $(window).off('resize.vas207modal');
            $(document).off('keydown.vas207modal');
            if ($card) {
    ...
    VAS.VAS_207_DraftedInProcessPOsWidget.prototype.widgetSizeChange = function (height, width) { };

    VAS.VAS_207_DraftedInProcessPOsWidget.prototype.refreshWidget = function () {
        this.refreshWidget();
    };

    VAS.VAS_207_DraftedInProcessPOsWidget.prototype.dispose = function () {
