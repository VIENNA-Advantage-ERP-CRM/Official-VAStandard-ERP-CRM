...
; (function (VAS, $) {
    ...
    function getDocStatusInfo(docStatus) {
        ...
        switch (code) {
            case 'DR':
                return { text: lbl('VAS_Drafted', 'Drafted'), chip: 'vas-204-chip-neutral' };
            case 'IP':
                return { text: lbl('VAS_InProcess', 'In process'), chip: 'vas-204-chip-prop' };
            case 'CO':
                return { text: lbl('VAS_Completed', 'Completed'), chip: 'vas-204-chip-ok' };
            case 'CL':
                return { text: lbl('VAS_Closed', 'Closed'), chip: 'vas-204-chip-ok' };
            case 'VO':
                return { text: lbl('VAS_Voided', 'Voided'), chip: 'vas-204-chip-risk' };
            case 'RE':
                return { text: lbl('VAS_Reversed', 'Reversed'), chip: 'vas-204-chip-risk' };
            case 'WC':
                return { text: lbl('VAS_WaitingConfirmation', 'Waiting Confirmation'), chip: 'vas-204-chip-prop' };
            case 'WP':
                return { text: lbl('VAS_WaitingPayment', 'Waiting Payment'), chip: 'vas-204-chip-prop' };
            default:
                var translated = VIS.Msg.getMsg(docStatus);
                return { text: (translated && translated.charAt(0) !== '[') ? translated : docStatus, chip: 'vas-204-chip-neutral' };
        }
    }
    ...
    function getDeliveryStatusInfo(status) {
        ...
        if (st === 'fully delivered' || st === 'full') {
            return { text: lbl('VAS_FullyDelivered', 'Fully delivered'), chip: 'vas-204-chip-ok' };
        } else if (st === 'partial') {
            return { text: lbl('VAS_Partial', 'Partial'), chip: 'vas-204-chip-warn' };
        } else if (st === 'not applicable' || st === 'na') {
            return { text: lbl('VAS_NotApplicable', 'Not applicable'), chip: 'vas-204-chip-neutral' };
        } else {
            return { text: lbl('VAS_Pending', 'Pending'), chip: 'vas-204-chip-neutral' };
        }
    }
    ...
    function getLineStatusInfo(status) {
        var st = (status || '').toLowerCase();
        if (st === 'received') {
            return { text: lbl('VAS_Received', 'Received'), chip: 'vas-204-chip-ok' };
        } else if (st === 'partial received' || st === 'partial') {
            return { text: lbl('VAS_PartialReceived', 'Partial received'), chip: 'vas-204-chip-warn' };
        } else if (st === 'drafted') {
            return { text: lbl('VAS_Drafted', 'Drafted'), chip: 'vas-204-chip-neutral' };
        } else if (st === 'voided') {
            return { text: lbl('VAS_Voided', 'Voided'), chip: 'vas-204-chip-risk' };
    ...
    var ICON_EMPTY = '<svg class="vas-204-empty-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 9h18M9 21V9"/></svg>';

    VAS.VAS_204_POQueueWidget = function () {

        this.frame = null;
        this.windowNo = 0;

        var $self = this;
        var $root = $('<div class="vas-204-root">');
        var $card = null;
        ...
        function showBusy(show) {
            if (!$busy) { return; }
            $busy.toggleClass('vas-204-hidden', !show);
        }
        ...
        function createWidgetHtml() {
            ...
            $card = $(
                '<div class="vas-204-card">' +
                    '<div class="vas-204-head">' +
                        '<div class="vas-204-head-txt">' +
                            '<p class="vas-204-title">' + esc(title) + '</p>' +
                            '<p class="vas-204-sub">' + esc(subtitle) + '</p>' +
                        '</div>' +
                        '<div class="vas-204-mfilter">' +
                            '<select class="vas-204-msel vas-204-month-sel" aria-label="' + esc(lbl('VAS_Month', 'Month')) + '"></select>' +
                            '<select class="vas-204-msel vas-204-year-sel" aria-label="' + esc(lbl('VAS_Year', 'Year')) + '"></select>' +
                        '</div>' +
                    '</div>' +
                    '<div class="vas-204-tbl">' +
                        '<div class="vas-204-trow vas-204-thead"></div>' +
                        '<div class="vas-204-tbody"></div>' +
                    '</div>' +
                    '<div class="vas-204-wfoot">' +
                        '<span class="vas-204-helper"></span>' +
                        '<div class="vas-204-pager">' +
                            '<button type="button" class="vas-204-pbtn vas-204-pbtn-prev" data-dir="-1" aria-label="' + esc(lbl('VAS_Previous', 'Previous')) + '">' + ICON_PREV + '</button>' +
                            '<span class="vas-204-ptxt">1 ' + esc(lbl('VAS_Of', 'of')) + ' 1</span>' +
                            '<button type="button" class="vas-204-pbtn vas-204-pbtn-next" data-dir="1" aria-label="' + esc(lbl('VAS_Next', 'Next')) + '">' + ICON_NEXT + '</button>' +
                        '</div>' +
                    '</div>' +
                '</div>'
            );

            $monthSelect = $card.find('.vas-204-month-sel');
            $yearSelect = $card.find('.vas-204-year-sel');
            $tableHead = $card.find('.vas-204-thead');
            $tableBody = $card.find('.vas-204-tbody');
            $helper = $card.find('.vas-204-helper');
            $pageText = $card.find('.vas-204-ptxt');
            $prevBtn = $card.find('.vas-204-pbtn-prev');
            $nextBtn = $card.find('.vas-204-pbtn-next');
            ...
            $tableBody.on('click', '.vas-204-trow', function (e) {
            ...
            $busy = $(
                '<div class="vas-204-busy vas-204-hidden">' +
        ...
        function renderTableHeaders() {
            ...
            for (var i = 0; i < headers.length; i++) {
                var h = headers[i];
                var alignCls = (h.align === 'right') ? ' vas-204-right' : '';
                hHtml += '<span class="vas-204-cell' + alignCls + '" title="' + esc(h.label) + '">' + esc(h.label) + '</span>';
            }
        ...
        function loadQueueData() {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_204_POQueueWidget/GetPOQueueData',
                type: 'GET',
                data: params,
                cache: false,
        ...
        function renderTableRows(records) {
            ...
            for (var i = 0; i < records.length; i++) {
                ...
                rowsHtml +=
                    '<div class="vas-204-trow" data-po-id="' + r.PurchaseOrderID + '" data-po-no="' + esc(poNo) + '">' +
                        '<span class="vas-204-cell vas-204-c-link" title="' + esc(poNo) + '">' + esc(poNo) + '</span>' +
                        '<span class="vas-204-cell vas-204-c-std" title="' + esc(poDateFull) + '">' + esc(poDateShort) + '</span>' +
                        '<span class="vas-204-cell vas-204-c-std" title="' + esc(vendor) + '">' + esc(vendor) + '</span>' +
                        '<span class="vas-204-cell vas-204-c-dark" title="' + esc(warehouse) + '">' + esc(warehouse) + '</span>' +
                        '<span class="vas-204-cell vas-204-c-std" title="' + esc(allReqs) + '">' + esc(firstReq) + '</span>' +
                        '<span class="vas-204-cell vas-204-c-dark" title="' + esc(rep) + '">' + esc(rep) + '</span>' +
                        '<span class="vas-204-cell vas-204-c-std" title="' + esc(expDateFull) + '">' + esc(expDateShort) + '</span>' +
                        '<span class="vas-204-cell vas-204-right vas-204-c-emph" title="' + esc(formattedVal) + '">' + esc(formattedVal) + '</span>' +
                        '<span class="vas-204-cell" title="' + esc(stInfo.text) + '"><span class="vas-204-chip ' + stInfo.chip + '">' + esc(stInfo.text) + '</span></span>' +
                    '</div>';
            }
        ...
        function ensureModalHost() {
                    ...
                    '<div class="vas-204-modal">' +
                        '<div class="vas-204-modal-header">' +
                            '<div class="vas-204-htxt-wrap">' +
                                '<button type="button" class="vas-204-xbtn vas-204-back-btn" aria-label="' + esc(lbl('VAS_Back', 'Back')) + '" style="display:none;">' +
                                    ICON_BACK +
                                '</button>' +
                                '<div class="vas-204-htxt">' +
                                    '<h2 class="vas-204-mtitle"></h2>' +
                                    '<div class="vas-204-msub"></div>' +
                                '</div>' +
                            '</div>' +
                            '<div class="vas-204-hact">' +
                                '<button type="button" class="vas-204-xbtn vas-204-close-btn" aria-label="' + esc(lbl('VAS_Close', 'Close')) + '">' +
                                    ICON_CLOSE +
                                '</button>' +
                            '</div>' +
                        '</div>' +
            ...
            $modalHost.find('.vas-204-close-btn').on('click', closeModal);
            $modalHost.find('.vas-204-back-btn').on('click', popModal);
            ...
            $(window).on('resize.vas204', function () {
                if ($modalHost && $modalHost.hasClass('vas-204-open')) {
                    fitAllTables();
                }
            });

            // Delegate table pagination buttons inside modal
            $modalHost.find('.vas-204-modal-body').on('click', function (e) {
        ...
        function openModal(cfg, isBack) {
            ...
            var $backBtn = $modalHost.find('.vas-204-back-btn');
            ...
            var $mBody = $modalHost.find('.vas-204-modal-body');
            var $mFoot = $modalHost.find('.vas-204-modal-foot');
            var $modal = $modalHost.find('.vas-204-modal');

            $mBody.attr('class', 'vas-204-modal-body' + (cfg.bodyClass ? ' ' + cfg.bodyClass : ''));
            $modal.attr('class', 'vas-204-modal' + (cfg.size ? ' vas-204-' + cfg.size : ''));

            $modalHost.find('.vas-204-mtitle').text(cfg.title || '');
            $modalHost.find('.vas-204-msub').text(cfg.subtitle || '');

            $mBody.html(cfg.body || '');
            $mFoot.html(cfg.foot || '<span class="vas-204-foot-note"></span><button type="button" class="vas-204-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button>');

            $modalHost.addClass('vas-204-open');
        ...
        function closeModal() {
            if ($modalHost) {
                $modalHost.removeClass('vas-204-open');
            }
            modalStack = [];
        ...
        function pagedTable(cols, rows, opts) {
            ...
            return '<div class="vas-204-mtwrap' + (opts.fixed ? ' vas-204-fixed' : '') + '" id="' + id + '"></div>';
        }
        ...
        function drawTable(id) {
            ...
            var headHtml = '<div class="vas-204-mtbl"><div class="vas-204-mrow vas-204-mhead" style="grid-template-columns:' + tpl + '">';
            for (var ci = 0; ci < t.cols.length; ci++) {
                var col = t.cols[ci];
                var rCls = col.align === 'right' ? ' vas-204-right' : (col.align === 'center' ? ' vas-204-center' : '');
                headHtml += '<span class="vas-204-cell' + rCls + '" title="' + esc(col.label || '') + '">' + esc(col.label || '') + '</span>';
            }
            headHtml += '</div><div class="vas-204-mbody">';

            var bodyRowsHtml = '';
            for (var ri = 0; ri < slice.length; ri++) {
                var rowData = slice[ri];
                bodyRowsHtml += '<div class="vas-204-mrow" style="grid-template-columns:' + tpl + '">';
                for (var rci = 0; rci < t.cols.length; rci++) {
                    var cell = rowData[rci];
                    var colDef = t.cols[rci];
                    var alignCls = colDef.align === 'right' ? ' vas-204-right' : (colDef.align === 'center' ? ' vas-204-center' : '');
                    var customCls = colDef.cls ? ' ' + colDef.cls : '';

                    if (cell && typeof cell === 'object' && cell.chip) {
                        bodyRowsHtml += '<span class="vas-204-cell' + alignCls + '" title="' + esc(cell.text) + '"><span class="vas-204-chip ' + cell.chip + '">' + esc(cell.text) + '</span></span>';
                    } else {
                        var cellVal = cell != null ? String(cell) : '—';
                        bodyRowsHtml += '<span class="vas-204-cell' + alignCls + customCls + '" title="' + esc(cellVal) + '">' + esc(cellVal) + '</span>';
                    }
                }
                bodyRowsHtml += '</div>';
            }
            ...
            var footHtml = '<div class="vas-204-mtfoot"><span class="vas-204-helper">' + esc(helperText) + '</span>';
            if (pages > 1) {
                footHtml +=
                    '<span class="vas-204-pager">' +
                        '<button type="button" class="vas-204-pbtn" data-mt="' + id + '" data-dir="-1"' + (t.page === 0 ? ' disabled' : '') + ' aria-label="' + esc(lbl('VAS_Previous', 'Previous')) + '">' + ICON_PREV + '</button>' +
                        '<span class="vas-204-ptxt">' + (t.page + 1) + ' ' + lbl('VAS_Of', 'of') + ' ' + pages + '</span>' +
                        '<button type="button" class="vas-204-pbtn" data-mt="' + id + '" data-dir="1"' + (t.page >= pages - 1 ? ' disabled' : '') + ' aria-label="' + esc(lbl('VAS_Next', 'Next')) + '">' + ICON_NEXT + '</button>' +
                    '</span>';
            } else {
                footHtml += '<span></span>';
            }
        ...
        function fitAllTables() {
            ...
            if ($modalHost) {
                var mBody = $modalHost.find('.vas-204-modal-body')[0];
                if (mBody) {
                    $(mBody).removeClass('vas-204-overflowing');
                    if (mBody.scrollHeight > mBody.clientHeight + 2) {
                        $(mBody).addClass('vas-204-overflowing');
                    }
                }
            }
        }
        ...
        function mstatsHtml(items) {
            var html = '<div class="vas-204-mstats">';
            for (var i = 0; i < items.length; i++) {
                var s = items[i];
                html +=
                    '<div class="vas-204-mstat">' +
                        '<div class="vas-204-l" title="' + esc(s.l) + '">' + esc(s.l) + '</div>' +
                        '<div class="vas-204-v" title="' + esc(s.v) + '">' + esc(s.v) + '</div>' +
                    '</div>';
            }
            html += '</div>';
            return html;
        ...
        function openRecordModal(poId, poNo) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_204_POQueueWidget/GetPODetail?C_Order_ID=' + poId,
                type: 'GET',
                cache: false,
                success: function (res) {
                    var data = parseResponse(res);
                    ...
                    $.ajax({
                        url: VIS.Application.contextUrl + 'VAS_204_POQueueWidget/GetPOLines?C_Order_ID=' + poId,
                        type: 'GET',
                        cache: false,
                        success: function (lineRes) {
                            ...
                            var lineCols = [
                                ...
                                { label: lbl('VAS_Product', 'Product'), w: 1.5, cls: 'vas-204-c-prim' },
                                ...
                                { label: lbl('VAS_Pending', 'Pending'), w: 0.7, align: 'right', cls: 'vas-204-c-prim' },
                                { label: lbl('VAS_Rate', 'Rate'), w: 0.7, align: 'right' },
                                { label: lbl('VAS_Amount', 'Amount'), w: 0.9, align: 'right', cls: 'vas-204-c-emph' },
                            ...
                            var bodyHtml =
                                headerStats +
                                '<div class="vas-204-msec">' + esc(lbl('VAS_PurchaseOrderLines', 'Purchase order lines')) + '</div>' +
                            ...
                            openModal({
                                ...
                                body: bodyHtml,
                                foot: '<span class="vas-204-foot-note">' + esc(footNote) + '</span>' +
                                      '<span><button type="button" class="vas-204-btn vas-204-btn-primary vas-204-btn-open-record">' + esc(lbl('VAS_OpenRecord', 'Open Record')) + '</button> ' +
                                      '<button type="button" class="vas-204-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button></span>',
                                after: function () {
                                    $modalHost.find('.vas-204-btn-open-record').on('click', function () {
    ...
    VAS.VAS_204_POQueueWidget.prototype.widgetFirevalueChanged = function (value) {
        if (this.listener) { this.listener.widgetFirevalueChanged(value); }
    };

    VAS.VAS_204_POQueueWidget.prototype.addChangeListener = function (listener) {
        this.listener = listener;
    };

    VAS.VAS_204_POQueueWidget.prototype.init = function (windowNo, frame) {
        this.frame = frame;
    ...
    VAS.VAS_204_POQueueWidget.prototype.widgetSizeChange = function (height, width) { };

    VAS.VAS_204_POQueueWidget.prototype.refreshWidget = function () {
        this.refreshWidget();
    };

    VAS.VAS_204_POQueueWidget.prototype.dispose = function () {
