 ...
 *                   6 | vs                            | VAS_233_Vs
 *                   7 | Dashboard period              | VAS_233_DashboardPeriod
 *                   8 | Couldn't load                 | VAS_233_CouldntLoad
 *
    ...
    VAS.VAS_233_BankChargesWidget = function () {
        ...
        function loadBootstrap() {
            ...
            $.ajax({
                ...
                success: function (raw) {
                    ...
                    if (!data || data.error) { renderState(label('VAS_233_CouldntLoad', "Couldn't load")); return; }
                ...
                error: function () {
                    if (_disposed) { return; }
                    renderState(label('VAS_233_CouldntLoad', "Couldn't load"));
                }
            });
        }
        ...
        function buildPicker() {
            $picker = $('<div class="vas-233-pp vas-233-hidden" role="listbox" aria-label="' +
                escapeHtml(label('VAS_233_DashboardPeriod', 'Dashboard period')) + '"></div>');
            $('body').append($picker);
