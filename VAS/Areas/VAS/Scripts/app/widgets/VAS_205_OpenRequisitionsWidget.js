/**
 * VAS_205_OpenRequisitionsWidget
...
; (function (VAS, $) {
    ...
    VAS.VAS_205_OpenRequisitionsWidget = function () {
        ...
        var $self = this;
        var $wrapper = $('<div class="vas-205-orw-container"></div>');
        var $root = $('<div class="vas-205-orw-root"></div>');
        ...
        function toast(msg) {
            var $t = $('#vas_205_toast');
            if (!$t.length) {
                $t = $('<div id="vas_205_toast" class="vas-205-orw-toast"></div>');
                $('body').append($t);
            }
        ...
        function createWidgetUI() {
            ...
            $header = $('<div class="vas-205-orw-head"></div>');
            var $headTxt = $('<div class="vas-205-orw-head-txt">' +
                '<p class="vas-205-orw-title" title="' + esc(lbl('VAS_OpenRequisitions', 'Open Requisitions')) + '">' + esc(lbl('VAS_OpenRequisitions', 'Open Requisitions')) + '</p>' +
                '<p class="vas-205-orw-sub" title="' + esc(lbl('VAS_ApprovedAndReadySub', 'Approved and ready to convert into a PO')) + '">' + esc(lbl('VAS_ApprovedAndReadySub', 'Approved and ready to convert into a PO')) + '</p>' +
                '</div>');
            $statPill = $('<span class="vas-205-orw-hpill vas-205-orw-hpill-success" id="vas_205_pill">0 ' + esc(lbl('VAS_LinesReady', 'lines ready')) + '</span>');
            ...
            $tblContainer = $('<div class="vas-205-orw-tbl"></div>');
            var R_COLS = 'minmax(0, 1.6fr) minmax(0, 0.6fr) minmax(0, 0.9fr) minmax(0, 0.9fr) minmax(0, 1fr)';
            $tblHead = $('<div class="vas-205-orw-trow vas-205-orw-thead" style="grid-template-columns:' + R_COLS + ';">' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_Requisition', 'Requisition')) + '">' + esc(lbl('VAS_Requisition', 'Requisition')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_Lines', 'Lines')) + '">' + esc(lbl('VAS_Lines', 'Lines')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_PendingQty', 'Pending qty')) + '">' + esc(lbl('VAS_PendingQty', 'Pending qty')) + '</span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_NeededBy', 'Needed by')) + '">' + esc(lbl('VAS_NeededBy', 'Needed by')) + '</span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_Status', 'Status')) + '">' + esc(lbl('VAS_Status', 'Status')) + '</span>' +
                '</div>');
            $tblBody = $('<div class="vas-205-orw-tbody"></div>');
            $tblContainer.append($tblHead).append($tblBody);

            // 3. Footer with Helper Text and Pager
            $footer = $('<div class="vas-205-orw-wfoot"></div>');
            $helperText = $('<span class="vas-205-orw-helper"></span>');
            $pagerContainer = $('<div class="vas-205-orw-pager"></div>');

            $prevBtn = $('<button type="button" class="vas-205-orw-pbtn" aria-label="' + esc(lbl('VAS_Previous', 'Previous')) + '" disabled>' +
                ...
                '</button>');
            $pageText = $('<span class="vas-205-orw-ptxt">1 ' + lbl('VAS_Of', 'of') + ' 1</span>');
            $nextBtn = $('<button type="button" class="vas-205-orw-pbtn" aria-label="' + esc(lbl('VAS_Next', 'Next')) + '" disabled>' +
            ...
            // 4. Busy Indicator
            $busy = $('<div class="vas-205-orw-busy vas-205-orw-hidden"><div class="vis-busyindicatorinnerwrap"><i class="vis_widgetloader"></i></div></div>');
            ...
            $tblBody.on('click', '.vas-205-orw-trow.pickable', function (e) {
                e.stopPropagation();
        ...
        function showBusy(show) {
            if ($busy && $busy[0]) {
                $busy.toggleClass('vas-205-orw-hidden', !show);
            }
        }
        ...
        function loadOpenRequisitions() {
            ...
            var url = VIS.Application.contextUrl + 'VAS_205_OpenRequisitionsWidget/GetOpenRequisitions';
            $.ajax({
                url: url,
                type: 'GET',
                cache: false,
        ...
        function renderSkeletonRows() {
            ...
            var html = '';
            for (var i = 0; i < 6; i++) {
                html += '<div class="vas-205-orw-trow" style="grid-template-columns:' + R_COLS + ';">' +
                    '<span class="vas-205-orw-cell"><span style="display:block;width:70%;height:1em;background:#EAF1F7;border-radius:4px;"></span></span>' +
                    '<span class="vas-205-orw-cell right"><span style="display:inline-block;width:40%;height:1em;background:#EAF1F7;border-radius:4px;"></span></span>' +
                    '<span class="vas-205-orw-cell right"><span style="display:inline-block;width:50%;height:1em;background:#EAF1F7;border-radius:4px;"></span></span>' +
                    '<span class="vas-205-orw-cell"><span style="display:block;width:60%;height:1em;background:#EAF1F7;border-radius:4px;"></span></span>' +
                    '<span class="vas-205-orw-cell"><span style="display:block;width:65%;height:1em;background:#EAF1F7;border-radius:4px;"></span></span>' +
                    '</div>';
            }
            $tblBody.html(html);
        ...
        function renderErrorState() {
            $tblBody.empty();
            var errHtml = $('<div class="vas-205-orw-empty-box">' +
                '<p class="vas-205-orw-empty-msg">' + esc(lbl('VAS_FailedToLoadOpenReqs', 'Failed to load open requisitions')) + '</p>' +
                '<button type="button" class="vas-205-orw-retry-btn">' + esc(lbl('VAS_Retry', 'Retry')) + '</button>' +
                '</div>');
            errHtml.find('.vas-205-orw-retry-btn').on('click', function () {
                loadOpenRequisitions();
            });
        ...
        function renderGridPage() {
            ...
            if (count === 0) {
                var emptyHtml = '<div class="vas-205-orw-empty-box">' +
                    '<p class="vas-205-orw-empty-msg">' + esc(lbl('VAS_NoOpenReqsFound', 'No open requisitions found')) + '</p>' +
                    '</div>';
            ...
            for (var i = 0; i < pageSlice.length; i++) {
                ...
                rowsHtml += '<div class="vas-205-orw-trow pickable" data-req-id="' + r.requisitionId + '" style="grid-template-columns:' + R_COLS + ';">' +
                    '<span class="vas-205-orw-cell c-link" title="' + esc(reqNo) + '">' + esc(reqNo) + '</span>' +
                    '<span class="vas-205-orw-cell right c-dark" title="' + r.lineCount + '">' + r.lineCount + '</span>' +
                    '<span class="vas-205-orw-cell right c-dark" title="' + num(r.pendingQty) + '">' + num(r.pendingQty) + '</span>' +
                    '<span class="vas-205-orw-cell c-std" title="' + esc(needDisplay) + '">' + esc(needShort) + '</span>' +
                    '<span class="vas-205-orw-cell" title="' + esc(r.status) + '"><span class="vas-205-orw-chip ' + esc(r.statusChip) + '">' + esc(r.status) + '</span></span>' +
                    '</div>';
            }
        ...
        function getModalHost() {
            if (!$modalHost || !$modalHost[0] || !document.body.contains($modalHost[0])) {
                $('#vas_205_modal_mask').remove();
                $modalHost = $('<div class="vas-205-orw-mask" id="vas_205_modal_mask" role="dialog" aria-modal="true">' +
                    '<div class="vas-205-orw-modal" id="vas_205_modal_box">' +
                    '  <div class="vas-205-orw-modal-header">' +
                    '    <div class="vas-205-orw-mhead-left">' +
                    '      <button type="button" class="vas-205-orw-xbtn" id="vas_205_mBack" aria-label="' + esc(lbl('VAS_Back', 'Back')) + '" hidden>' +
                    '        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>' +
                    '      </button>' +
                    '      <div class="vas-205-orw-htxt">' +
                    '        <h2 id="vas_205_mTitle"></h2>' +
                    '        <div class="vas-205-orw-msub" id="vas_205_mSub"></div>' +
                    '      </div>' +
                    '    </div>' +
                    '    <div class="vas-205-orw-hact">' +
                    '      <button type="button" class="vas-205-orw-xbtn" id="vas_205_mClose" aria-label="' + esc(lbl('VAS_Close', 'Close')) + '">' +
                    '        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg>' +
                    '      </button>' +
                    '    </div>' +
                    '  </div>' +
                    '  <div class="vas-205-orw-modal-body" id="vas_205_mBody"></div>' +
                    '  <div class="vas-205-orw-modal-foot" id="vas_205_mFoot"></div>' +
                    '</div>' +
                    '</div>');

                $('body').append($modalHost);

                $modalHost.find('#vas_205_mClose').on('click', function () {
                    closeModal();
                });

                $modalHost.find('#vas_205_mBack').on('click', function () {
                    popModal();
                });
                ...
                $(document).off('keydown.vas_205_esc').on('keydown.vas_205_esc', function (e) {
        ...
        function openModal(cfg, isBack) {
            ...
            var $backBtn = $host.find('#vas_205_mBack');
            ...
            var $box = $host.find('#vas_205_modal_box');
            $box.removeClass('vas-205-orw-modal-sm vas-205-orw-modal-md');
            if (cfg.size === 'sm') $box.addClass('vas-205-orw-modal-sm');
            if (cfg.size === 'md') $box.addClass('vas-205-orw-modal-md');

            var $mBody = $host.find('#vas_205_mBody');
            $mBody.removeClass('compact overflowing');
            if (cfg.bodyClass) {
                $mBody.addClass(cfg.bodyClass);
            }

            $host.find('#vas_205_mTitle').text(cfg.title || '');
            $host.find('#vas_205_mSub').text(cfg.subtitle || '');
            ...
            var $mFoot = $host.find('#vas_205_mFoot');
            $mFoot.empty();
            if (typeof cfg.foot === 'function') {
                ...
                var $defFoot = $('<span class="vas-205-orw-foot-note"></span>' +
                    '<button type="button" class="vas-205-orw-btn vas-205-orw-btn-close">' + esc(lbl('VAS_Close', 'Close')) + '</button>');
                $defFoot.filter('.vas-205-orw-btn-close').on('click', closeModal);
        ...
        function openStep2LinesModal(req) {
            ...
            var summaryStripHtml = '<div class="vas-205-orw-posum">' +
                ...
                '</div>' +
                '<div class="vas-205-orw-msec">' + esc(lbl('VAS_RequisitionLines', 'Requisition lines · select one or many')) + '</div>' +
                '<div class="vas-205-orw-mtwrap" id="vas_205_lines_wrap"></div>';
            ...
            openModal({
                ...
                foot: function ($foot) {
                    $foot.html('<span class="vas-205-orw-foot-note" id="vas_205_step2_note">' + esc(lbl('VAS_NoLinesSelected', 'No lines selected')) + '</span>' +
                        '<span>' +
                        '<button type="button" class="vas-205-orw-btn vas-205-orw-btn-cancel">' + esc(lbl('VAS_Cancel', 'Cancel')) + '</button> ' +
                        '<button type="button" class="vas-205-orw-btn vas-205-orw-btn-primary" id="vas_205_btn_continue" disabled>' + esc(lbl('VAS_Continue', 'Continue')) + '</button>' +
                        '</span>');

                    $foot.find('.vas-205-orw-btn-cancel').on('click', closeModal);
                    $foot.find('#vas_205_btn_continue').on('click', function () {
                        proceedToStep3Details();
                    });
                },
                afterRender: function ($host) {
                    loadRequisitionLines(reqId, $host.find('#vas_205_lines_wrap'));
                }
            });
        }

        function loadRequisitionLines(reqId, $container) {
            $container.html('<div class="vas-205-orw-empty-box"><p class="vas-205-orw-empty-msg">' + esc(lbl('VAS_Loading', 'Loading...')) + '</p></div>');

            // Load lookups and lines together
            var lookupsUrl = VIS.Application.contextUrl + 'VAS_205_OpenRequisitionsWidget/GetFormLookups';
            var linesUrl = VIS.Application.contextUrl + 'VAS_205_OpenRequisitionsWidget/GetRequisitionLines';
            ...
            $.when(
                ...
                $container.html('<div class="vas-205-orw-empty-box"><p class="vas-205-orw-empty-msg">' + esc(lbl('VAS_FailedToLoadOpenReqs', 'Failed to load requisition lines.')) + '</p></div>');
            });
        }
        ...
        function renderPagedStep2LinesTable($container) {
            ...
            var $tableWrap = $('<div class="vas-205-orw-mtbl"></div>');
            var $tableHead = $('<div class="vas-205-orw-mrow vas-205-orw-mhead" style="grid-template-columns:' + LINE_COLS + ';">' +
                '<span class="vas-205-orw-cell"></span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_Product', 'Product')) + '">' + esc(lbl('VAS_Product', 'Product')) + '</span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_Attribute', 'Attribute')) + '">' + esc(lbl('VAS_Attribute', 'Attribute')) + '</span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_UoM', 'UoM')) + '">' + esc(lbl('VAS_UoM', 'UoM')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_ReqQty', 'Req qty')) + '">' + esc(lbl('VAS_ReqQty', 'Req qty')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_AlreadyOrdered', 'Already ordered')) + '">' + esc(lbl('VAS_AlreadyOrdered', 'Already ordered')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_PendingQty', 'Pending')) + '">' + esc(lbl('VAS_PendingQty', 'Pending')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_QtyToOrder', 'Qty to order')) + '">' + esc(lbl('VAS_QtyToOrder', 'Qty to order')) + '</span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_Vendor', 'Vendor')) + '">' + esc(lbl('VAS_Vendor', 'Vendor')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_Rate', 'Rate')) + '">' + esc(lbl('VAS_Rate', 'Rate')) + '</span>' +
                '</div>');

            var $tableBody = $('<div class="vas-205-orw-mbody"></div>');
            var $tableFoot = $('<div class="vas-205-orw-mtfoot"></div>');
            ...
            function drawStep2Page() {
                ...
                if (activeReqLines.length === 0) {
                    $tableBody.html('<div class="vas-205-orw-empty-box"><p class="vas-205-orw-empty-msg">' + esc(lbl('VAS_NoOpenReqsFound', 'No pending lines found')) + '</p></div>');
                    $tableFoot.html('<span class="vas-205-orw-helper">' + esc(lbl('VAS_Showing', 'Showing') + ' 0 ' + lbl('VAS_Of', 'of') + ' 0') + '</span>');
                    return;
                }
                ...
                for (var i = 0; i < slice.length; i++) {
                    ...
                    var rowHtml = '<div class="vas-205-orw-mrow pick' + (l.selected ? ' sel' : '') + '" data-idx="' + globalIdx + '" style="grid-template-columns:' + LINE_COLS + ';">' +
                        '<span class="vas-205-orw-cell"><input type="checkbox" class="vas-205-orw-chk chk-line" data-idx="' + globalIdx + '"' + (l.selected ? ' checked' : '') + ' aria-label="' + esc(lbl('VAS_SelectLine', 'Select line')) + '"></span>' +
                        '<span class="vas-205-orw-cell c-dark" title="' + esc(l.productName) + '">' + esc(l.productName) + '</span>' +
                        '<span class="vas-205-orw-cell c-std" title="' + esc(l.attributeDescription || lbl('VAS_Standard', 'Standard')) + '">' + esc(l.attributeDescription || lbl('VAS_Standard', 'Standard')) + '</span>' +
                        '<span class="vas-205-orw-cell c-std" title="' + esc(l.uomName) + '">' + esc(l.uomName) + '</span>' +
                        '<span class="vas-205-orw-cell right c-std" title="' + num(l.requestedQty) + '">' + num(l.requestedQty) + '</span>' +
                        '<span class="vas-205-orw-cell right ' + (l.alreadyOrderedQty > 0 ? 'c-dark' : 'c-std') + '" title="' + (l.alreadyOrderedQty > 0 ? num(l.alreadyOrderedQty) : '—') + '">' + (l.alreadyOrderedQty > 0 ? num(l.alreadyOrderedQty) : '—') + '</span>' +
                        '<span class="vas-205-orw-cell right c-prim" title="' + num(l.pendingQty) + '">' + num(l.pendingQty) + '</span>' +
                        '<span class="vas-205-orw-cell"><input class="vas-205-orw-rowin in-qty" type="number" min="1" max="' + l.pendingQty + '" value="' + (l.orderQty || l.pendingQty) + '" data-idx="' + globalIdx + '" aria-label="' + esc(lbl('VAS_QtyToOrder', 'Qty to order')) + '"></span>' +
                        '<span class="vas-205-orw-cell"><select class="vas-205-orw-rowsel sel-vend" data-idx="' + globalIdx + '" aria-label="' + esc(lbl('VAS_Vendor', 'Vendor')) + '">' + vendorOpts + '</select></span>' +
                        '<span class="vas-205-orw-cell right c-std" title="' + esc(fmtMoney(l.rate)) + '">' + esc(fmtMoney(l.rate)) + '</span>' +
                        '</div>';
                ...
                var pagerHtml = '<span class="vas-205-orw-helper">' + esc(footHelper) + '</span>';
                if (lTotalPages > 1) {
                    pagerHtml += '<span class="vas-205-orw-pager">' +
                        '<button type="button" class="vas-205-orw-pbtn vas-205-m-prev"' + (lPage === 0 ? ' disabled' : '') + ' aria-label="' + esc(lbl('VAS_Previous', 'Previous')) + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><polyline points="15 18 9 12 15 6"/></svg></button>' +
                        '<span class="vas-205-orw-ptxt">' + (lPage + 1) + ' ' + lbl('VAS_Of', 'of') + ' ' + lTotalPages + '</span>' +
                        '<button type="button" class="vas-205-orw-pbtn vas-205-m-next"' + (lPage >= lTotalPages - 1 ? ' disabled' : '') + ' aria-label="' + esc(lbl('VAS_Next', 'Next')) + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><polyline points="9 18 15 12 9 6"/></svg></button>' +
                        '</span>';
                }

                $tableFoot.html(pagerHtml);

                $tableFoot.find('.vas-205-m-prev').on('click', function (e) {
                    e.stopPropagation();
                    if (lPage > 0) { lPage--; drawStep2Page(); }
                });

                $tableFoot.find('.vas-205-m-next').on('click', function (e) {
                    e.stopPropagation();
            ...
            $tableBody.off('click.rowpick').on('click.rowpick', '.vas-205-orw-mrow.pick', function (e) {
            ...
            $tableBody.off('change.chkline').on('change.chkline', '.chk-line', function (e) {
                ...
                if (activeReqLines[idx]) {
                    ...
                    $(this).closest('.vas-205-orw-mrow').toggleClass('sel', activeReqLines[idx].selected);
                    syncStep2FootNote();
                }
            });
        ...
        function syncStep2FootNote() {
            ...
            var $note = $('#vas_205_step2_note');
            var $btn = $('#vas_205_btn_continue');
            if (!$note.length || !$btn.length) return;

            if (!sel.length) {
                $note.attr('class', 'vas-205-orw-foot-note').text(lbl('VAS_NoLinesSelected', 'No lines selected'));
            ...
            if (distinctVendors.length > 1) {
                $note.attr('class', 'vas-205-orw-warnnote').text(
            ...
            $note.attr('class', 'vas-205-orw-foot-note').text(
        ...
        function openStep3DetailsPage() {
            ...
            var star = '<span class="vas-205-orw-req-star">*</span>';
            ...
            var bodyHtml = summaryStripHtml +
                '<div class="vas-205-orw-formwrap">' +
                '  <div class="vas-205-orw-formsec">' + esc(lbl('VAS_Document', 'Document')) + '</div>' +
                '  <div class="vas-205-orw-form-grid">' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_TargetDocType', 'Target document type')) + star + '</label><select class="vas-205-orw-fctl" id="fDocType">' + docTypeOpts + '</select></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_OrderReference', 'Order reference')) + '</label><input class="vas-205-orw-fctl" id="fOrderRef" value="' + esc(poHeaderValues.orderReference || '') + '"></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_PODate', 'PO date')) + star + '</label><input type="date" class="vas-205-orw-fctl" id="fPoDate" value="' + (poHeaderValues.dateOrdered || fIso()) + '"></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_DatePromised', 'Date promised')) + star + '</label><input type="date" class="vas-205-orw-fctl" id="fPromised" value="' + (poHeaderValues.datePromised || fIso()) + '"></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_Priority', 'Priority')) + '</label><select class="vas-205-orw-fctl" id="fPriority">' +
                ...
                '  <div class="vas-205-orw-formsec">' + esc(lbl('VAS_VendorAndPayment', 'Vendor and payment')) + '</div>' +
                '  <div class="vas-205-orw-form-grid">' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_Vendor', 'Vendor')) + star + '</label><input class="vas-205-orw-fctl" value="' + esc(selectedVendorName) + '" disabled></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_VendorLocation', 'Vendor location')) + star + '</label><select class="vas-205-orw-fctl" id="fVendLoc"></select></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_VendorContact', 'Vendor contact')) + '</label><select class="vas-205-orw-fctl" id="fVendCon"></select></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_PaymentTerm', 'Payment term')) + star + '</label><select class="vas-205-orw-fctl" id="fTerm">' + termOpts + '</select></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_PaymentMethod', 'Payment method')) + '</label><select class="vas-205-orw-fctl" id="fMethod">' + methOpts + '</select></div>' +
                '  </div>' +

                '  <div class="vas-205-orw-formsec">' + esc(lbl('VAS_DeliveryAndPricing', 'Delivery and pricing')) + '</div>' +
                '  <div class="vas-205-orw-form-grid">' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_Warehouse', 'Warehouse')) + star + '</label><select class="vas-205-orw-fctl" id="fWh">' + whOpts + '</select></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_PriceList', 'Price list')) + '</label><select class="vas-205-orw-fctl" id="fPrice">' + plOpts + '</select></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_Currency', 'Currency')) + '</label><select class="vas-205-orw-fctl" id="fCur">' + curOpts + '</select></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_CurrencyRateType', 'Currency rate type')) + '</label><select class="vas-205-orw-fctl" id="fRate">' + convOpts + '</select></div>' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_Incoterm', 'Incoterm')) + '</label><select class="vas-205-orw-fctl" id="fInco">' + incoOpts + '</select></div>' +
                '  </div>' +

                '  <div class="vas-205-orw-formsec">' + esc(lbl('VAS_TaxAndDescription', 'Tax and description')) + '</div>' +
                '  <div class="vas-205-orw-form-grid">' +
                '    <div class="vas-205-orw-field"><label>' + esc(lbl('VAS_Tax', 'Tax')) + star + '</label><select class="vas-205-orw-fctl" id="fTax">' + taxOpts + '</select></div>' +
                '    <div class="vas-205-orw-field span-2"><label>' + ICON_DESC + esc(lbl('VAS_Description', 'Description')) + '</label><input class="vas-205-orw-fctl" id="fDesc" value="' + esc(poHeaderValues.description || '') + '"></div>' +
                '  </div>' +
                '</div>';
            ...
            openModal({
                ...
                foot: function ($foot) {
                    $foot.html('<span class="vas-205-orw-foot-note"><span class="vas-205-orw-req-star">*</span> ' + esc(lbl('VAS_RequiredLineDetailsOnNextPage', 'required · line details on the next page')) + '</span>' +
                        '<span>' +
                        '<button type="button" class="vas-205-orw-btn vas-205-orw-btn-back">' + esc(lbl('VAS_Back', 'Back')) + '</button> ' +
                        '<button type="button" class="vas-205-orw-btn vas-205-orw-btn-primary" id="vas_205_to_lines">' + esc(lbl('VAS_ContinueToLines', 'Continue to lines')) + '</button>' +
                        '</span>');

                    $foot.find('.vas-205-orw-btn-back').on('click', popModal);
                    $foot.find('#vas_205_to_lines').on('click', function () {
                        captureHeaderValues();
                        openStep3LinesPage();
                    });
                },
        ...
        function loadVendorLocationsAndContacts(vendorId, $host) {
            var url = VIS.Application.contextUrl + 'VAS_205_OpenRequisitionsWidget/GetVendorLocationsAndContacts';
            $.ajax({
                url: url,
                type: 'GET',
        ...
        function openStep3LinesPage() {
            ...
            var recapStripHtml = '<div class="vas-205-orw-posum">' +
                '  <div><div class="l">' + esc(lbl('VAS_Vendor', 'Vendor')) + '</div><div class="v">' + esc(selectedVendorName) + '</div></div>' +
                ...
                '</div>' +
                '<div class="vas-205-orw-msec">' + esc(lbl('VAS_POLinesSection', 'Purchase order lines · attribute, UoM, tax, date and text can differ per line')) + '</div>' +
                '<div class="vas-205-orw-mtwrap" id="vas_205_polines_table_wrap"></div>' +
                '<div class="vas-205-orw-totline" id="vas_205_totline">' +
                ...
                '</div>' +
                '<div class="vas-205-orw-linedesc" id="vas_205_linedesc_box" hidden>' +
                '  <div class="ldhead">' + esc(lbl('VAS_LineText', 'Line text')) + ' · <b id="vas_205_ldName"></b></div>' +
                '  <div class="ldgrid">' +
                '    <div class="vas-205-orw-field"><label>' + ICON_DESC + esc(lbl('VAS_Description', 'Description')) + '</label><textarea class="vas-205-orw-fctl" id="vas_205_ldDesc" rows="2"></textarea></div>' +
                '    <div class="vas-205-orw-field"><label>' + ICON_PRINT + esc(lbl('VAS_PrintDescription', 'Print description')) + '</label><textarea class="vas-205-orw-fctl" id="vas_205_ldPrint" rows="2" placeholder="' + esc(lbl('VAS_TextPrintedOnVendorCopy', 'Text printed on the vendor copy')) + '"></textarea></div>' +
                '  </div>' +
                '</div>';
            ...
            openModal({
                ...
                foot: function ($foot) {
                    $foot.html('<span class="vas-205-orw-foot-note" id="vas_205_step3_footnote">' + poLinesState.length + ' ' + (poLinesState.length > 1 ? esc(lbl('VAS_Lines', 'lines')) : esc(lbl('VAS_LineSelected', 'line'))) + ' · ' + num(totals.totalQty) + ' ' + esc(lbl('VAS_Qty', 'qty')) + ' · ' + esc(lbl('VAS_OrderTotal', 'order total')) + ' ' + fmtMoney(totals.grandTotal) + '</span>' +
                        '<span>' +
                        '<button type="button" class="vas-205-orw-btn vas-205-orw-btn-back">' + esc(lbl('VAS_BackToDetails', 'Back to details')) + '</button> ' +
                        '<button type="button" class="vas-205-orw-btn vas-205-orw-btn-primary" id="vas_205_btn_create_po">' + esc(lbl('VAS_CreatePO', 'Create PO')) + '</button>' +
                        '</span>');

                    $foot.find('.vas-205-orw-btn-back').on('click', popModal);
                    $foot.find('#vas_205_btn_create_po').on('click', function () {
                        submitCreatePurchaseOrder();
                    });
                },
                afterRender: function ($host) {
                    renderPagedStep3LinesTable($host.find('#vas_205_polines_table_wrap'), $host);
                }
            });
        }
        ...
        function renderPagedStep3LinesTable($container, $host) {
            ...
            var $tableWrap = $('<div class="vas-205-orw-mtbl"></div>');
            var $tableHead = $('<div class="vas-205-orw-mrow vas-205-orw-mhead" style="grid-template-columns:' + PO_LINE_COLS + ';">' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_Product', 'Product')) + '">' + esc(lbl('VAS_Product', 'Product')) + '</span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_Attribute', 'Attribute')) + '">' + esc(lbl('VAS_Attribute', 'Attribute')) + '</span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_UoM', 'UoM')) + '">' + esc(lbl('VAS_UoM', 'UoM')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_Qty', 'Qty')) + '">' + esc(lbl('VAS_Qty', 'Qty')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_Rate', 'Rate')) + '">' + esc(lbl('VAS_Rate', 'Rate')) + '</span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_Tax', 'Tax')) + '">' + esc(lbl('VAS_Tax', 'Tax')) + '</span>' +
                '<span class="vas-205-orw-cell right" title="' + esc(lbl('VAS_Amount', 'Amount')) + '">' + esc(lbl('VAS_Amount', 'Amount')) + '</span>' +
                '<span class="vas-205-orw-cell" title="' + esc(lbl('VAS_DatePromised', 'Date promised')) + '">' + esc(lbl('VAS_DatePromised', 'Date promised')) + '</span>' +
                '<span class="vas-205-orw-cell"></span>' +
                '</div>');
            ...
            function drawStep3Page() {
                ...
                for (var i = 0; i < slice.length; i++) {
                    ...
                    var rowHtml = '<div class="vas-205-orw-mrow" data-idx="' + globalIdx + '" style="grid-template-columns:' + PO_LINE_COLS + ';">' +
                        '<span class="vas-205-orw-cell c-dark" title="' + esc(l.productName) + '">' + esc(l.productName) + '</span>' +
                        '<span class="vas-205-orw-cell" title="' + esc(l.attributeDescription || lbl('VAS_Standard', 'Standard')) + '">' + esc(l.attributeDescription || lbl('VAS_Standard', 'Standard')) + '</span>' +
                        '<span class="vas-205-orw-cell" title="' + esc(l.uomName) + '">' + esc(l.uomName) + '</span>' +
                        '<span class="vas-205-orw-cell right c-dark" title="' + num(l.orderQty) + '">' + num(l.orderQty) + '</span>' +
                        '<span class="vas-205-orw-cell right c-std" title="' + esc(fmtMoney(l.rate)) + '">' + esc(fmtMoney(l.rate)) + '</span>' +
                        '<span class="vas-205-orw-cell"><select class="vas-205-orw-rowsel sel-ltax" data-idx="' + globalIdx + '" aria-label="' + esc(lbl('VAS_Tax', 'Tax')) + '">' + taxOptionsHtml + '</select></span>' +
                        '<span class="vas-205-orw-cell right c-emph" title="' + esc(fmtMoney(amt)) + '">' + esc(fmtMoney(amt)) + '</span>' +
                        '<span class="vas-205-orw-cell"><input class="vas-205-orw-rowin in-ldate" type="date" value="' + (l.datePromised || fIso()) + '" data-idx="' + globalIdx + '" aria-label="' + esc(lbl('VAS_DatePromised', 'Date promised')) + '"></span>' +
                        '<span class="vas-205-orw-cell vas-205-orw-actcell">' +
                        '  <button type="button" class="vas-205-orw-iconbtn sm btn-act-desc" data-idx="' + globalIdx + '" title="' + esc(lbl('VAS_Description', 'Description')) + '">' + ICON_DESC + '</button>' +
                        '  <button type="button" class="vas-205-orw-iconbtn sm btn-act-print" data-idx="' + globalIdx + '" title="' + esc(lbl('VAS_PrintDescription', 'Print description')) + '">' + ICON_PRINT + '</button>' +
                        '</span>' +
                        '</div>';
        ...
        function refreshStep3Totals($host) {
            var totals = calculatePoTotals();
            var $totline = $host.find('#vas_205_totline');
            if ($totline.length) {
                $totline.html(
                    '<span>' + esc(lbl('VAS_Subtotal', 'Subtotal')) + ' <b>' + esc(fmtMoney(totals.subtotal)) + '</b></span>' +
                    '<span>' + esc(lbl('VAS_Tax', 'Tax')) + ' <b>' + esc(fmtMoney(totals.taxTotal)) + '</b></span>' +
            ...
            var $footNote = $host.find('#vas_205_step3_footnote');
            if ($footNote.length) {
        ...
        function showLineDescExpander(idx, focusPrint, $host) {
            ...
            var $box = $host.find('#vas_205_linedesc_box');
            $box.removeAttr('hidden').show();

            $host.find('#vas_205_ldName').text(line.productName + ' · ' + (line.attributeDescription || lbl('VAS_Standard', 'Standard')));
            var $desc = $host.find('#vas_205_ldDesc');
            var $print = $host.find('#vas_205_ldPrint');
        ...
        function submitCreatePurchaseOrder() {
            ...
            var url = VIS.Application.contextUrl + 'VAS_205_OpenRequisitionsWidget/CreatePurchaseOrder';
        ...
        this.disposeComponent = function () {
            ...
            $(document).off('keydown.vas_205_esc');
            if ($modalHost) {
                $modalHost.remove();
                $modalHost = null;
            }
    ...
    VAS.VAS_205_OpenRequisitionsWidget.prototype.widgetFirevalueChanged = function (value) {
        if (this.listener) {
            this.listener.widgetFirevalueChanged(value);
        }
    };

    VAS.VAS_205_OpenRequisitionsWidget.prototype.addChangeListener = function (listener) {
        this.listener = listener;
    };

    VAS.VAS_205_OpenRequisitionsWidget.prototype.init = function (windowNo, frame) {
        this.frame = frame;
    ...
    VAS.VAS_205_OpenRequisitionsWidget.prototype.widgetSizeChange = function (height, width) { };

    VAS.VAS_205_OpenRequisitionsWidget.prototype.refreshWidget = function () {
        if (this.refreshWidget) {
            this.refreshWidget();
        }
    };

    VAS.VAS_205_OpenRequisitionsWidget.prototype.dispose = function () {
