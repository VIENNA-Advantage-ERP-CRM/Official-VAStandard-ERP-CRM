/**
 * VAS_201_POsExpectedThisMonthWidget
...
; (function (VAS, $) {
    ...
    VAS.VAS_201_POsExpectedThisMonthWidget = function () {
        ...
        var $self = this;
        var $wrapper = $('<div class="vas-201-container"></div>');
        var $root = $('<div class="vas-201-root"></div>');
        var $card = null;
        var $kpiCount = null;
        var $kpiMeta = null;
        var $subTitle = null;
        ...
        function showBusy(show) {
            if (!$busy) { return; }
            $busy.toggleClass('vas-201-hidden', !show);
        }
        ...
        function createWidgetHtml() {
            ...
            $card = $(
                '<section class="vas-201-card vas-201-glass">' +
                    '<div class="vas-201-head">' +
                        '<div class="vas-201-head-txt">' +
                            '<p class="vas-201-title">' + esc(title) + '</p>' +
                            '<p class="vas-201-sub">' + esc(initialSub) + '</p>' +
                        '</div>' +
                    '</div>' +
                    '<button type="button" class="vas-201-headline" id="vas201_headline" aria-label="' + esc(title) + '">' +
                        '<span class="vas-201-kpi-val vas-201-info">—</span>' +
                        '<span class="vas-201-kpi-meta">—</span>' +
                    '</button>' +
                    '<div class="vas-201-mini" id="vas201_mini_list"></div>' +
                '</section>'
            );

            $kpiCount = $card.find('.vas-201-kpi-val');
            $kpiMeta = $card.find('.vas-201-kpi-meta');
            $subTitle = $card.find('.vas-201-sub');
            $listContainer = $card.find('.vas-201-mini');

            // Headline click opens the full expected-PO list modal
            $card.find('.vas-201-headline').on('click', function () {
                openExpectedPOsModal();
            });

            // List row click delegates to open exact Purchase Order record
            $listContainer.on('click', '.vas-201-hrow', function () {
            ...
            $busy = $(
                '<div class="vas-201-busy vas-201-hidden">' +
        ...
        function loadWidgetData() {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_201_POsExpectedThisMonthWidget/GetPOsExpectedThisMonth',
                type: 'GET',
                cache: false,
                data: {
                    year: currentYear,
            ...
            if ($listContainer) {
                ...
                if (records.length === 0) {
                    ...
                    for (var i = 0; i < top3.length; i++) {
                        ...
                        html +=
                            '<button type="button" class="vas-201-hrow" data-po-id="' + p.PurchaseOrderID + '" data-po-no="' + esc(p.PurchaseOrderNo) + '" title="' + esc(lbl('VAS_PurchaseOrder', 'Purchase order') + ' ' + p.PurchaseOrderNo) + '">' +
                                '<span class="vas-201-row-left">' +
                                    '<span class="vas-201-mname vas-201-c-link">' + esc(p.PurchaseOrderNo) + '</span>' +
                                    '<span class="vas-201-mmeta">' + esc(vendorShort) + ' · ' + esc(expDateDisplay) + '</span>' +
                                '</span>' +
                                '<span class="vas-201-mval">' + esc(pVal) + '</span>' +
                            '</button>';
                    }
        ...
        function ensureModalHost() {
            ...
            var html =
                '<div class="vas-201-mask" role="dialog" aria-modal="true">' +
                    '<div class="vas-201-modal">' +
                        '<div class="vas-201-modal-header">' +
                            '<div class="vas-201-htxt-wrap">' +
                                '<button type="button" class="vas-201-xbtn vas-201-back-btn" aria-label="' + esc(lbl('VAS_Back', 'Back')) + '" style="display:none;">' +
                                    ICON_BACK +
                                '</button>' +
                                '<div class="vas-201-htxt">' +
                                    '<h2 class="vas-201-mtitle"></h2>' +
                                    '<div class="vas-201-msub"></div>' +
                                '</div>' +
                            '</div>' +
                            '<div class="vas-201-hact">' +
                                '<button type="button" class="vas-201-xbtn vas-201-close-btn" aria-label="' + esc(lbl('VAS_Close', 'Close')) + '">' +
                                    ICON_CLOSE +
                                '</button>' +
                            '</div>' +
                        '</div>' +
            ...
            $modalHost.find('.vas-201-close-btn').on('click', closeModal);
            $modalHost.find('.vas-201-back-btn').on('click', popModal);
            ...
            $(window).on('resize.vas201', function () {
                if ($modalHost && $modalHost.hasClass('vas-201-open')) {
                    fitAllTables();
                }
            });

            // Delegate table click events
            $modalHost.find('.vas-201-modal-body').on('click', function (e) {
                ...
                var $lk = $(e.target).closest('.vas-201-lnk');
                if ($lk.length) {
        ...
        function openModal(cfg, isBack) {
            ...
            var $backBtn = $modalHost.find('.vas-201-back-btn');
            ...
            var $modal = $modalHost.find('.vas-201-modal');
            $modal.removeClass('vas-201-modal-sm vas-201-modal-md');
            if (cfg.size === 'sm') { $modal.addClass('vas-201-modal-sm'); }
            if (cfg.size === 'md') { $modal.addClass('vas-201-modal-md'); }

            $modalHost.find('.vas-201-mtitle').text(cfg.title || '');
            $modalHost.find('.vas-201-msub').text(cfg.subtitle || '');
            $modalHost.find('.vas-201-modal-body').html(cfg.body || '');
            $modalHost.find('.vas-201-modal-foot').html(cfg.foot || '<span class="vas-201-foot-note"></span><button type="button" class="vas-201-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button>');

            $modalHost.addClass('vas-201-open');
            drawAllTables();
        ...
        function closeModal() {
            if ($modalHost) {
                $modalHost.removeClass('vas-201-open');
            }
            modalStack = [];
        ...
        function pagedTable(cols, rows, opts) {
            ...
            return '<div class="vas-201-mtwrap' + (opts.fixed ? ' vas-201-fixed' : '') + '" id="' + id + '"></div>';
        }

        function cellHTML(cell, col) {
            if (cell && typeof cell === 'object') {
                if (cell.link) {
                    return '<span class="vas-201-cell"><button type="button" class="vas-201-lnk" data-po-id="' + esc(cell.id) + '" data-po-no="' + esc(cell.link) + '" title="' + esc(cell.link) + '">' + esc(cell.link) + '</button></span>';
                }
                if (cell.icon) {
                    return '<span class="vas-201-cell vas-201-center"><button type="button" class="vas-201-iconbtn" data-lines-po-id="' + esc(cell.id) + '" data-lines-po-no="' + esc(cell.icon) + '" title="' + esc(lbl('VAS_Lines', 'Lines')) + '">' + ICON_LINES + '</button></span>';
                }
                if (cell.chip) {
                    return '<span class="vas-201-cell" title="' + esc(cell.text) + '"><span class="vas-201-chip ' + esc(cell.chip) + '">' + esc(cell.text) + '</span></span>';
                }
            }
            var alignClass = (col && col.align === 'right') ? ' vas-201-right' : '';
            var textClass = (col && col.cls) ? (' ' + col.cls) : ' vas-201-c-std';
            return '<span class="vas-201-cell' + alignClass + textClass + '" title="' + esc(cell) + '">' + esc(cell) + '</span>';
        }
        ...
        function drawTable(id) {
            ...
            var h = '<div class="vas-201-mtbl">' +
                        '<div class="vas-201-mrow vas-201-mhead" style="grid-template-columns:' + tpl + '">';
            for (var ci = 0; ci < t.cols.length; ci++) {
                var c = t.cols[ci];
                var cAlign = c.align === 'right' ? ' vas-201-right' : '';
                h += '<span class="vas-201-cell' + cAlign + '" title="' + esc(c.label || '') + '">' + esc(c.label || '') + '</span>';
            }
            h += '</div>' +
                 '<div class="vas-201-mbody">';

            if (slice.length === 0) {
                h += '<div class="vas-201-empty-row">' + esc(lbl('VAS_NoExpectedPOsFound', 'No expected POs found for this month')) + '</div>';
            } else {
                for (var ri = 0; ri < slice.length; ri++) {
                    var r = slice[ri];
                    h += '<div class="vas-201-mrow" style="grid-template-columns:' + tpl + '">';
            ...
            h += '<div class="vas-201-mtfoot">' +
                    '<span class="vas-201-helper">' + esc(showingText) + '</span>';

            if (pages > 1) {
                h += '<span class="vas-201-pager">' +
                        '<button type="button" class="vas-201-pbtn" data-mt="' + id + '" data-dir="-1"' + (t.page === 0 ? ' disabled' : '') + ' aria-label="Previous">' + ICON_PREV + '</button>' +
                        '<span class="vas-201-ptxt">' + (t.page + 1) + ' ' + lbl('VAS_Of', 'of') + ' ' + pages + '</span>' +
                        '<button type="button" class="vas-201-pbtn" data-mt="' + id + '" data-dir="1"' + (t.page >= pages - 1 ? ' disabled' : '') + ' aria-label="Next">' + ICON_NEXT + '</button>' +
                     '</span>';
            } else {
                h += '<span></span>';
            }
        ...
        function fitTable(id) {
            ...
            var head = el.querySelector('.vas-201-mhead');
            var foot = el.querySelector('.vas-201-mtfoot');
            var row = el.querySelector('.vas-201-mbody .vas-201-mrow');
            if (!head || !row) { return; }
        ...
        function mstatsHtml(items) {
            var h = '<div class="vas-201-mstats">';
            for (var i = 0; i < items.length; i++) {
                var it = items[i];
                h += '<div class="vas-201-mstat">' +
                        '<div class="vas-201-mstat-l">' + esc(it.l) + '</div>' +
                        '<div class="vas-201-mstat-v" title="' + esc(it.v) + '">' + esc(it.v) + '</div>' +
                     '</div>';
            }
            h += '</div>';
            return h;
        ...
        function openExpectedPOsModal() {
            ...
            var cols = [
                { label: '', w: 0.32 },
                { label: lbl('VAS_PONo', 'PO No'), w: 1.2, cls: 'vas-201-c-link' },
                ...
                { label: lbl('VAS_Value', 'Value'), w: 0.9, align: 'right', cls: 'vas-201-c-emph' },
            ...
            var rows = all.map(function (p) {
                ...
                var delivChip = (p.DeliveryStatusChip === 'chip-warn') ? 'vas-201-chip-warn' : 'vas-201-chip-neutral';
                var statusText = p.DocStatusText || p.DocumentStatus;
                var statusChip = (p.DocStatusChip === 'chip-prop') ? 'vas-201-chip-prop' : 'vas-201-chip-neutral';
            ...
            var bodyHtml =
                statStrip +
                '<div class="vas-201-msec">' + esc(lbl('VAS_ExpectedDeliveries', 'Expected deliveries')) + '</div>' +
            ...
            openModal({
                ...
                body: bodyHtml,
                foot: '<span class="vas-201-foot-note">' + all.length + ' ' + esc(lbl('VAS_ExpectedPOs', 'Expected POs')) + ' · ' + esc(formattedValue) + '</span>' +
                      '<span><button type="button" class="vas-201-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button></span>'
            });
        }
        ...
        function openLinesModal(poId, poNo) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_201_POsExpectedThisMonthWidget/GetPOLines',
                type: 'GET',
                cache: false,
                ...
                success: function (res) {
                    ...
                    var lineCols = [
                        ...
                        { label: lbl('VAS_Product', 'Product'), w: 1.5, cls: 'vas-201-c-prim' },
                        ...
                        { label: lbl('VAS_Pending', 'Pending'), w: 0.7, align: 'right', cls: 'vas-201-c-prim' },
                        { label: lbl('VAS_Rate', 'Rate'), w: 0.7, align: 'right' },
                        { label: lbl('VAS_Amount', 'Amount'), w: 0.9, align: 'right', cls: 'vas-201-c-emph' },
                    ...
                    var topPolink =
                        '<div class="vas-201-polink">' +
                            esc(lbl('VAS_PurchaseOrder', 'Purchase order')) + ' ' +
                            '<button type="button" class="vas-201-lnk" data-po-id="' + poId + '" data-po-no="' + esc(poNo) + '">' + esc(poNo) + '</button>' +
                    ...
                    var bodyHtml =
                        topPolink +
                        statStrip +
                        '<div class="vas-201-msec">' + esc(lbl('VAS_PurchaseOrderLines', 'Purchase order lines')) + '</div>' +
                    ...
                    openModal({
                        ...
                        body: bodyHtml,
                        foot: '<span class="vas-201-foot-note">' + esc(poNo) + ' · ' + esc(vendor || data.vendorName) + '</span>' +
                              '<span><button type="button" class="vas-201-btn" onclick="VAS.VAS_201_POsExpectedThisMonthWidget.back()">' + esc(lbl('VAS_Back', 'Back')) + '</button> ' +
                              '<button type="button" class="vas-201-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button></span>'
                    });
                },
                error: function () {
                    showBusy(false);
        ...
        function openRecordModal(poId, poNo) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + 'VAS_201_POsExpectedThisMonthWidget/GetPODetail',
                type: 'GET',
                cache: false,
                data: { C_Order_ID: poId },
                    ...
                    $.ajax({
                        ...
                        success: function (resLines) {
                            ...
                            openModal({
                                ...
                                body: bodyHtml,
                                foot: '<span class="vas-201-foot-note">' + lines.length + ' ' + esc(lbl('VAS_Lines', 'lines')) + ' · ' + formatNumber(totalOrd) + ' ' + esc(lbl('VAS_QtyOrdered', 'qty ordered')) + ' · ' + esc(lData.deliveryStatus || lbl('VAS_Pending', 'Pending')) + '</span>' +
                                      '<span><button type="button" class="vas-201-btn vas-201-btn-primary" id="vas201_open_window_btn">' + ICON_OPEN_EXT + ' ' + esc(lbl('VAS_OpenInWindow', 'Open in Window')) + '</button> ' +
                                      '<button type="button" class="vas-201-btn" onclick="VAS.VAS_201_POsExpectedThisMonthWidget.back()">' + esc(lbl('VAS_Back', 'Back')) + '</button> ' +
                                      '<button type="button" class="vas-201-btn" data-vas-close="1">' + esc(lbl('VAS_Close', 'Close')) + '</button></span>',
                                after: function () {
    ...
    VAS.VAS_201_POsExpectedThisMonthWidget.back = function () {
        var host = $('.vas-201-mask');
        if (host.length) {
            host.find('.vas-201-back-btn').trigger('click');
        }
    };

    VAS.VAS_201_POsExpectedThisMonthWidget.prototype.widgetFirevalueChanged = function (value) {
        if (this.listener) {
            this.listener.widgetFirevalueChanged(value);
        }
    };

    VAS.VAS_201_POsExpectedThisMonthWidget.prototype.addChangeListener = function (listener) {
        this.listener = listener;
    };

    VAS.VAS_201_POsExpectedThisMonthWidget.prototype.init = function (windowNo, frame) {
        this.frame = frame;
    ...
    VAS.VAS_201_POsExpectedThisMonthWidget.prototype.widgetSizeChange = function (height, width) { };

    VAS.VAS_201_POsExpectedThisMonthWidget.prototype.refreshWidget = function () {
        this.refreshWidget();
    };

    VAS.VAS_201_POsExpectedThisMonthWidget.prototype.dispose = function () {
