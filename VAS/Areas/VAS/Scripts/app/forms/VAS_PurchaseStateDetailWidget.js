...
; (function (VAS, $) {
    ...
    VAS.VAS_PurchaseStateDetailWidget = function () {
        ...
        /*Looping through this array to show label of amount*/
        var msgArray = [VAS.translatedTexts.VAS_DueAmt, VAS.translatedTexts.VAS_DueSoon, VAS.translatedTexts.VAS_Disputed, VAS.translatedTexts.VAS_IsHold, VAS.translatedTexts.VAS_IPAmt,
            ...
            VIS.dataContext.getJSONData(VIS.Application.contextUrl + "VAS/PoReceipt/GetPurchaseStateDetail", { "ISOtrx": VIS.Env.getCtx().isSOTrx($self.windowNo) }, function (dr) {
                ...
                if (gridDataResult != null) {
                    ...
                    if (VIS.Env.getCtx().isSOTrx($self.windowNo)) {
                        ...
                        HeadingDesign += '<h1><span>' + VAS.translatedTexts.VAS_Purchase + '</span><span>' + VAS.translatedTexts.VAS_State + '</span></h1>';
                    }
                    HeadingDesign += '</div>';
                    ...
                    $maindiv.append(
                        ...
                        formatLargeNumber(gridDataResult[0].TotalAmt, gridDataResult[0].stdPrecision) + unit + (gridDataResult[0].Symbol.length == 3 ? ' ' + gridDataResult[0].Symbol : '')+ '</div>' +
                    ...
                    for (var i = 1; i < gridDataResult.length; i++) {
                        $maindiv.append(
                            ...
                            + formatLargeNumber(gridDataResult[i].TotalAmt, gridDataResult[i].stdPrecision) + unit + (gridDataResult[i].Symbol.length == 3 ? ' ' + gridDataResult[i].Symbol : '') + '</div>' +
