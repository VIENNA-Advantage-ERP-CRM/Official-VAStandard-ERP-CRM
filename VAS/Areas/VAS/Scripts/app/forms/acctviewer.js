                    ...
                    if (row[j] != null && typeof (row[j]) == "object") {
                        ...
                        if (row[j] != null && dataObj.DataType[j].indexOf("Date") >= 0) {
                            ...
                            (VIS.translatedTexts.AmtAcctCr == dataObj.Columns[j] ||
                                ...
                                VIS.translatedTexts.AmtSourceCr == dataObj.Columns[j] ||
                                VIS.translatedTexts.AmtSourceDr == dataObj.Columns[j])) {
                            line[dataObj.Columns[j]] = parseFloat(row[j]).toLocaleString(window.navigator.language, { minimumFractionDigits: dataObj.Precision, maximumFractionDigits: dataObj.Precision });
                        }
                        else {
