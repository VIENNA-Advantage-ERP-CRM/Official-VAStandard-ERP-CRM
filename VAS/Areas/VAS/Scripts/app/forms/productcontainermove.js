...
; (function (VIS, $) {
    ...
    function productContainerMove() {
        ...
        function createMainView() {
            ...
            $cmbFromWarehouse = $('<select class="vis-ev-col-readonly">');
            ...
            $cmbToWarehouse = $('<select class="vis-ev-col-readonly">');
        ...
        function loadToWarehouse() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    var result = JSON.parse(data);
                    if (result) {
                        //VA319:_ If Towarehouse not selected then Towarehouse field in form is editable
                        if (fromWarehouse_ID == 0) {
                            $cmbToWarehouse.append("<option value= 0 >Select</option>");
                            $cmbToWarehouse.removeClass("vis-ev-col-readonly");
                        }
                        for (var i = 0; i < result.length; i++) {
                            key = VIS.Utility.Util.getValueOfInt(result[i].ID);
                            value = VIS.Utility.Util.getValueOfString(result[i].Name);
