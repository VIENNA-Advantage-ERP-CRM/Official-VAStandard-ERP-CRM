 ...
 * Purpose        : Delivery Order Bottom Panel — client logic. The customer
 *                  shipment counterpart of VAS_240_RequisitionBottomPanel: the
 *                  same inline line grid, catalog picker, attribute picker, scan
 *                  dialog and dictionary-driven Additional Info modal, over
 *                  M_InOut / M_InOutLine.
 *
 *                  A shipment line carries no money — M_InOutLine has no
 ...
 *                  and it has ONE locator: the bin the goods leave from.
 *
 *                  VAS_249 (GRN) edits the same table in the other direction. The
...
; (function (VAS, $) {
    ...
    VAS.VAS_248_DeliveryOrderBottomPanel = function () {
        ...
        /* Column-name prefixes that belong to core AD/ERP modules and are always present.
           Mirrors the C# _systemPrefixes set in VAS_248_DeliveryOrderBottomPanelModel. */
        var SYSTEM_PREFIXES = {
        ...
        /* A shipment line's editable cells, in tab order. There is no price cell, and
           one locator: the bin the goods leave from. */
        ...
        /* Strip non-numeric characters from a raw quantity string, honour the locale
           ...
           A shipment quantity is never negative, so no sign is accepted. */
        ...
        function icon(name, glyph) { return '<span class="vas-dol-icon" data-icon="' + name + '">' + (glyph || "") + "</span>"; }
        ...
        this.init = function () {
            ...
            $(window).on("resize.vasdol", function () {
                if (fitRaf) return;
        ...
        /* Fallback for browsers WITHOUT :has() - see the "Full-width host" block in
           VAS_248_DeliveryOrderBottomPanel.css. The framework sizes the tab-panel host
        ...
        this.fetchData = function (recordID, page) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_248_DeliveryOrderBottomPanel/GetPanelData",
                ...
                success: function (raw) {
                    ...
                    // new shipment so its val rule re-resolves against the new header context
        ...
        /* ---------- document kind ----------
           ...
           one place: a site that translates VAS_248_DocShipment changes the whole panel
           at once. */
        function docNoun() {
            return lbl("VAS_248_DocShipment", "Delivery Order");
        }
        ...
        function buildShell() {
            ...
            // dropping the vas-dol-is-hidden class).
            var $scanBtn = $('<button type="button" class="vas-dol-btn vas-dol-btn--outline vas-dol-is-hidden" data-action="open-scan">' + icon("scan-line", "▭") +
                "<span>" + esc(lbl("VAS_248_Scan", "Scan")) + "</span></button>");
            $addBtn = $('<button type="button" class="vas-dol-btn vas-dol-btn--outline" data-action="add-line" title="' + esc(lbl("VAS_248_AddLine", "Add line")) + ' (Ctrl+Alt+N)">' + icon("plus", "+") +
                "<span>" + esc(lbl("VAS_248_AddLine", "Add line")) + "</span></button>");
            $saveBtn = $('<button type="button" class="vas-dol-btn vas-dol-btn--save vas-dol-is-disabled" data-action="save-rows" title="' + esc(lbl("VAS_248_SaveRow", "Save row")) + ' (Ctrl+Alt+S)"></button>');
            $deleteBtn = $('<button type="button" class="vas-dol-btn vas-dol-btn--danger vas-dol-is-disabled" data-action="delete-selected" title="' + esc(lbl("VAS_248_DeleteRecord", "Delete record")) + ' (Ctrl+Alt+D)" disabled>' +
                icon("trash", "🗑") + "<span>" + esc(lbl("VAS_248_DeleteRecord", "Delete record")) + ' <span class="vas-dol-sel-count"></span></span></button>');
            $refreshBtn = $('<button type="button" class="vas-dol-btn vas-dol-btn--outline" data-action="refresh" title="' + esc(lbl("VAS_248_Refresh", "Refresh")) + ' (Ctrl+Alt+Q)">' +
                icon("refresh-cw", "↺") + "<span>" + esc(lbl("VAS_248_Refresh", "Refresh")) + "</span></button>");
            $actions.append($scanBtn, $addBtn, $saveBtn, $deleteBtn, $refreshBtn);
            $header.append($actions);
            $panel.append($header);

            var $table = $('<div class="vas-dol-table" role="table"></div>');
        ...
        function buildHeadRow() {
            var $row = $('<div class="vas-dol-row vas-dol-row--head" role="row"></div>');
            $selectAll = $('<input type="checkbox" aria-label="' + esc(lbl("VAS_248_SelectAll", "Select all lines")) + '" />');
            $row.append($('<div class="vas-dol-cell vas-dol-cell--check" role="columnheader"></div>').append($selectAll));
        ...
        function render() {
            ...
            $linesBody.empty();
            if (!lines.length) {
                $linesBody.append('<div class="vas-dol-emptyrow">' + esc(lbl("VAS_248_NoLines", "No lines yet - use Add line")) + "</div>");
            } else {
        ...
        function renderPager() {
            ...
            var showing = lbl("VAS_248_Showing", "Showing") + " " + start + "–" + end + " " + lbl("VAS_248_Of", "of") + " " + total;
            var pageInfo = (linePage + 1) + " " + lbl("VAS_248_Of", "of") + " " + pageCount;
            $pager.html(
                '<span class="vas-dol-linepager__showing">' + esc(showing) + "</span>" +
                '<div class="vas-dol-linepager__nav">' +
                '<button type="button" class="vas-dol-attr-pagebtn" data-act="lp-prev"' + (linePage <= 0 ? " disabled" : "") +
                ' aria-label="' + esc(lbl("VAS_248_Prev", "Previous")) + '">' + icon("chevron-left", "‹") + "</button>" +
                '<span class="vas-dol-linepager__info">' + esc(pageInfo) + "</span>" +
                '<button type="button" class="vas-dol-attr-pagebtn" data-act="lp-next"' + (linePage >= pageCount - 1 ? " disabled" : "") +
                ' aria-label="' + esc(lbl("VAS_248_Next", "Next")) + '">' + icon("chevron-right", "›") + "</button>" +
                "</div>"
            ).show();
        }
        ...
        function renderHeaderButtons() {
            ...
            $addBtn.removeClass("vas-dol-is-hidden");
            $saveBtn.removeClass("vas-dol-is-hidden");
            var plural = n > 1 ? lbl("VAS_248_PluralS", "s") : "";
            var countTxt = n > 0 ? " (" + n + ")" : "";
            $saveBtn.html(icon("hard-drive", "💾") + "<span>" + esc(lbl("VAS_248_SaveRow", "Save row")) + plural + countTxt + "</span>");
            var sc = selectedCount();
            $deleteBtn.find(".vas-dol-sel-count").text(sc > 0 ? "(" + sc + ")" : "");
            ...
            $saveBtn.prop("disabled", locked).toggleClass("vas-dol-is-disabled", locked || n === 0);
            VAS.PanelUtil.applyButtonState([
                { $el: $addBtn, disabled: locked, disabledCls: "vas-dol-is-disabled" },
                { $el: $deleteBtn, disabled: sc === 0 || locked, disabledCls: "vas-dol-is-disabled" },
                { $el: $refreshBtn, disabled: false, disabledCls: "vas-dol-is-disabled" }
            ]);
        }

        function renderRow(line) {
            var v = line.values;
            var $row = $('<div class="vas-dol-row vas-dol-row--line" role="row" data-rowid="' + line.rowId + '"></div>');
            if (line._sel) $row.addClass("is-selected");
            if (line.status === "new" || line.dirty) $row.addClass("is-unsaved");

            // Read-only shipment (completed/void/closed): the row can't be deleted, so its
            ...
            $row.append($('<div class="vas-dol-cell vas-dol-cell--check" role="cell"></div>').append(cb));
            ...
            $row.on("click", function (e) {
                if (!panelEditable()) return;   // checkbox is disabled on a locked shipment
                if ($(e.target).closest("input, select, textarea, button, a, [role=button], .vas-dol-attr-link").length) return;
                line._sel = !line._sel;
            ...
            $row.append(renderEditableCell(line, "description", v.Description, lbl("VAS_248_AddDescription", "Add description…"), { maxLength: colFieldLength("Description") }));
            ...
            if (line._error) {
                $row.addClass("is-invalid")
                    .append('<div class="vas-dol-row-error" role="alert">' + esc(line._error) + "</div>");
            }
            ...
            if (line._busy || line._saving) $row.addClass("is-busy").append(rowSpinHtml(line._saving ? lbl("VAS_248_Saving", "Saving…") : lbl("VAS_248_Calculating", "Calculating…")));
            return $row;
        }

        /* A shipment line's primary value is a product OR a charge - M_InOutLine carries
        ...
        function dispInput(line, field, text, opts) {
            ...
            if (editable && !opts.readOnly) $i.on("click", function () { startEdit(line, field); });
                ...
                $i.addClass("vas-dol-cell-disp--ro");
        ...
        function renderPrimaryCell(line) {
            ...
            if (isEditing) {
                ...
                if (line.values.M_Product_ID > 0 && (line.display.attrName || line.display.hasAttributeSet)) {
                    ...
                    var attrTxt = hasAttr ? line.display.attrName : lbl("VAS_248_SetAttribute", "Set attribute…");
                    var $attr = $('<span class="vas-dol-attr-link"></span>').text(attrTxt).attr("title", attrTxt);
                    if (!hasAttr) $attr.addClass("vas-dol-attr-link--empty");
                    // Clickable only when the shipment is editable AND the product actually
                    // carries an attribute set. On a read-only shipment, or on a saved line
                    ...
                    else $attr.addClass("vas-dol-attr-link--disabled");
                    wrap.append($attr);
                }
            }
            return cell;
        ...
        function renderLocatorCell(line, field, valueKey, displayKey) {
            ...
            if (isEditing) {
                var $sel = $('<select class="vas-dol-cell-edit__select"></select>');
        ...
        function renderQtyUomCell(line) {
            ...
            if (editQty) {
                ...
                wrap.append(dispInput(line, "quantity", hasQ ? fmtAmtInput(v.QtyEntered, QTY_PRECISION) : "",
                    { align: "right", placeholder: lbl("VAS_248_Qty", "Qty"), cls: "vas-dol-qtyval", readOnly: isColumnReadOnly(line, "QtyEntered") }));
            }
        ...
        function renderMoreCell(line) {
            ...
            if (editable && !line._saving && (canUndoEdits || canDiscardNew)) {
                var undoTitle = (canDiscardNew ? lbl("VAS_248_UndoNewLine", "Undo (remove line)") : lbl("VAS_248_UndoChanges", "Undo changes")) + " (Ctrl+Alt+Z)";
                var $undo = $('<button type="button" class="vas-dol-undo-btn" title="' + esc(undoTitle) + '">' + icon("rotate-ccw", "↺") + "</button>");
            ...
            var _btnTitle = _hasVisible
                ? esc(lbl("VAS_248_More", "More"))
                : esc(lbl("VAS_248_NoAdditionalInfo", "No additional info for this line"));
        ...
        function openMoreDialog(line) {
            ...
            var primaryName = line.display.productName || (lbl("VAS_248_Line", "Line") + " " + line.values.Line);

            var $backdrop = $('<div class="vas-dol-dialog-backdrop" id="vasDolMore"></div>');
            var $dialog = $('<div class="vas-dol-dialog"></div>');
            $dialog.html(
                '<header class="vas-dol-dialog__header"><div class="vas-dol-dialog__header-row">' +
                '<h3 class="vas-dol-dialog__title">' + esc(primaryName) + " - " + esc(lbl("VAS_248_AdditionalInfo", "Additional Info")) + "</h3>" +
            ...
            var $mbody = $dialog.find("#vasDolMoreBody");
            ...
            $dialog.on("click", "[data-act=fldgrp-toggle]", function (e) {
                ...
                toggleFieldGroup($(this).closest(".vas-dol-fldgrp"));
            });
            ...
            $mbody.removeClass("vas-dol-more-grid").addClass("vas-dol-dialog__body--loading")
                .html('<div class="vas-dol-dialog-loading"><span class="vas-dol-dialog-spin" aria-label="' +
                    esc(lbl("VAS_248_Loading", "Loading…")) + '"></span></div>');
            setTimeout(function () {
                if (morePopoverFor !== line.rowId || !$mbody.parent().length) return;
                $mbody.removeClass("vas-dol-dialog__body--loading").addClass("vas-dol-more-grid").empty();
                primeLineContext(line);
                appendDynFields(line, $mbody);
                applyFieldGroups($mbody);
                if (!$mbody.children("[data-col]:not(.vas-dol-dyn-hidden)").length)
                    $mbody.append('<p class="vas-dol-empty-message">' +
                        esc(lbl("VAS_248_NoAdditionalInfo", "No additional info for this line")) + "</p>");
                $mbody.find("[data-col]:not(.vas-dol-dyn-hidden)").find("input,select,textarea").first().focus();
            }, 0);
        }
         ...
         * line's values + shipment header + session context. The filtered lists are
        ...
        function ensureRowLookups(line, cb) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_248_DeliveryOrderBottomPanel/GetLookupData",
        ...
        function focusMoreBtn(line) {
            setTimeout(function () {
                var $b = $linesBody.find('[data-rowid="' + line.rowId + '"] .vas-dol-more-btn');
                if ($b.length) $b.focus();
            }, 0);
        }
        ...
        function blockedByDirtyHeader() {
            ...
            showToast(lbl("VAS_248_SaveHeaderFirst", "Please save the header record before adding lines."));
            return true;
        }
        ...
        function resetCatalog(term, inner, line, $inp) {
            ...
            catalog.$pop.on("mousedown", ".vas-dol-catalog-popover__item", function (e) {
                e.preventDefault();
            ...
            catalog.$pop.html('<div class="vas-dol-catalog__hint">' + esc(lbl("VAS_248_Loading", "Loading…")) + "</div>");
        ...
        function loadCatalogPage(inner, line, $inp, isReset) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_248_DeliveryOrderBottomPanel/SearchCatalog",
                type: "GET", dataType: "json",
                data: {
        ...
        function appendCatalogRows(items, startIdx) {
            ...
            if (startIdx === 0 && !items.length) {
                catalog.$pop.html('<div class="vas-dol-catalog__hint">' + esc(lbl("VAS_248_NoMatches", "No matches")) + "</div>");
                positionCatalog();
                return;
            }
        ...
        function catalogRowHtml(it, idx) {
            ...
            // a shipment line takes a product OR a charge, and the two behave differently
            // once picked, so the row must say which it is before it is chosen.
            var badge = it.Kind === "C" ? "charge" : "product";
            var blabel = it.Kind === "C" ? lbl("VAS_248_Charge", "Charge") : lbl("VAS_248_Product", "Product");
            return '<button type="button" class="vas-dol-catalog-popover__item" data-catalog-item="true" data-idx="' + idx +
                '" title="' + esc(it.DisplayName + (it.SearchKey ? " (" + it.SearchKey + ")" : "")) + '">' +
                '<span class="vas-dol-catalog-popover__name">' + esc(it.DisplayName) + "</span>" +
                '<span class="vas-dol-badge vas-dol-badge--' + badge + '">' + esc(blabel) + "</span></button>";
        }
        ...
        function setHighlight(idx) {
            ...
            var $items = catalog.$pop.children(".vas-dol-catalog-popover__item");
         ...
         * mTab / mField / ctx shims backed by THIS line + the parent shipment context -
        ...
        /* The callout string to run for a column: whatever the dictionary configures on
           ...
           because there is nothing to fall back TO: the standard shipment callouts are the
        ...
        function runCallout(line, column, done) {
            ...
            // conversion in one reliable round trip against the same code the shipment
        ...
        function rowSpinHtml(label) { return '<span class="vas-dol-row-spin" aria-label="' + esc(label || "") + '"></span>'; }
        ...
        function setRowBusy(line, on, label) {
            ...
            $r.find(".vas-dol-row-spin").remove();
            if (on) $r.append(rowSpinHtml(label || lbl("VAS_248_Calculating", "Calculating…")));
        }
        ...
        function makeCalloutCtx(line) {
            ...
            function resolve(key) {
                ...
                switch (key) {
                    ...
                    // A customer shipment IS a sales transaction: a callout that branches on
                    // this token must read 'Y', which is also the direction the server model
                    // refuses to serve anything else for.
                    case "IsSOTrx": return "Y";
                    default:
        ...
        function runCalloutServer(line, trigger, done) {
            var v = line.values;
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_248_DeliveryOrderBottomPanel/RunCallout",
                type: "GET", dataType: "json",
                data: {
         ...
         * the shipment header / document context. */
        ...
        /* Resolve a logic token. Login/session tokens (@$Element_OT@, @#Global@) come from
           ...
           else from the current line then the shipment header. A not-found login token
        ...
        function logicCtxVal(line, key) {
            ...
            switch (key) {
                ...
                // A customer shipment is sold, not bought - a field whose logic gates on
                // the sales flag must read 'Y' rather than an unresolved token.
                case "IsSOTrx": return "Y";
                case "Processed": return (parent && parent.Processed) ? "Y" : "N";
                case "DocStatus": return (parent && parent.DocStatus) || "";
                default: break;
            }
            // Header columns the shipment line's DisplayLogic names as tokens. The server
            // omits a column that is NULL on the shipment, so an absent key resolves to "" -
        ...
        function applyFieldGroups($mbody) {
            ...
            $mbody.children(".vas-dol-fldgrp").remove();
            for (var g = 0; g < MORE_FIELD_GROUPS.length; g++) {
                ...
                $anchor.before(
                    '<div class="vas-dol-fldgrp" data-grp="' + grp.anchor + '"' + (collapsed ? ' data-collapsed="1"' : "") + '>' +
                    '<span class="vas-dol-fldgrp-name">' + esc(lbl(grp.key, grp.def)) + "</span>" +
                    '<button type="button" class="vas-dol-fldgrp-toggle" data-act="fldgrp-toggle">' +
                    '<span class="vas-dol-fldgrp-txt">' + esc(collapsed ? lbl("VAS_248_ShowMore", "Show More") : lbl("VAS_248_ShowLess", "Show Less")) + "</span>" +
                    '<svg class="vas-dol-fldgrp-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 15 12 9 18 15"/></svg>' +
                    "</button></div>");
            }
        ...
        function applyGroupCollapse($mbody) {
            $mbody.children(".vas-dol-fldgrp").each(function () {
                var $hdr = $(this);
                $hdr.nextUntil(".vas-dol-fldgrp").toggleClass("vas-dol-grp-collapsed", $hdr.attr("data-collapsed") === "1");
            });
        }
        ...
        function syncGroupHeaders($mbody) {
            ...
            $mbody.children(".vas-dol-fldgrp").each(function () {
                var $hdr = $(this);
                var live = $hdr.nextUntil(".vas-dol-fldgrp", "[data-col]").not(".vas-dol-dyn-hidden").length;
                $hdr.toggleClass("vas-dol-fldgrp-empty", !live);
            });
        }
        ...
        function toggleFieldGroup($hdr) {
            ...
            $hdr.find(".vas-dol-fldgrp-txt").text(collapsed ? lbl("VAS_248_ShowMore", "Show More") : lbl("VAS_248_ShowLess", "Show Less"));
            applyGroupCollapse($hdr.closest(".vas-dol-more-grid"));
        }
        ...
        /* Apply DisplayLogic to the ALREADY-BUILT modal fields: toggle each field's
           visibility (display:none via .vas-dol-dyn-hidden) instead of adding/removing DOM,
        ...
        function applyDynDisplay(line, $mbody) {
            ...
            $mbody.children("[data-col]").each(function () {
                ...
                $(this).toggleClass("vas-dol-dyn-hidden", !show);
                if (show) visible++;
            });
        ...
        function buildDynField(line, m) {
            ...
            $field.toggleClass("vas-dol-dyn-mandatory", dynMandatory(line, m));
        ...
        /* Prime the framework window context (VIS.context @ this panel's windowNo) with the
           shipment HEADER + the CURRENT line's column values, so a modal FK control's
           ...
           while the Shipment window's framework context already holds the Pascal-cased
        ...
        function primeLineContext(line) {
            ...
            if (parent) {
                ...
                // A customer shipment IS a sales transaction; a val rule gating on the sales
                // flag must see 'Y' rather than an unset token.
                stage("IsSOTrx", true);
            ...
            // columns are skipped to avoid casing collisions with the Shipment window
            ...
            // Re-assert the header org so @AD_Org_ID@ always resolves to the shipment's org.
        ...
        /* True when the line's product actually carries an attribute set
           (M_AttributeSet_ID > 0). Reads the raw server flag VASDOLDISP_HasAttrSet off the
        ...
        function productHasAttributeSet(line) {
            var raw = lineVal(line, "VASDOLDISP_HasAttrSet");
        ...
        function buildFallbackControl(line, m, kind, ro) {
            ...
            if (kind === "list") {
                var $l = $('<select></select>');
                $l.append($('<option value=""></option>').text(lbl("VAS_248_SelectOption", "Select")));
        ...
        function refreshMoreDialog(line) {
            ...
            $b.children(".vas-dol-empty-message").remove();
            if (!visible)
                $b.append('<p class="vas-dol-empty-message">' + esc(lbl("VAS_248_NoAdditionalInfo", "No additional info for this line")) + "</p>");
        }
        ...
        function ensureRefLabel(line, m, $input) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_248_DeliveryOrderBottomPanel/GetRefLookup",
                type: "POST", dataType: "json",
        ...
        function buildFkControl(line, m) {
            ...
            var $wrap = $('<div class="vas-dol-fk" style="position:relative"></div>');
            var $i = $('<input type="text" class="vas-dol-field__input" autocomplete="off" />').val(dynDisplay(line, m));
            $wrap.append($i);
        ...
        function showFkPopover($wrap, $i, line, m, rows, fk) {
            ...
            fk.$pop = $('<div class="vas-dol-catalog-popover vas-dol-fk-popover"></div>');
            if (!rows.length) {
                fk.$pop.html('<div class="vas-dol-catalog__hint">' + esc(lbl("VAS_248_NoMatches", "No matches")) + "</div>");
            } else {
                for (var i = 0; i < rows.length; i++) {
                    fk.$pop.append($('<button type="button" class="vas-dol-catalog-popover__item" data-fk-item="true"></button>')
                        .text(rows[i].Name).attr("data-id", rows[i].Id).attr("data-name", rows[i].Name));
                }
            }
            fk.$pop.on("mousedown", ".vas-dol-catalog-popover__item", function (e) {
                e.preventDefault();
        ...
        function validateLine(line) {
            ...
            if (!(v.QtyEntered > 0))
                return { col: "QtyEntered", msg: lbl("VAS_248_QtyRequired", "Quantity must be greater than zero") };

            // The locator is structural on a PRODUCT line: a shipment that does not say
            ...
            for (var d = 0; d < dyn.length; d++) {
                (function (dm) {
                    ...
                    dynRules.push({
                        col: dm.ColumnName,
                        msg: lbl("VAS_248_FieldRequired", "Required") + ": " + (dm.Name || dm.ColumnName),
                        check: function (vv) {
        ...
        function validateUnsaved(batch) {
            ...
            if (anyBad) {
                ...
                var $first = $linesBody.find(".vas-dol-row--line.is-invalid").first();
        ...
        function openAttrDialog(line) {
            ...
            // no caller can bypass it. Uses the raw VASDOLDISP_HasAttrSet flag, not the
            ...
            VIS.AttributeControl.open({
                ...
                // A shipment takes goods OUT of a bin, so the instance is normally being
                ...
                onApply: function (res) {
                    ...
                    // the stock is actually in. Adopt it as the line's locator when the user has
        ...
        function openScanDialog() {
            ...
            var backdrop = $('<div class="vas-dol-dialog-backdrop" id="vasDolScan"></div>');
            var dialog = $('<div class="vas-dol-dialog vas-dol-dialog--wide"></div>');
            dialog.html(
                '<header class="vas-dol-dialog__header">' +
                '<div class="vas-dol-dialog__header-row"><div class="vas-dol-dialog__title-block">' + icon("scan-line", "▭") + '<h3 class="vas-dol-dialog__title">' + esc(lbl("VAS_248_QuickAddBarcode", "Quick add by barcode")) + "</h3></div>" +
                '<p class="vas-dol-dialog__hint">' + esc(lbl("VAS_248_ScanToReviewSubmit", "Scan to add · review · submit")) + "</p></div>" +
                '<p class="vas-dol-dialog__sub">' + esc(lbl("VAS_248_ScanHint", "Scan a product barcode; items collect below. Duplicate scans increase quantity.")) + "</p>" +
                '<div class="vas-dol-scan-input"><button type="button" class="vas-dol-scan-input__icon" data-act="sim-scan" title="' + esc(lbl("VAS_248_SimulateScan", "Simulate scan")) + '">' + icon("scan-line", "▭") + "</button>" +
                '<input type="text" id="vasDolScanInput" placeholder="' + esc(lbl("VAS_248_ListeningScans", "Listening for scans… (type a code + Enter)")) + '" autocomplete="off" /></div>' +
                ...
                '<div class="vas-dol-dialog__actions"><button type="button" class="vas-dol-btn vas-dol-btn--ghost" data-act="close-scan">' + esc(lbl("VAS_248_Cancel", "Cancel")) +
                '</button><button type="button" class="vas-dol-btn vas-dol-btn--primary" data-act="submit-scan" disabled>' + esc(lbl("VAS_248_AddLines", "Add lines")) + "</button></div></footer>");
            backdrop.append(dialog);
            $("body").append(backdrop);

            backdrop.on("mousedown", function (e) { if (e.target === backdrop[0]) closeDialogs(); });
            var $field = dialog.find("#vasDolScanInput");
            ...
            dialog.on("click", ".vas-dol-scan-del", function () { var id = $(this).attr("data-id"); scanState.rows = scanState.rows.filter(function (r) { return r.id !== id; }); renderScanRows(); });
        ...
        function handleScan(code) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_248_DeliveryOrderBottomPanel/ScanLookup",
                ...
                success: function (raw) {
                    ...
                    scanState.error = (it && it.RecordId > 0) ? "" : lbl("VAS_248_NoMatchFor", "No catalog match for") + ' "' + code + '"';
                    renderScanRows();
                },
                error: function (err) { console.log(err); }
            });
        ...
        function renderScanRows() {
            var d = $("#vasDolScan"); if (!d.length) return;
            var err = d.find("#vasDolScanError");
            err.toggleClass("vas-dol-is-hidden", !scanState.error).text(scanState.error);
            var empty = d.find("#vasDolScanEmpty"), grid = d.find("#vasDolScanGrid");
            if (!scanState.rows.length) { empty.removeClass("vas-dol-is-hidden"); grid.addClass("vas-dol-is-hidden"); }
            else {
                empty.addClass("vas-dol-is-hidden"); grid.removeClass("vas-dol-is-hidden");
                var body = d.find("#vasDolScanRows"); body.empty();
                scanState.rows.forEach(function (r) {
                    var matched = !!r.item;
                    var badge = matched ? "product" : "danger";
                    var blabel = matched ? lbl("VAS_248_Product", "Product") : lbl("VAS_248_Unknown", "Unknown");
                    var row = $('<div class="vas-dol-scan-grid__row' + (matched ? "" : " is-unknown") + '"></div>');
                    row.html('<span class="vas-dol-scan-grid__code">' + esc(r.code) + '</span><span class="vas-dol-scan-grid__name">' + esc(matched ? r.item.DisplayName : "—") +
                        '</span><span class="vas-dol-badge vas-dol-badge--' + badge + '">' + esc(blabel) + "</span>" +
                        '<div class="vas-dol-qty-stepper"><button type="button" data-qminus="' + r.id + '" ' + (!matched || parseInt(r.qty, 10) <= 1 ? "disabled" : "") + ">−</button>" +
                        ...
                        '<button type="button" class="vas-dol-icon-btn--danger vas-dol-scan-del" data-id="' + r.id + '">' + icon("trash", "🗑") + "</button>");
                    body.append(row);
                });
            }
            ...
            d.find("#vasDolScanSummary").text(!scanState.rows.length ? lbl("VAS_248_NoScans", "No scans yet")
                : matched.length + " " + lbl("VAS_248_Matched", "matched") + " · " + units + " " + lbl("VAS_248_Units", "unit(s)") + (unknown > 0 ? " · " + unknown + " " + lbl("VAS_248_Unknown", "unknown") : ""));
            d.find("[data-act=submit-scan]").prop("disabled", matched.length === 0).text(lbl("VAS_248_AddLines", "Add lines") + (matched.length ? " (" + matched.length + ")" : ""));
        }
        ...
        function saveRows(done, restrictIds) {
            ...
            batch.forEach(function (l) { l._saving = true; setRowBusy(l, true, lbl("VAS_248_Saving", "Saving…")); });
            renderHeaderButtons();   // the batch no longer counts as "unsaved" -> Save mutes
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_248_DeliveryOrderBottomPanel/SaveLines",
                type: "POST", dataType: "json",
                ...
                success: function (raw) {
                    ...
                    if (res && res.Success) {
                        ...
                        showToast(lbl("VAS_248_LinesSaved", "Lines saved"));
        ...
        function showServerSaveErrors(batch, res) {
            ...
            for (var i = 0; i < errs.length; i++) {
                ...
                if (l) { l._error = errs[i].Message || lbl("VAS_248_SaveFailed", "Save failed"); mapped++; }
            }
            if (!mapped) {
                ...
                var msg = lbl((res && res.ErrorKey) || "VAS_248_SaveFailed", "Save failed") + (detail ? " - " + detail : "");
        ...
        function deleteSelected() {
            ...
            showBusy(true);
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_248_DeliveryOrderBottomPanel/DeleteLines",
                type: "POST", dataType: "json",
                ...
                success: function (raw) {
                    ...
                    if (res && res.Success) {
                        ...
                        showToast(lbl("VAS_248_LinesDeleted", "Lines deleted"));
                        refreshSummary(); refreshHeaderRecord();
                    }
                    else showToast(lbl((res && res.ErrorKey) || "VAS_248_DeleteFailed", "Delete failed"));
                },
                error: function (err) { console.log(err); showBusy(false); showToast(lbl("VAS_248_DeleteFailed", "Delete failed")); }
            });
        }
        ...
        function closeDialogs() { $("#vasDolAttr, #vasDolScan, #vasDolMore").remove(); attrState = null; scanState = null; morePopoverFor = null; }
        ...
        $(document).on("keydown.vasdol", function (e) {
        ...
        $self._shortcuts = VAS.PanelShortcuts.register({
            ...
            hasBlockingDialog: function () {
                return !!(attrState || scanState || morePopoverFor ||
                    document.getElementById("vasDolAttr") ||
                    document.getElementById("vasDolScan"));
            },
            ...
            onDelete: function () {
                if (!selectedCount()) {
                    showToast(lbl("VAS_248_SelectRowToDelete", "Select a row to delete"));
                    return;
                }
                deleteSelected();
            },
        ...
        this.dispose_ = function () {
            ...
            $(document).off("mousedown.vasdol").off("keydown.vasdol");
    ...
    VAS.VAS_248_DeliveryOrderBottomPanel.prototype.startPanel = function (windowNo, curTab) {
    ...
    VAS.VAS_248_DeliveryOrderBottomPanel.prototype.sizeChanged = function (height, width) {
