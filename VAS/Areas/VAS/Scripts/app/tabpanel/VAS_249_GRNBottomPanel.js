/************************************************************
 * Module Name    : CRM Extension VAS_249
 * Purpose        : GRN Bottom Panel — client logic. The vendor
 *                  receipt counterpart of VAS_240_RequisitionBottomPanel: the
 *                  same inline line grid, catalog picker, attribute picker, scan
 *                  dialog and dictionary-driven Additional Info modal, over
 *                  M_InOut / M_InOutLine.
 *
 *                  A receipt line carries no money — M_InOutLine has no
 *                  PriceActual, no C_Tax_ID and no discount, because those belong
 *                  to the order and the invoice — so there is no Unit Price
 *                  column, no Line Amount column and no amount in the totals: the
 *                  summary is a line count and a total quantity. It does carry
 *                  C_Charge_ID, so the catalog offers charges alongside products,
 *                  and it has ONE locator: the bin the goods are received into.
 *
 *                  VAS_248 (Delivery Order) edits the same table in the other direction. The
 *                  two are separate panels over separate models, and the direction
 *                  is enforced server-side, so neither can be used to edit the
 *                  other's documents.
 * Employee Code  : VAI154
 * Date           : 09-Sep-2026
 *
 * Chronological development:
 *   VAI163   2026-09-17  GRN round of corrections:
 *                        - New Record: a plain white panel with a "save the header"
 *                          hint - never the host's blue, and no line details until
 *                          the header is saved. The tab's data-status is watched
 *                          for the insert (the framework never tells a tab panel
 *                          about New Record).
 *                        - One panel per window: a second instance started for the
 *                          same windowNo parks itself (hidden, no shortcuts, no
 *                          fetch) and takes over only if the first is disposed; a
 *                          repeat startPanel on one instance rebuilds in place. This
 *                          is also what stopped the "Nothing to undo" / "Select a
 *                          row to delete" toasts after a shortcut that had in fact
 *                          worked - the second, idle instance was answering too.
 *                          (Later the same day: the stale test no longer reads a
 *                          not-yet-appended root as stale - the host builds both
 *                          instances before appending either - and the shared
 *                          shortcut util swallows auto-repeated keydown.)
 *                        - No document type / number in the heading.
 *                        - No column is dropped at any width or zoom: the tracks
 *                          tighten instead (CSS).
 *                        - Product column follows the tenant's IsAllowNonItem:
 *                          "Product / Charge" (everything) or "Product" (items only).
 *                        - No product code under the product name.
 *                        - Locator: "Select the locator where the goods will be
 *                          received"; no blank row in the dropdown; the resting cell
 *                          always names the locator (never its id).
 *                        - Newest line first (model orders Line DESC); 20 per page.
 *                        - Additional Info: Project Phase, Target Quantity,
 *                          Description Only, Confirmed / Picked / Scrapped Quantity
 *                          and Invoiced are gone; "Dimensions"; Drop Shipment on a
 *                          drop-ship GRN only; Order Line editable; the "..." opens
 *                          read-only on a completed / closed GRN.
 *                        - Add / Save / Delete (and every cell) lock the moment the
 *                          GRN is completed / closed from the header (liveDocLocked).
 *                        - Save on a line with no product flags the line and says so;
 *                          the product cell stays selectable afterwards (a torn-down
...
; (function (VAS, $) {
    ...
    VAS.VAS_249_GRNBottomPanel = function () {
        ...
        /* parent receipt context returned by GetPanelData */
        var parent = null;
        ...
        /* summed MovementQty across EVERY saved line, from the server. The totals strip
           ...
           receipt; it is refreshed on every load / save / delete. */
        var totalQty = 0;
        /* dropdown catalogs */
        var uomList = [], locatorList = [];
        ...
        /* Column-name prefixes that belong to core AD/ERP modules and are always present.
           Mirrors the C# _systemPrefixes set in VAS_249_GRNBottomPanelModel. */
        var SYSTEM_PREFIXES = {
            "AD_": 1, "C_": 1, "M_": 1, "A_": 1, "G_": 1, "K_": 1, "R_": 1, "I_": 1,
            "B_": 1, "T_": 1, "S_": 1, "W_": 1, "U_": 1,
            "VAS_": 1, "VIS_": 1, "VA_": 1, "VB_": 1,
            // Core columns whose prefix names a ROLE, not a module (Ref_InOutLine_ID
        ...
        /* A receipt line's editable cells, in tab order. There is no price cell, and
           one locator: the bin the goods are received into. */
        var TAB_ORDER = ["primary", "description", "locator", "quantity", "uom", "more"];
        ...
        /* Strip non-numeric characters from a raw quantity string, honour the locale
           ...
           A receipt quantity is never negative, so no sign is accepted. */
        ...
        function icon(name, glyph) { return '<span class="vas-grn-icon" data-icon="' + name + '">' + (glyph || "") + "</span>"; }
        ...
        this.init = function () {
            ...
            createBusyIndicator();
            buildShell();
            $(document).on("mousedown.vasgrn", onDocMouseDown);
            registerShortcuts();
            fitHostWidth();
            ...
            $(window).on("resize.vasgrn", function () {
                if (fitRaf) return;
        ...
        /* Fallback for browsers WITHOUT :has() - see the "Full-width host" block in
           VAS_249_GRNBottomPanel.css. The framework sizes the tab-panel host
        ...
        this.fetchData = function (recordID, page) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_249_GRNBottomPanel/GetPanelData",
                type: "GET", dataType: "json",
                data: { M_InOut_ID: recordID, AD_Window_ID: $self.AD_Window_ID || 0, page: reqPage },
                success: function (raw) {
                    ...
                    // new receipt so its val rule re-resolves against the new header context
        ...
        function fromServerRow(r) {
            ...
            vals.M_InOutLine_ID = r.M_InOutLine_ID;
            vals.Line = r.Line;
        ...
        /* Seed every M_InOutLine column (from the cached columnMeta) on a new line so
        ...
        function seedAllColumns(v) {
            ...
            if (parent) v.M_InOut_ID = parent.M_InOut_ID;
        }
        ...
        /* ---------- document kind ----------
           ...
           one place: a site that translates VAS_249_DocReceipt changes the whole panel
           at once. */
        function docNoun() {
            return lbl("VAS_249_DocReceipt", "Goods Receipt");
        }
        ...
        function docTitle() {
            return docMsg("VAS_249_LinesSummaryFor", "{0} Lines & Summary");
        }
        ...
        function primaryHeading() {
            return allowNonItem() ? lbl("VAS_249_ProductCharge", "Product / Charge") : lbl("VAS_249_Product", "Product");
        }
        function primaryPlaceholder() {
            return allowNonItem() ? lbl("VAS_249_AddProductCharge", "Add product / charge…") : lbl("VAS_249_AddProduct", "Add product…");
        }

        /* Re-label everything that names the document. buildShell() builds the header
           before the receipt data arrives, so render() calls this once parent is known,
        ...
        function updateDocTypeLabels() {
            if (!$body) return;
            var title = docTitle();
            $body.find(".vas-grn-panel__title").text(title);
            $body.find(".vas-grn-panel").attr("aria-label", title);
            $body.find(".vas-grn-head-primary").text(primaryHeading());
        ...
        function buildShell() {
            $body.empty();
            var $panel = $('<section class="vas-grn-panel" aria-label="' + esc(docTitle()) + '"></section>');

            var $header = $('<header class="vas-grn-panel__header"></header>');
            $header.append('<div><h2 class="vas-grn-panel__title">' + esc(docTitle()) + '</h2></div>');

            var $actions = $('<div class="vas-grn-panel__actions"></div>');
            // Scan hidden for now (handler/markup kept so it can be re-enabled by
            // dropping the vas-grn-is-hidden class).
            var $scanBtn = $('<button type="button" class="vas-grn-btn vas-grn-btn--outline vas-grn-is-hidden" data-action="open-scan">' + icon("scan-line", "▭") +
                "<span>" + esc(lbl("VAS_249_Scan", "Scan")) + "</span></button>");
            $addBtn = $('<button type="button" class="vas-grn-btn vas-grn-btn--outline" data-action="add-line" title="' + esc(lbl("VAS_249_AddLine", "Add line")) + ' (Ctrl+Alt+N)">' + icon("plus", "+") +
                "<span>" + esc(lbl("VAS_249_AddLine", "Add line")) + "</span></button>");
            $saveBtn = $('<button type="button" class="vas-grn-btn vas-grn-btn--save vas-grn-is-disabled" data-action="save-rows" title="' + esc(lbl("VAS_249_SaveRow", "Save row")) + ' (Ctrl+Alt+S)"></button>');
            $deleteBtn = $('<button type="button" class="vas-grn-btn vas-grn-btn--danger vas-grn-is-disabled" data-action="delete-selected" title="' + esc(lbl("VAS_249_DeleteRecord", "Delete record")) + ' (Ctrl+Alt+D)" disabled>' +
                icon("trash", "🗑") + "<span>" + esc(lbl("VAS_249_DeleteRecord", "Delete record")) + ' <span class="vas-grn-sel-count"></span></span></button>');
            $refreshBtn = $('<button type="button" class="vas-grn-btn vas-grn-btn--outline" data-action="refresh" title="' + esc(lbl("VAS_249_Refresh", "Refresh")) + ' (Ctrl+Alt+Q)">' +
                icon("refresh-cw", "↺") + "<span>" + esc(lbl("VAS_249_Refresh", "Refresh")) + "</span></button>");
            $actions.append($scanBtn, $addBtn, $saveBtn, $deleteBtn, $refreshBtn);
            $header.append($actions);
            $panel.append($header);

            var $table = $('<div class="vas-grn-table" role="table"></div>');
            $table.append(buildHeadRow());
            $linesBody = $('<div class="vas-grn-tbody"></div>');
            $table.append($linesBody);
            // No totals block (17-Sep-2026): the line count and total quantity are not
            // asked for on a receipt, and the strip took the space of a line.
            $panel.append($table);
            // Server-side line pager (20/page): "X-Y of N" + prev/next.
            $pager = $('<div class="vas-grn-linepager" style="display:none;"></div>');
        ...
        function buildHeadRow() {
            var $row = $('<div class="vas-grn-row vas-grn-row--head" role="row"></div>');
            $selectAll = $('<input type="checkbox" aria-label="' + esc(lbl("VAS_249_SelectAll", "Select all lines")) + '" />');
            $row.append($('<div class="vas-grn-cell vas-grn-cell--check" role="columnheader"></div>').append($selectAll));
            ...
            $row.append('<div class="vas-grn-cell vas-grn-head-primary" role="columnheader">' + esc(primaryHeading()) + "</div>");
            $row.append('<div class="vas-grn-cell" role="columnheader">' + esc(lbl("VAS_249_Description", "Description")) + "</div>");
            $row.append('<div class="vas-grn-cell" role="columnheader">' + esc(lbl("VAS_249_Locator", "Locator")) + "</div>");
            $row.append('<div class="vas-grn-cell vas-grn-cell--right" role="columnheader">' + esc(lbl("VAS_249_QtyUom", "Quantity / UOM")) + "</div>");
            $row.append('<div class="vas-grn-cell vas-grn-cell--more" role="columnheader" aria-label="' + esc(lbl("VAS_249_More", "More")) + '"></div>');
            return $row;
        }
        ...
        function render() {
            ...
            $linesBody.empty();
            if (!lines.length) {
                $linesBody.append('<div class="vas-grn-emptyrow">' + esc(lbl("VAS_249_NoLines", "No lines yet - use Add line")) + "</div>");
            } else {
        ...
        /* Server-side line pager: "Showing X-Y of N" on the left, "‹ P of Q ›" on the
           right. Shown whenever the receipt has saved lines (even a single page). */
        function renderPager() {
            ...
            var showing = lbl("VAS_249_Showing", "Showing") + " " + start + "–" + end + " " + lbl("VAS_249_Of", "of") + " " + total;
            var pageInfo = (linePage + 1) + " " + lbl("VAS_249_Of", "of") + " " + pageCount;
            $pager.html(
                '<span class="vas-grn-linepager__showing">' + esc(showing) + "</span>" +
                '<div class="vas-grn-linepager__nav">' +
                '<button type="button" class="vas-grn-attr-pagebtn" data-act="lp-prev"' + (linePage <= 0 ? " disabled" : "") +
                ' aria-label="' + esc(lbl("VAS_249_Prev", "Previous")) + '">' + icon("chevron-left", "‹") + "</button>" +
                '<span class="vas-grn-linepager__info">' + esc(pageInfo) + "</span>" +
                '<button type="button" class="vas-grn-attr-pagebtn" data-act="lp-next"' + (linePage >= pageCount - 1 ? " disabled" : "") +
                ' aria-label="' + esc(lbl("VAS_249_Next", "Next")) + '">' + icon("chevron-right", "›") + "</button>" +
                "</div>"
            ).show();
        }
        ...
        function gotoLinePage(p) {
            ...
            if (unsavedLines().length) { showToast(lbl("VAS_249_SavePageFirst", "Save or discard your changes before changing page")); return; }
        ...
        /* Doc statuses that end the receipt's editable life - the same set the server uses
           for IsEditable (DocStatus NOT IN (CO, CL, VO, RE)). */
        var LOCKED_DOC_STATUS = { CO: 1, CL: 1, VO: 1, RE: 1 };

        /* Whether the receipt the panel is showing is locked RIGHT NOW, read off the
           ...
           Completing (or closing / voiding / reversing) the receipt from the tab's own
           ...
           completed receipt until the record was re-selected. The GridTab knows the new
           status immediately, so it is asked first - in BOTH directions, so a
           re-activated receipt unlocks just as promptly - and only when the tab is
        ...
        /* The panel is editable only while the receipt can still take line changes -
        ...
        /* The hosting tab's data status changed. Two cases matter:
             - New Record. The framework never calls refreshPanelData for it, so the panel
               would keep showing the previous receipt's lines beside an empty header. The
             ...
             - A doc action (Complete / Close / Void / Reverse / Re-activate) run from the
               header, which flips the receipt's editable state under an already-loaded
        ...
        function onTabDataStatus() {
            ...
            if (locked) {
                showToast(docMsg("VAS_249_DocLockedNow",
        ...
        function renderHeaderButtons() {
            ...
            var locked = !panelEditable();
            $addBtn.removeClass("vas-grn-is-hidden");
            $saveBtn.removeClass("vas-grn-is-hidden");
            var plural = n > 1 ? lbl("VAS_249_PluralS", "s") : "";
            var countTxt = n > 0 ? " (" + n + ")" : "";
            $saveBtn.html(icon("hard-drive", "💾") + "<span>" + esc(lbl("VAS_249_SaveRow", "Save row")) + plural + countTxt + "</span>");
            var sc = selectedCount();
            $deleteBtn.find(".vas-grn-sel-count").text(sc > 0 ? "(" + sc + ")" : "");
            ...
            $saveBtn.prop("disabled", locked).toggleClass("vas-grn-is-disabled", locked || n === 0);
            VAS.PanelUtil.applyButtonState([
                { $el: $addBtn, disabled: locked, disabledCls: "vas-grn-is-disabled" },
                { $el: $deleteBtn, disabled: sc === 0 || locked, disabledCls: "vas-grn-is-disabled" },
                { $el: $refreshBtn, disabled: false, disabledCls: "vas-grn-is-disabled" }
            ]);
        }

        function renderRow(line) {
            var v = line.values;
            var $row = $('<div class="vas-grn-row vas-grn-row--line" role="row" data-rowid="' + line.rowId + '"></div>');
            if (line._sel) $row.addClass("is-selected");
            if (line.status === "new" || line.dirty) $row.addClass("is-unsaved");

            // Read-only receipt (completed/void/closed): the row can't be deleted, so its
            ...
            $row.append($('<div class="vas-grn-cell vas-grn-cell--check" role="cell"></div>').append(cb));
            ...
            $row.on("click", function (e) {
                if (!panelEditable()) return;   // checkbox is disabled on a locked receipt
                if ($(e.target).closest("input, select, textarea, button, a, [role=button], .vas-grn-attr-link").length) return;
                line._sel = !line._sel;
            ...
            $row.append(renderEditableCell(line, "description", v.Description, lbl("VAS_249_AddDescription", "Add description…"), { maxLength: colFieldLength("Description") }));
            ...
            if (line._error) {
                $row.addClass("is-invalid")
                    .append('<div class="vas-grn-row-error" role="alert">' + esc(line._error) + "</div>");
            }
            ...
            if (line._busy || line._saving) $row.addClass("is-busy").append(rowSpinHtml(line._saving ? lbl("VAS_249_Saving", "Saving…") : lbl("VAS_249_Calculating", "Calculating…")));
            return $row;
        }

        /* A receipt line's primary value is a product OR a charge - M_InOutLine carries
           both columns and they are mutually exclusive, so the cell shows whichever the
           line actually holds. A line with neither (a fresh row) reads as a product. */
        ...
        function dispInput(line, field, text, opts) {
            ...
            var $i = $('<input type="text" readonly tabindex="-1" draggable="false" class="vas-grn-cell-edit__input vas-grn-cell-disp" />');
            $i.val(text || "");
            ...
            if (editable && !opts.readOnly) $i.on("click", function () { startEdit(line, field); });
                ...
                $i.addClass("vas-grn-cell-disp--ro");
        ...
        function renderPrimaryCell(line) {
            ...
            var cell = $('<div class="vas-grn-cell vas-grn-cell--product" role="cell"></div>');
            var wrap = $('<div class="vas-grn-cell-edit"></div>');
            ...
            if (isEditing) {
                ...
                $inp.attr("placeholder", editing.field === "charge"
                    ...
                    : (allowNonItem() ? lbl("VAS_249_SearchProductCharge", "Search product / charge…")
                                      : lbl("VAS_249_SearchProduct", "Search product…")));
        ...
        function renderEditableCell(line, field, value, placeholder, opts) {
            opts = opts || {};
            var editable = panelEditable();   // live tab status first, then the snapshot
            var cell = $('<div class="vas-grn-cell' + (opts.align === "right" ? " vas-grn-cell--right" : "") + '" role="cell"></div>');
            var wrap = $('<div class="vas-grn-cell-edit"></div>');
        ...
        function renderLocatorCell(line, field, valueKey, displayKey) {
            ...
            var cell = $('<div class="vas-grn-cell" role="cell"></div>');
            var wrap = $('<div class="vas-grn-cell-edit"></div>');
            ...
            if (isEditing) {
                var $sel = $('<select class="vas-grn-cell-edit__select"></select>');
                ...
                wrap.append(dispInput(line, field, locatorLabel(line, valueKey, displayKey),
                    { placeholder: lbl("VAS_249_PickLocator", "Locator…"), readOnly: ro }));
            }
            return cell;
        }
        ...
        function renderQtyUomCell(line) {
            var v = line.values;
            var editable = panelEditable();   // live tab status first, then the snapshot
            var cell = $('<div class="vas-grn-cell vas-grn-cell--right" role="cell"></div>');
            var wrap = $('<div class="vas-grn-cell-edit"></div>');
            ...
            if (editQty) {
                ...
                wrap.append(dispInput(line, "quantity", hasQ ? fmtAmtInput(v.QtyEntered, QTY_PRECISION) : "",
                    { align: "right", placeholder: lbl("VAS_249_Qty", "Qty"), cls: "vas-grn-qtyval", readOnly: isColumnReadOnly(line, "QtyEntered") }));
            }
        ...
        function renderMoreCell(line) {
            var editable = panelEditable();   // live tab status first, then the snapshot
            var cell = $('<div class="vas-grn-cell vas-grn-cell--more" role="cell" style="position:relative"></div>');
            ...
            if (editable && !line._saving && (canUndoEdits || canDiscardNew)) {
                var undoTitle = (canDiscardNew ? lbl("VAS_249_UndoNewLine", "Undo (remove line)") : lbl("VAS_249_UndoChanges", "Undo changes")) + " (Ctrl+Alt+Z)";
                var $undo = $('<button type="button" class="vas-grn-undo-btn" title="' + esc(undoTitle) + '">' + icon("rotate-ccw", "↺") + "</button>");
            ...
            var _btnTitle = _hasVisible
                ? esc(lbl("VAS_249_More", "More"))
                : esc(lbl("VAS_249_NoAdditionalInfo", "No additional info for this line"));
            var $btn = $('<button type="button" class="vas-grn-more-btn" title="' + _btnTitle + '">' + icon("more-horizontal", "⋯") + "</button>");
            ...
            // same DisplayLogic check the modal runs). On a completed / closed receipt the
        ...
        function openMoreDialog(line) {
            ...
            var primaryName = line.display.productName || (lbl("VAS_249_Line", "Line") + " " + line.values.Line);

            var $backdrop = $('<div class="vas-grn-dialog-backdrop" id="vasGrnMore"></div>');
            var $dialog = $('<div class="vas-grn-dialog"></div>');
            $dialog.html(
                '<header class="vas-grn-dialog__header"><div class="vas-grn-dialog__header-row">' +
                '<h3 class="vas-grn-dialog__title">' + esc(primaryName) + " - " + esc(lbl("VAS_249_AdditionalInfo", "Additional Info")) + "</h3>" +
            ...
            var $mbody = $dialog.find("#vasGrnMoreBody");
            ...
            $dialog.on("click", "[data-act=fldgrp-toggle]", function (e) {
                ...
                toggleFieldGroup($(this).closest(".vas-grn-fldgrp"));
            });
            ...
            $mbody.removeClass("vas-grn-more-grid").addClass("vas-grn-dialog__body--loading")
                .html('<div class="vas-grn-dialog-loading"><span class="vas-grn-dialog-spin" aria-label="' +
                    esc(lbl("VAS_249_Loading", "Loading…")) + '"></span></div>');
            setTimeout(function () {
                if (morePopoverFor !== line.rowId || !$mbody.parent().length) return;
                $mbody.removeClass("vas-grn-dialog__body--loading").addClass("vas-grn-more-grid").empty();
                primeLineContext(line);
                appendDynFields(line, $mbody);
                applyFieldGroups($mbody);
                if (!$mbody.children("[data-col]:not(.vas-grn-dyn-hidden)").length)
                    $mbody.append('<p class="vas-grn-empty-message">' +
                        esc(lbl("VAS_249_NoAdditionalInfo", "No additional info for this line")) + "</p>");
                $mbody.find("[data-col]:not(.vas-grn-dyn-hidden)").find("input,select,textarea").first().focus();
            }, 0);
        }
        ...
        function setUom(line, uomId, uomName) {
            ...
            if (line.values.M_Product_ID > 0 || line.values.C_Charge_ID > 0) runCallout(line, "C_UOM_ID");
        }

        /* Set the line's locator. There is nothing to recompute - a locator does not affect
           quantity or unit - so no callout runs unless the dictionary configures one on the
           column (setDyn's path handles that case for the modal). */
         ...
         * line's values + receipt header + session context. The filtered lists are
        ...
        function rowLocatorList(line) {
            return (line._lk && line._lk.sig === rowLookupSig(line) && line._lk.locator) ? line._lk.locator : locatorList;
        }
        ...
        function ensureRowLookups(line, cb) {
            if (!parent || !parent.M_InOut_ID) { if (cb) cb(); return; }
            ...
            line._lk = { sig: sig, loading: true, loaded: false, uom: null, locator: null, cbs: cb ? [cb] : [] };
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_249_GRNBottomPanel/GetLookupData",
                type: "POST", dataType: "json",
                data: { payload: JSON.stringify({ M_InOut_ID: parent.M_InOut_ID, RowValues: compactCtx(line.values) }) },
                success: function (raw) {
        ...
        /* Tab order: product / charge -> description -> locator -> quantity -> uom ->
           "..." -> save the row. */
        ...
        function focusMoreBtn(line) {
            setTimeout(function () {
                var $b = $linesBody.find('[data-rowid="' + line.rowId + '"] .vas-grn-more-btn');
                if ($b.length) $b.focus();
            }, 0);
        }
        ...
        /* ---------- line operations ---------- */
        function addLine() {
            if (!panelEditable()) { showToast(docMsg("VAS_249_NotEditable", "This {0} cannot take new lines")); return; }
            if (blockedByDirtyHeader()) return;
            var maxLine = 0;
            ...
            var line = {
                ...
                _productType: "",
                values: {
                    M_InOutLine_ID: 0, Line: maxLine + 10, M_Product_ID: 0, C_Charge_ID: 0,
                    M_AttributeSetInstance_ID: 0, M_Locator_ID: 0,
                    C_UOM_ID: 0, QtyEntered: 0, MovementQty: 0, Description: ""
                },
                display: {
                    productValue: "", productName: "", chargeName: "", uomName: "",
                    locatorName: "", attrName: "", hasAttributeSet: false
                }
            };
        ...
        /* A row counts as unsaved only once it names a product or a charge - a blank row
           the user added and abandoned must not keep the Save button lit. */
        ...
        function blockedByDirtyHeader() {
            ...
            showToast(lbl("VAS_249_SaveHeaderFirst", "Please save the header record before adding lines."));
            return true;
        }
        ...
        function resetCatalog(term, inner, line, $inp) {
            ...
            catalog.$pop.on("mousedown", ".vas-grn-catalog-popover__item", function (e) {
                e.preventDefault();
            ...
            catalog.$pop.html('<div class="vas-grn-catalog__hint">' + esc(lbl("VAS_249_Loading", "Loading…")) + "</div>");
        ...
        function loadCatalogPage(inner, line, $inp, isReset) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_249_GRNBottomPanel/SearchCatalog",
                type: "GET", dataType: "json",
                data: {
                    M_InOut_ID: parent.M_InOut_ID, query: catalog.term,
        ...
        function appendCatalogRows(items, startIdx) {
            ...
            if (startIdx === 0 && !items.length) {
                catalog.$pop.html('<div class="vas-grn-catalog__hint">' + esc(lbl("VAS_249_NoMatches", "No matches")) + "</div>");
                positionCatalog();
                return;
            }
        ...
        function catalogRowHtml(it, idx) {
            // Name + kind badge (the key line is intentionally omitted to keep the dropdown
            // rows compact). Full "name (key)" stays in the tooltip. The badge matters here:
            // a receipt line takes a product OR a charge, and the two behave differently
            // once picked, so the row must say which it is before it is chosen.
            var badge = it.Kind === "C" ? "charge" : "product";
            var blabel = it.Kind === "C" ? lbl("VAS_249_Charge", "Charge") : lbl("VAS_249_Product", "Product");
            return '<button type="button" class="vas-grn-catalog-popover__item" data-catalog-item="true" data-idx="' + idx +
                '" title="' + esc(it.DisplayName + (it.SearchKey ? " (" + it.SearchKey + ")" : "")) + '">' +
                '<span class="vas-grn-catalog-popover__name">' + esc(it.DisplayName) + "</span>" +
                '<span class="vas-grn-badge vas-grn-badge--' + badge + '">' + esc(blabel) + "</span></button>";
        }
        ...
        function setHighlight(idx) {
            ...
            var $items = catalog.$pop.children(".vas-grn-catalog-popover__item");
        ...
        function commitCatalogItem(line, item) {
            ...
            if (item.Kind === "C") {
                // Product and charge are mutually exclusive on the line: picking one clears
                // the other, and a charge carries no attribute set.
                v.C_Charge_ID = item.RecordId; v.M_Product_ID = 0; v.M_AttributeSetInstance_ID = 0;
                d.chargeName = item.DisplayName; d.productName = ""; d.productValue = "";
        ...
        function commitPrimary(line, field, newValue, fromTab) {
            ...
            // row (commitCatalogItem). Text typed into the search box but never selected
            // from the dropdown is neither, so leaving the cell (Tab / blur) discards it and
            // keeps the cell on the committed record (or empty on a new line).
         ...
         * mTab / mField / ctx shims backed by THIS line + the parent receipt context -
         ...
         * unit and the base-unit MovementQty the way MInOutLine.BeforeSave will. */
        var ID_COLS = { M_Product_ID: 1, C_Charge_ID: 1, C_UOM_ID: 1, M_AttributeSetInstance_ID: 1, M_Locator_ID: 1 };
        var PANEL_COLS = {
            M_Product_ID: 1, C_Charge_ID: 1, M_AttributeSetInstance_ID: 1,
            M_Locator_ID: 1, QtyEntered: 1, MovementQty: 1,
            C_UOM_ID: 1, Description: 1, M_InOut_ID: 1
        };
        /* The callout string to run for a column: whatever the dictionary configures on
           ...
           because there is nothing to fall back TO: the standard receipt callouts are the
        ...
        function runCallout(line, column, done) {
            ...
            // A product / charge change always uses the server path: it resolves the unit
            // (the product's own, or the system default for a charge) and the entered->base
            // conversion in one reliable round trip against the same code the receipt
            // window itself saves through.
        ...
        function rowSpinHtml(label) { return '<span class="vas-grn-row-spin" aria-label="' + esc(label || "") + '"></span>'; }
        ...
        function setRowBusy(line, on, label) {
            ...
            $r.find(".vas-grn-row-spin").remove();
            if (on) $r.append(rowSpinHtml(label || lbl("VAS_249_Calculating", "Calculating…")));
        }
        ...
        function fieldGet(line, col) {
            if (col === "M_InOut_ID") return parent.M_InOut_ID;
            if (col === "QtyEntered") {
        ...
        function makeCalloutCtx(line) {
            ...
            function resolve(key) {
                ...
                switch (key) {
                    case "M_InOut_ID": return parent.M_InOut_ID || 0;
                    ...
                    // A vendor receipt is a PURCHASE, not a sale: a callout that branches on
                    // this token must read 'N', which is also the direction the server model
                    // refuses to serve anything else for.
        ...
        /* Server path: states the line's unit and the base-unit MovementQty exactly as
           MInOutLine.BeforeSave will, so the grid never shows a quantity the save would
           then change. Used on product change and whenever no client callout is loaded. */
        function runCalloutServer(line, trigger, done) {
            var v = line.values;
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_249_GRNBottomPanel/RunCallout",
                type: "GET", dataType: "json",
                data: {
                    M_InOut_ID: parent.M_InOut_ID, TriggerColumn: trigger,
        ...
        function requiredMsg(col) { return lbl("VAS_249_FieldRequired", "Required") + ": " + colLabel(col); }
         ...
         * the receipt header / document context. */
        /* Columns the panel always shows read-only, whatever the dictionary says: each is
           ...
           counted the stock, the reversal that undid the receipt — so editing them here
        ...
        var FORCED_READONLY_COLS = {
            ...
            // line a return comes back on, and the reversal that undid the receipt. The
            // purchase ORDER LINE is not here (17-Sep-2026): it is entered by hand on a
            // receipt raised without Create From, so the dictionary decides it.
        ...
        function isColumnReadOnly(line, col) {
            ...
            // on, so it is a delete-and-re-enter, not an edit.
            if (col === "C_UOM_ID" && line && line.values) {
                if ((line.values.C_Charge_ID || 0) > 0) return true;
                return (line.values.M_InOutLine_ID || 0) > 0;
            }
        ...
        var FIELD_COL = {
            ...
            locator: "M_Locator_ID", product: "M_Product_ID", charge: "C_Charge_ID"
        };
        function fieldReadOnly(line, field) {
            var col = FIELD_COL[field];
            return col ? isColumnReadOnly(line, col) : false;
        }
        /* Dynamic mandatory evaluation for Additional-Info modal fields. M_InOutLine has
        ...
        /* Resolve a logic token. Login/session tokens (@$Element_OT@, @#Global@) come from
           ...
           else from the current line then the receipt header. A not-found login token
        ...
        function logicCtxVal(line, key) {
            ...
            switch (key) {
                case "M_InOut_ID": return String((parent && parent.M_InOut_ID) || 0);
                ...
                // A vendor receipt is bought, not sold - a field whose logic gates on
                // the sales flag must read 'N' rather than an unresolved token.
                case "IsSOTrx": return "N";
                case "Processed": return (parent && parent.Processed) ? "Y" : "N";
                case "DocStatus": return (parent && parent.DocStatus) || "";
                default: break;
            }
            // Header columns the receipt line's DisplayLogic names as tokens. The server
            // omits a column that is NULL on the receipt, so an absent key resolves to "" -
         ...
         * Renders the M_InOutLine tab fields that are NOT one of the fixed panel
        ...
        var ADDITIONAL_INFO_FIELDS = [
            ...
            { col: "C_Campaign_ID" },
            { col: "C_Activity_ID" },
            // --- Receipt group: how the goods are being sent ---
            // Target Quantity and Description Only were dropped on 17-Sep-2026.
            { col: "M_Shipper_ID" },
            // --- References group ---
            // The purchase-order line this receipt fulfils (entered by hand on a receipt
            // raised without Create From, so editable), the RMA line a return comes back
            // on and the reversal that undid the receipt (both stamped by the process that
            // produced them, see FORCED_READONLY_COLS). Confirmed / Picked / Scrapped
            // Quantity and Invoiced were dropped on 17-Sep-2026.
            { col: "C_OrderLine_ID" },
            { col: "M_RMALine_ID" },
            // Drop Shipment: only on a receipt whose HEADER is itself flagged
            // M_InOut.IsDropShip. On any other receipt the line flag is meaningless, so
            // the field is not offered at all.
            { col: "IsDropShip", when: "dropShip" },
            { col: "ReversalLine_ID" },
            // The work-order this receipt was raised for (VAMFG module); skipped silently
            // wherever that module is not installed.
            { col: "VAMFG_M_WorkOrder_ID", when: "vamfg_" }
        ];

        /* The RECEIPT HEADER is flagged as a drop shipment (M_InOut.IsDropShip = 'Y'). */
        ...
        function dynCondMet(line, when) {
            ...
            // Receipt whose header carries the drop-shipment flag.
        ...
        var MORE_FIELD_GROUPS = [
            ...
            { anchor: "AD_OrgTrx_ID", key: "VAS_249_GrpDimensions", def: "Dimensions", collapsed: false },
            { anchor: "M_Shipper_ID", key: "VAS_249_GrpReceipt", def: "Receipt", collapsed: false },
            { anchor: "C_OrderLine_ID", key: "VAS_249_GrpReferences", def: "References", collapsed: false }
        ];
        ...
        function applyFieldGroups($mbody) {
            ...
            $mbody.children(".vas-grn-fldgrp").remove();
            for (var g = 0; g < MORE_FIELD_GROUPS.length; g++) {
                ...
                $anchor.before(
                    '<div class="vas-grn-fldgrp" data-grp="' + grp.anchor + '"' + (collapsed ? ' data-collapsed="1"' : "") + '>' +
                    '<span class="vas-grn-fldgrp-name">' + esc(lbl(grp.key, grp.def)) + "</span>" +
                    '<button type="button" class="vas-grn-fldgrp-toggle" data-act="fldgrp-toggle">' +
                    '<span class="vas-grn-fldgrp-txt">' + esc(collapsed ? lbl("VAS_249_ShowMore", "Show More") : lbl("VAS_249_ShowLess", "Show Less")) + "</span>" +
                    '<svg class="vas-grn-fldgrp-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 15 12 9 18 15"/></svg>' +
                    "</button></div>");
            }
        ...
        function applyGroupCollapse($mbody) {
            $mbody.children(".vas-grn-fldgrp").each(function () {
                var $hdr = $(this);
                $hdr.nextUntil(".vas-grn-fldgrp").toggleClass("vas-grn-grp-collapsed", $hdr.attr("data-collapsed") === "1");
            });
        }
        ...
        function syncGroupHeaders($mbody) {
            ...
            $mbody.children(".vas-grn-fldgrp").each(function () {
                var $hdr = $(this);
                var live = $hdr.nextUntil(".vas-grn-fldgrp", "[data-col]").not(".vas-grn-dyn-hidden").length;
                $hdr.toggleClass("vas-grn-fldgrp-empty", !live);
            });
        }
        ...
        function toggleFieldGroup($hdr) {
            ...
            $hdr.find(".vas-grn-fldgrp-txt").text(collapsed ? lbl("VAS_249_ShowMore", "Show More") : lbl("VAS_249_ShowLess", "Show Less"));
            applyGroupCollapse($hdr.closest(".vas-grn-more-grid"));
        }
        ...
        /* Apply DisplayLogic to the ALREADY-BUILT modal fields: toggle each field's
           visibility (display:none via .vas-grn-dyn-hidden) instead of adding/removing DOM,
        ...
        function applyDynDisplay(line, $mbody) {
            ...
            $mbody.children("[data-col]").each(function () {
                ...
                $(this).toggleClass("vas-grn-dyn-hidden", !show);
                if (show) visible++;
            });
        ...
        function buildDynField(line, m) {
            ...
            $field.toggleClass("vas-grn-dyn-mandatory", dynMandatory(line, m));
        ...
        /* Prime the framework window context (VIS.context @ this panel's windowNo) with the
           receipt HEADER + the CURRENT line's column values, so a modal FK control's
           ...
           Duplicate-key safety: line.values contains ALL M_InOutLine columns. On
           ...
           while the Receipt window's framework context already holds the Pascal-cased
        ...
        function primeLineContext(line) {
            ...
            if (parent) {
                stage("M_InOut_ID", parent.M_InOut_ID || 0);
                ...
                // A vendor receipt is a purchase, not a sale; a val rule gating on the sales
            ...
            // columns are skipped to avoid casing collisions with the Receipt window
            ...
            // Re-assert the header org so @AD_Org_ID@ always resolves to the receipt's org.
        ...
        /* True when the line's product actually carries an attribute set
           (M_AttributeSet_ID > 0). Reads the raw server flag VASGRNDISP_HasAttrSet off the
        ...
        function productHasAttributeSet(line) {
            var raw = lineVal(line, "VASGRNDISP_HasAttrSet");
        ...
        function buildFallbackControl(line, m, kind, ro) {
            ...
            if (kind === "list") {
                var $l = $('<select></select>');
                $l.append($('<option value=""></option>').text(lbl("VAS_249_SelectOption", "Select")));
        ...
        function refreshMoreDialog(line) {
            ...
            $b.children(".vas-grn-empty-message").remove();
            if (!visible)
                $b.append('<p class="vas-grn-empty-message">' + esc(lbl("VAS_249_NoAdditionalInfo", "No additional info for this line")) + "</p>");
        }
        ...
        function ensureRefLabel(line, m, $input) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_249_GRNBottomPanel/GetRefLookup",
                type: "POST", dataType: "json",
                data: { payload: JSON.stringify({ M_InOut_ID: parent.M_InOut_ID, ColumnName: col, Id: +v }) },
                success: function (raw) {
        ...
        function buildFkControl(line, m) {
            ...
            var $wrap = $('<div class="vas-grn-fk" style="position:relative"></div>');
            var $i = $('<input type="text" class="vas-grn-field__input" autocomplete="off" />').val(dynDisplay(line, m));
            $wrap.append($i);
        ...
        function fkSearch(line, m, $wrap, $i, term, fk) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_249_GRNBottomPanel/GetRefLookup",
                type: "POST", dataType: "json",
                data: { payload: JSON.stringify({ M_InOut_ID: parent.M_InOut_ID, ColumnName: m.ColumnName, Query: term, PageSize: CATALOG_PAGE_SIZE, Offset: 0, RowValues: compactCtx(line.values) }) },
                success: function (raw) {
        ...
        function showFkPopover($wrap, $i, line, m, rows, fk) {
            ...
            fk.$pop = $('<div class="vas-grn-catalog-popover vas-grn-fk-popover"></div>');
            if (!rows.length) {
                fk.$pop.html('<div class="vas-grn-catalog__hint">' + esc(lbl("VAS_249_NoMatches", "No matches")) + "</div>");
            } else {
                for (var i = 0; i < rows.length; i++) {
                    fk.$pop.append($('<button type="button" class="vas-grn-catalog-popover__item" data-fk-item="true"></button>')
                        .text(rows[i].Name).attr("data-id", rows[i].Id).attr("data-name", rows[i].Name));
                }
            }
            fk.$pop.on("mousedown", ".vas-grn-catalog-popover__item", function (e) {
                e.preventDefault();
        ...
        function productRequiredMsg() {
            return allowNonItem()
                ? lbl("VAS_249_ProductChargeRequired", "Please select a product or charge")
                : lbl("VAS_249_ProductRequired", "Please select a product");
        }
        ...
        function validateLine(line) {
            ...
            if (!(v.QtyEntered > 0))
                return { col: "QtyEntered", msg: lbl("VAS_249_QtyRequired", "Quantity must be greater than zero") };

            // The locator is structural on a PRODUCT line: a receipt that does not say
            // which bin the goods land in cannot be put away, and MInOutLine would reject it
            ...
            if (v.M_Product_ID > 0 && !(v.M_Locator_ID > 0))
                return { col: "M_Locator_ID", msg: lbl("VAS_249_SelectLocator", "Select the locator where the goods will be received") };
            ...
            for (var d = 0; d < dyn.length; d++) {
                (function (dm) {
                    ...
                    dynRules.push({
                        col: dm.ColumnName,
                        msg: lbl("VAS_249_FieldRequired", "Required") + ": " + (dm.Name || dm.ColumnName),
                        check: function (vv) {
        ...
        function validateUnsaved(batch) {
            ...
            if (anyBad) {
                ...
                var $first = $linesBody.find(".vas-grn-row--line.is-invalid").first();
        ...
        function openAttrDialog(line) {
            ...
            // no caller can bypass it. Uses the raw VASGRNDISP_HasAttrSet flag, not the
            ...
            VIS.AttributeControl.open({
                ...
                // A receipt brings goods INTO stock, so the instance is normally being
                // CREATED - the lot or serial number arriving on this delivery does not
                // exist yet - which is why the new-attribute form opens directly. The
                // existing list is still one click away for a re-receipt of stock already
                // known to the system.
                IsSOTrx: false,
                newAttribute: true,
                // Show every instance, including zero / negative on-hand: a receipt is
                ...
                onApply: function (res) {
                    ...
                    // No locator is adopted from the picker here, unlike the delivery panel:
                    // a picker row's locator says where stock ALREADY is, which on a receipt
                    // is the wrong answer - the bin is where the goods are being PUT, and
        ...
        function openScanDialog() {
            ...
            var backdrop = $('<div class="vas-grn-dialog-backdrop" id="vasGrnScan"></div>');
            var dialog = $('<div class="vas-grn-dialog vas-grn-dialog--wide"></div>');
            dialog.html(
                '<header class="vas-grn-dialog__header">' +
                '<div class="vas-grn-dialog__header-row"><div class="vas-grn-dialog__title-block">' + icon("scan-line", "▭") + '<h3 class="vas-grn-dialog__title">' + esc(lbl("VAS_249_QuickAddBarcode", "Quick add by barcode")) + "</h3></div>" +
                '<p class="vas-grn-dialog__hint">' + esc(lbl("VAS_249_ScanToReviewSubmit", "Scan to add · review · submit")) + "</p></div>" +
                '<p class="vas-grn-dialog__sub">' + esc(lbl("VAS_249_ScanHint", "Scan a product barcode; items collect below. Duplicate scans increase quantity.")) + "</p>" +
                '<div class="vas-grn-scan-input"><button type="button" class="vas-grn-scan-input__icon" data-act="sim-scan" title="' + esc(lbl("VAS_249_SimulateScan", "Simulate scan")) + '">' + icon("scan-line", "▭") + "</button>" +
                '<input type="text" id="vasGrnScanInput" placeholder="' + esc(lbl("VAS_249_ListeningScans", "Listening for scans… (type a code + Enter)")) + '" autocomplete="off" /></div>' +
                '<p class="vas-grn-dialog__error vas-grn-is-hidden" id="vasGrnScanError"></p></header>' +
                '<div class="vas-grn-dialog__body vas-grn-dialog__body--fixed">' +
                '<div class="vas-grn-scan-empty" id="vasGrnScanEmpty"><div class="vas-grn-scan-empty__badge">' + icon("scan-line", "▭") + "</div>" +
                '<p class="vas-grn-scan-empty__title">' + esc(lbl("VAS_249_ScanToBegin", "Scan a barcode to begin")) + '</p><p class="vas-grn-scan-empty__hint">' + esc(lbl("VAS_249_ScanExampleCode", "e.g. PRD-BLW-001")) + '</p></div>' +
                '<div class="vas-grn-scan-grid vas-grn-is-hidden" id="vasGrnScanGrid"><div class="vas-grn-scan-grid__head"><div>' + esc(lbl("VAS_249_Code", "Code")) + "</div><div>" + esc(lbl("VAS_249_Product", "Product")) +
                "</div><div>" + esc(lbl("VAS_249_Status", "Status")) + "</div><div>" + esc(lbl("VAS_249_Qty", "Qty")) + '</div><div></div></div><div class="vas-grn-scan-grid__body" id="vasGrnScanRows"></div></div></div>' +
                '<footer class="vas-grn-dialog__footer"><p class="vas-grn-dialog__summary" id="vasGrnScanSummary">' + esc(lbl("VAS_249_NoScans", "No scans yet")) + "</p>" +
                '<div class="vas-grn-dialog__actions"><button type="button" class="vas-grn-btn vas-grn-btn--ghost" data-act="close-scan">' + esc(lbl("VAS_249_Cancel", "Cancel")) +
                '</button><button type="button" class="vas-grn-btn vas-grn-btn--primary" data-act="submit-scan" disabled>' + esc(lbl("VAS_249_AddLines", "Add lines")) + "</button></div></footer>");
            backdrop.append(dialog);
            $("body").append(backdrop);

            backdrop.on("mousedown", function (e) { if (e.target === backdrop[0]) closeDialogs(); });
            var $field = dialog.find("#vasGrnScanInput");
            ...
            dialog.on("click", ".vas-grn-scan-del", function () { var id = $(this).attr("data-id"); scanState.rows = scanState.rows.filter(function (r) { return r.id !== id; }); renderScanRows(); });
        ...
        function handleScan(code) {
            ...
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_249_GRNBottomPanel/ScanLookup",
                type: "GET", dataType: "json", data: { M_InOut_ID: parent.M_InOut_ID, code: code },
                success: function (raw) {
                    ...
                    scanState.error = (it && it.RecordId > 0) ? "" : lbl("VAS_249_NoMatchFor", "No catalog match for") + ' "' + code + '"';
                    renderScanRows();
                },
                error: function (err) { console.log(err); }
            });
        ...
        function renderScanRows() {
            var d = $("#vasGrnScan"); if (!d.length) return;
            var err = d.find("#vasGrnScanError");
            err.toggleClass("vas-grn-is-hidden", !scanState.error).text(scanState.error);
            var empty = d.find("#vasGrnScanEmpty"), grid = d.find("#vasGrnScanGrid");
            if (!scanState.rows.length) { empty.removeClass("vas-grn-is-hidden"); grid.addClass("vas-grn-is-hidden"); }
            else {
                empty.addClass("vas-grn-is-hidden"); grid.removeClass("vas-grn-is-hidden");
                var body = d.find("#vasGrnScanRows"); body.empty();
                scanState.rows.forEach(function (r) {
                    var matched = !!r.item;
                    var badge = matched ? "product" : "danger";
                    var blabel = matched ? lbl("VAS_249_Product", "Product") : lbl("VAS_249_Unknown", "Unknown");
                    var row = $('<div class="vas-grn-scan-grid__row' + (matched ? "" : " is-unknown") + '"></div>');
                    row.html('<span class="vas-grn-scan-grid__code">' + esc(r.code) + '</span><span class="vas-grn-scan-grid__name">' + esc(matched ? r.item.DisplayName : "—") +
                        '</span><span class="vas-grn-badge vas-grn-badge--' + badge + '">' + esc(blabel) + "</span>" +
                        '<div class="vas-grn-qty-stepper"><button type="button" data-qminus="' + r.id + '" ' + (!matched || parseInt(r.qty, 10) <= 1 ? "disabled" : "") + ">−</button>" +
                        ...
                        '<button type="button" class="vas-grn-icon-btn--danger vas-grn-scan-del" data-id="' + r.id + '">' + icon("trash", "🗑") + "</button>");
                    body.append(row);
                });
            }
            ...
            d.find("#vasGrnScanSummary").text(!scanState.rows.length ? lbl("VAS_249_NoScans", "No scans yet")
                : matched.length + " " + lbl("VAS_249_Matched", "matched") + " · " + units + " " + lbl("VAS_249_Units", "unit(s)") + (unknown > 0 ? " · " + unknown + " " + lbl("VAS_249_Unknown", "unknown") : ""));
            d.find("[data-act=submit-scan]").prop("disabled", matched.length === 0).text(lbl("VAS_249_AddLines", "Add lines") + (matched.length ? " (" + matched.length + ")" : ""));
        }
        ...
        function submitScan() {
            ...
            scanState.rows.forEach(function (r) {
                ...
                var isCharge = r.item.Kind === "C";
                var line = {
                    rowId: "r" + (++rowCounter), status: "new", dirty: true,
                    _productType: isCharge ? "" : (r.item.ProductType || ""),
                    values: {
                        M_InOutLine_ID: 0, Line: maxLine,
                        M_Product_ID: isCharge ? 0 : r.item.RecordId,
                        C_Charge_ID: isCharge ? r.item.RecordId : 0,
                        M_AttributeSetInstance_ID: 0, M_Locator_ID: 0,
                        C_UOM_ID: r.item.C_UOM_ID || 0, QtyEntered: qty, MovementQty: 0, Description: ""
                    },
                    display: {
                        productValue: isCharge ? "" : (r.item.SearchKey || ""),
                        productName: isCharge ? "" : r.item.DisplayName,
                        chargeName: isCharge ? r.item.DisplayName : "",
                        uomName: r.item.UomName || "", locatorName: "",
                        attrName: "", hasAttributeSet: !isCharge && !!r.item.HasAttributeSet
                    }
                };
                seedAllColumns(line.values);
            ...
            closeDialogs(); render();
            // Each scanned product line still needs its locator, so it comes in flagged
            // invalid on the first Save rather than being silently unsaveable.
        ...
        function buildRowPayload(l) {
            ...
            return {
                M_InOutLine_ID: v.M_InOutLine_ID || 0, RowKey: l.rowId, Line: v.Line || 0,
                M_Product_ID: v.M_Product_ID || 0, C_Charge_ID: v.C_Charge_ID || 0,
                M_AttributeSetInstance_ID: v.M_AttributeSetInstance_ID || 0,
                M_Locator_ID: v.M_Locator_ID || 0,
        ...
        function saveRows(done, restrictIds) {
            ...
            if (isHeaderDirty()) { showToast(lbl("VAS_249_SaveHeaderFirst", "Please save the header record before adding lines.")); if (done) done(false); return; }
            ...
            batch.forEach(function (l) { l._saving = true; setRowBusy(l, true, lbl("VAS_249_Saving", "Saving…")); });
            renderHeaderButtons();   // the batch no longer counts as "unsaved" -> Save mutes
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_249_GRNBottomPanel/SaveLines",
                type: "POST", dataType: "json",
                data: { payload: JSON.stringify({ M_InOut_ID: parent.M_InOut_ID, AD_Window_ID: $self.AD_Window_ID || 0, Page: linePage, Lines: rows }) },
                success: function (raw) {
                    ...
                    if (res && res.Success) {
                        ...
                        showToast(lbl("VAS_249_LinesSaved", "Lines saved"));
        ...
        function showServerSaveErrors(batch, res) {
            ...
            for (var i = 0; i < errs.length; i++) {
                ...
                if (l) { l._error = errs[i].Message || lbl("VAS_249_SaveFailed", "Save failed"); mapped++; }
            }
            if (!mapped) {
                ...
                var msg = lbl((res && res.ErrorKey) || "VAS_249_SaveFailed", "Save failed") + (detail ? " - " + detail : "");
        ...
        /* Match a server LineSaveError back to its client line: by RowKey (the client
           rowId), then by M_InOutLine_ID (existing lines), then by Line number. */
        function findBatchLine(batch, e) {
            var i;
            if (e && e.RowKey) { for (i = 0; i < batch.length; i++) if (batch[i].rowId === e.RowKey) return batch[i]; }
            if (e && e.M_InOutLine_ID > 0) { for (i = 0; i < batch.length; i++) if ((batch[i].values.M_InOutLine_ID || 0) === e.M_InOutLine_ID) return batch[i]; }
        ...
        function mergeSavedLines(batch, serverRows) {
            ...
            lines.forEach(function (l) {
                var id = l.values.M_InOutLine_ID || 0;
                if (id > 0 && !inBatch(l) && l.dirty) dirtyById[id] = l;
            });
            // Brand-new client lines not part of this batch (added during the save).
            var newKeep = lines.filter(function (l) { return (l.values.M_InOutLine_ID || 0) <= 0 && !inBatch(l); });
            var merged = (serverRows || []).map(function (r) {
                return dirtyById[r.M_InOutLine_ID] || fromServerRow(r);
            });
        ...
        function reloadLinesKeepingUnsaved(serverRows) {
            ...
            var newKeep = lines.filter(function (l) { return (l.values.M_InOutLine_ID || 0) <= 0; });
            var merged = (serverRows || []).map(function (r) {
                return dirtyById[r.M_InOutLine_ID] || fromServerRow(r);
            });
            lines = newKeep.concat(merged);
        ...
        function deleteSelected() {
            ...
            var ids = [], localOnly = [];
            sel.forEach(function (l) { if (l.values.M_InOutLine_ID > 0) ids.push(l.values.M_InOutLine_ID); else localOnly.push(l); });
            ...
            showBusy(true);
            $.ajax({
                url: VIS.Application.contextUrl + "VAS_249_GRNBottomPanel/DeleteLines",
                type: "POST", dataType: "json",
                data: { payload: JSON.stringify({ M_InOut_ID: parent.M_InOut_ID, AD_Window_ID: $self.AD_Window_ID || 0, Page: linePage, LineIds: ids }) },
                success: function (raw) {
                    showBusy(false);
                    var res = (typeof raw === "string") ? jQuery.parseJSON(raw) : raw;
                    if (res && res.Success) {
                        applyLinePaging(res); reloadLinesKeepingUnsaved(res.Lines);
                        showToast(lbl("VAS_249_LinesDeleted", "Lines deleted"));
                        refreshSummary(); refreshHeaderRecord();
                    }
                    else showToast(lbl((res && res.ErrorKey) || "VAS_249_DeleteFailed", "Delete failed"));
                },
                error: function (err) { console.log(err); showBusy(false); showToast(lbl("VAS_249_DeleteFailed", "Delete failed")); }
            });
        }
        ...
        function closeDialogs() { $("#vasGrnAttr, #vasGrnScan, #vasGrnMore").remove(); attrState = null; scanState = null; morePopoverFor = null; }
        ...
        $(document).on("keydown.vasgrn", function (e) {
        ...
        function shortcutHandlers() { return {
            /**
             * Panel is active when it is visible in the DOM and a receipt is loaded.
            ...
            hasBlockingDialog: function () {
                return !!(attrState || scanState || morePopoverFor ||
                    document.getElementById("vasGrnAttr") ||
                    document.getElementById("vasGrnScan"));
            },
            ...
            onDelete: function () {
                if (!selectedCount()) {
                    showToast(lbl("VAS_249_SelectRowToDelete", "Select a row to delete"));
                    return;
                }
                deleteSelected();
            },
        ...
        this.unpark = function () {
            if (!$self._parked) return;
            $self._parked = false;
            if ($root) $root.removeClass("vas-grn-is-hidden");
            registerShortcuts();
        ...
        this.dispose_ = function () {
            ...
            $(document).off("mousedown.vasgrn").off("keydown.vasgrn");
            $(window).off("resize.vasgrn");
            closeCatalog();
            closeDialogs();
        };
    ...
    VAS.VAS_249_GRNBottomPanel.prototype.startPanel = function (windowNo, curTab) {
        ...
        this.init();
        if (this._parked) {
            this.getRoot().addClass("vas-grn-is-hidden");
    ...
    VAS.VAS_249_GRNBottomPanel.prototype.refreshPanelData = function (recordID, selectedRow) {
        ...
        if (selectedRow == undefined || recordID <= 0) {
            // Pass true when a row exists in the grid but the receipt has no DB ID yet
    ...
    VAS.VAS_249_GRNBottomPanel.prototype.sizeChanged = function (height, width) {
        this.panelHeight = height;
        this.panelWidth = width;
        if (typeof this.fitHostWidth === "function") this.fitHostWidth();
    };

    VAS.VAS_249_GRNBottomPanel.prototype.dispose = function () {
        this._disposed = true;
