...
; (function (VAS, $) {
    ...
    VAS.VAS_106_OverviewSalesOrder = function () {
        ...
        // Open a record window using whatever helper the host exposes; degrade
        // to a toast so a click never throws.
        function openRecord(tableName, recordId) {
            if (!tableName || !recordId || +recordId <= 0) return;
            try {
                if (VIS && VIS.Application && typeof VIS.Application.OpenWindow === "function") {
                    VIS.Application.OpenWindow(tableName, +recordId); return;
                }
                if (VIS && typeof VIS.zoomAcross === "function") { VIS.zoomAcross(tableName, +recordId); return; }
            } catch (e) { }
