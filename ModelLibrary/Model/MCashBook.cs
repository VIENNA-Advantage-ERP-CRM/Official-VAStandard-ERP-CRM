    ...
    public class MCashBook : X_C_CashBook
    {
       ...
       */
        protected override Boolean BeforeSave(Boolean newRecord)
        {
            ...
            {
                //VIS_0045: 12Oct2023, Task ID - 2583, Handle Alias issue for postgreSQL DB
                string sql = "SELECT SUM(t.total) AS TOTAL FROM  (SELECT COUNT(*) AS TOTAL FROM C_CashBook_Acct WHERE AD_Client_ID= " + GetAD_Client_ID() + " AND C_CashBook_ID =" + GetC_CashBook_ID()
                               ...
                               + " UNION ALL  SELECT COUNT(*) as TOTAL from C_CASHBOOKLINE WHERE AD_Client_ID= " + GetAD_Client_ID() + " AND C_CASHBOOK_ID =" + GetC_CashBook_ID() + ") t";
