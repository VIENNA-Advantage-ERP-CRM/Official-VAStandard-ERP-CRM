...
namespace VAdvantage.Model
{
    public class MCostQueue : X_M_CostQueue
    {
        ...
        /// <summary>
        /// This function is used to reset current cost price on product costs from Costs Queue based on LIFO/FIFO when stock Out / IN
        /// Get Cost Queue Records in Lifo/Fifo order
        /// </summary>
        /// <param name="product">product</param>
        /// <param name="M_ASI_ID">costing level ASI</param>
        ...
        public static Decimal GetLifoAndFifoCurrentCostFromCostQueue(int AD_Client_ID, int Org_ID, int M_Product, int M_ASI_ID, MAcctSchema mas,
             int M_Warehouse_ID, bool isFifo, Trx trxName, String costingLevel)
        {
                     ...
                     string optionalstr = "process")
        {
            ...
            //if (windowName == "Material Receipt" || windowName == "Invoice(Vendor)")
            //{
            //    IsPOCostingMethod = costingCheck != null && costingCheck.IsPOCostingethodBindedonProduct != null ? costingCheck.IsPOCostingethodBindedonProduct.Value :
            //        (product != null ? MCostElement.IsPOCostingmethod(ctx, AD_Client_ID, product.GetM_Product_ID(), trxName) : false);
            //}
            ...
            try
            {
                if (product != null)
                {
                    ...
                    costingCheck.GetLifoAndFIFoID(AD_Client_ID);

                    // Set AttributeSetinstance_ID 
                    if (costingCheck.M_ASI_ID == 0 && M_ASI_ID != 0)
                    {
                        costingCheck.M_ASI_ID = M_ASI_ID;
                    }
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (product != null)
                        {
                            ...
                            pca = MProductCategory.Get(ctx, product.GetM_Product_Category_ID());
                            ...
                            else
                            {
                                ...
                                // maintain costing level, Costing method and Costing element and Policy
                                if (costingCheck != null)
                                {
                                    costingCheck.costinglevel = costLevel;
                                    costingCheck.MMPolicy = pca.GetMMPolicy();
                                    ...
                                    if (!String.IsNullOrEmpty(costingCheck.costingMethod) && costingCheck.costingMethod.Equals("C"))
                                    {
                                        ...
                                        costingCheck.definedCostingElement = costingCheck.costingElement;

                                        // Get Linked Material Costing Method and Costing Element ID
                                        costingCheck.GetMaterialCostingMethodFroCombinaton(costingCheck.definedCostingElement, AD_Client_ID);
                                    }
                                    else
                                    {
                                        // When costing method binded on product category or accouting schema 
                                        if (costingCheck != null && costingCheck.dsCostElement != null)
                                        {
                                            DataTable dt = costingCheck.dsCostElement.Tables[0];
                                            if (dt.Rows.Count > 0)
                                            {
                                                dt.CaseSensitive = true;
                                                costingCheck.definedCostingElement = Util.GetValueOfInt(dt.Select("CostingMethod="
                                                                + GlobalVariable.TO_STRING(costingCheck.costingMethod))[0]["M_CostElement_ID"]);
                                            }
                                            else
                                            {
                                                costingCheck.errorMessage = "Product Cost Element not found for FIFO and LIFO";
                                            }
                                        }

                                        // Linked Material Costing Method and Costing Element ID
                                        costingCheck.materialCostingMethod = costingCheck.costingMethod;
                                        costingCheck.materialCostingElement = costingCheck.definedCostingElement;
                                    }
                                }

                                if (windowName != "Inventory Move")
                                {
                                    if (costingCheck.costinglevel.Equals(MAcctSchema.COSTINGLEVEL_Client) ||
                                        costingCheck.costinglevel.Equals(MAcctSchema.COSTINGLEVEL_BatchLot))
                                    {
                                        AD_Org_ID = 0;
                                    }
                                }
                        ...
                        if ((windowName.Equals("Material Receipt") || windowName.Equals("Customer Return") || windowName.Equals("Shipment") || windowName.Equals("Return To Vendor") ||
                            windowName.Equals("Inventory Move") || windowName.Equals("In")) && Qty == 0)
                        {
                        ...
                        {
                            costingCheck.IsQunatityValidated = true;
                        }
                        else
                        {
                            costingCheck.IsQunatityValidated = false;
                        }

                        // Quantity will be checked for LIFO, FIFO and Freight
                        if (costingCheck.IsQunatityValidated)
                        {                         // need to check qty available on product cost (Cost Queue tab) for reduction
                                 ...
                                 || windowName.Equals("Out")))
                            {
                                ...
                                else if (windowName == "Out" && po != null)
                                {
                                    costingCheck.movementDate = costingCheck.movementDate != null ? costingCheck.movementDate : Util.GetValueOfDateTime(po.Get_Value("DateAcct"));
                                }
                                ...
                                else if (windowName.Equals("Out") && costingCheck != null && costingCheck.po != null && loc == null)
                                {
                                    loc = MLocator.Get(ctx, Util.GetValueOfInt(costingCheck.po.Get_Value("M_Locator_ID")));
                                }
                                else
                                {
                                    if (loc == null)
                                ...
                                // Set Org & Warehouse on CostingCheck
                                costingCheck.AD_Org_ID = loc.GetAD_Org_ID();
                                costingCheck.M_Warehouse_ID = loc.GetM_Warehouse_ID();

                                // Check stock available on cost Queue or not
                                decimal productCostsQty = MCostQueue.CheckQtyAvailablity(ctx, acctSchema, AD_Client_ID,
                                    loc.GetAD_Org_ID(), product, M_ASI_ID, loc.GetM_Warehouse_ID(), costingCheck);

                                // when stock not available
                                if (productCostsQty < Decimal.Negate(Qty))
                                {
                                    _log.Info("CostingEngine: Qty not available on Product Costs for window = " + windowName +
                                        " Product Cost Qty = " + productCostsQty +
                            ...
                            else if (windowName == "Inventory Move" && AD_Org_ID > 0)
                            {
                                ...
                                // we have to take the impact on Cost Queue, when warehouse is different
                                ...
                                {
                                    _log.Info("CostingEngine: Qty not available on Product Cost for window = " + windowName +
                                        " Product Cost Qty = " + productCostsQty +
                                        " Transaction Qty = " + Math.Abs(Qty));
                                    costingCheck.errorMessage += "Qty not available on Product Cost, Product Cost Qty = " + productCostsQty +
                                                                 " Transaction Qty = " + Math.Abs(Qty);
                        ...
                        // Get and Convert Price, maitain orderline, inout, inoutline reference on costing check 
                        if (windowName == "Material Receipt" || windowName == "Customer Return" || windowName == "Shipment" || windowName == "Return To Vendor")
                        {
                            ...
                            // Get Movement Date and is document is reversed or not
                            ...
                            else
                            {
                                ...
                                // In case of return to vendor, make the price negative
                        ...
                        // Get and Convert Price, maitain invoice reference on costing check 
                        else if (windowName == "Invoice(Vendor)" || windowName == "Invoice(Customer)" || windowName == "Match IV" || windowName == "Invoice(Vendor)-Return")
                        {
                            ...
                            if (invoiceline.GetM_InOutLine_ID() > 0)
                            {
                                if (costingCheck != null && costingCheck.inout != null)
                                {
                                    M_Warehouse_Id = costingCheck.inout.GetM_Warehouse_ID();
                                    costingCheck.movementDate = costingCheck.inout.GetMovementDate();
                                }
                                if (M_Warehouse_Id <= 0)
                                {
                                    dsInOut = DB.ExecuteDataset(@"SELECT M_Warehouse_ID , MovementDate FROM M_InOut WHERE M_InOut_ID = 
                                    ( SELECT M_InOut_ID FROM M_InOutLine WHERE M_InOutLine_ID = " + invoiceline.GetM_InOutLine_ID() + " )", null, trxName);
                                    if (dsInOut != null && dsInOut.Tables.Count > 0 && dsInOut.Tables[0].Rows.Count > 0)
                                    {
                                        M_Warehouse_Id = Util.GetValueOfInt(dsInOut.Tables[0].Rows[0]["m_warehouse_id"]);
                                        costingCheck.movementDate = Util.GetValueOfDateTime(dsInOut.Tables[0].Rows[0]["MovementDate"]);
                                    }
                                }
                            ...
                            if (invoice.GetC_Currency_ID() != acctSchema.GetC_Currency_ID() && Price != 0)
                            {
                                ...
                                if (Price == 0)
                                {
                                    ...
                                    _log.Info("CostingEngine: Price not available for window = " + windowName +
                                                   " - Document No  = " + conversionNotFound);
                                    costingCheck.errorMessage += " Conversion not avialable";
                                    return false;
                                }
                            }
                        ...
                        if (windowName == "Invoice(Vendor)")
                        {
                            ...
                            if (invoiceline.GetC_OrderLine_ID() <= 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (invoiceline.GetM_InOutLine_ID() > 0 && (inoutline == null || inoutline.Get_ID() <= 0 || inoutline.Get_ID() != invoiceline.GetM_InOutLine_ID()))
                                    {
                                        inoutline = new MInOutLine(ctx, invoiceline.GetM_InOutLine_ID(), trxName);
                                        if (costingCheck != null && costingCheck.inoutline == null)
                                        {
                        ...
                        if (windowName == "Match PO")
                        {
                            ...
                            if (client.IsCostImmediate())
                            {
                                ...
                                if (costingMethodMatchPO == MCostElement.COSTINGMETHOD_AveragePO || costingMethodMatchPO == MCostElement.COSTINGMETHOD_LastPOPrice ||
                                ...
                                else if (1 != 1)
                                {
                                    ...
                                    if (!cd.Save(trxName))
                                    {
                                        ...
                                        _log.Severe("Error occured during saving a cost detail record, M_InoutLine_Id = " + inoutline.GetM_InOutLine_ID() + " in Cost-Detail -> MatchPO. Error Value :  " + pp.GetValue() + " AND Error Name : " + pp.GetName());
                                        return false;
                                    }
                                    continue;
                                }
                            ...
                            if (client.IsCostImmediate())
                            {
                                #region Get MR price from t_temp_CostDetail/Cost Detail  costing calculation on completion
                                MRPriceAvPo = 0;
                                MRPriceLastPO = 0;
                                query.Clear();
                            ...
                            if (cd.GetC_OrderLine_ID() == 0)
                            {
                                ...
                                if ((optionalstr == "window" && costingMethodMatchPO == "A") || optionalstr == "process")
                                {
                                    ...
                                    {
                                        cost = MCost.Get(product, 0, acctSchema, AD_Org_ID, costElementId, 0);
                                    }
                                    else if (cl == MProductCategory.COSTINGLEVEL_BatchLot || cl == MProductCategory.COSTINGLEVEL_OrgPlusBatch)
                                    {
                                        cost = MCost.Get(product, M_ASI_ID, acctSchema, AD_Org_ID, costElementId, 0);
                                    }
                                    else if (cl == MProductCategory.COSTINGLEVEL_Warehouse || cl == MProductCategory.COSTINGLEVEL_WarehousePlusBatch)
                                    {
                                    ...
                                    if (!cost.Save())
                                    {
                                        ...
                                        ValueNamePair pp = VLogger.RetrieveError();
                                        _log.Severe("Error occured during saving a Product costs record , M_inoutLine_ID = " + inoutline.GetM_InOutLine_ID() + " in cost detail -> MatchPO. Error Value :  " + pp.GetValue() + " AND Error Name : " + pp.GetName());
                                        return false;
                                    }

                                    cd.SetC_OrderLine_ID(orderLineId);
                                    cd.SetQty(Qty);
                                    cd.SetAmt(Decimal.Round(Decimal.Multiply(Price, Qty), acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero));
                                    if (!cd.Save(trxName))
                                    {
                                        if (optionalstr != "window")
                                        {
                                    ...
                                    {
                                        cost.SetCurrentCostPrice(Decimal.Round(Decimal.Divide(cost.GetCumulatedAmt(), cost.GetCumulatedQty()), acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero));
                                    }
                                    else
                                    {
                                    ...
                                    if (!cost.Save())
                                    {
                                        ...
                                        ValueNamePair pp = VLogger.RetrieveError();
                                        _log.Severe("Error occured during saving a product costs record, M_InoutLine_ID = " + inoutline.GetM_InOutLine_ID() + " in M-Cost -> MatchPO. Error Value :  " + pp.GetValue() + " AND Error Name : " + pp.GetName());
                                        return false;
                                    }
                                    else
                                    {
                        ...
                        if (windowName == "Match IV")
                        {
                            ...
                            int ceFifo = costingCheck.Fifo_ID;
                            int ceLifo = costingCheck.Lifo_ID;
                            ...
                            if (dsCE != null && dsCE.Tables.Count > 0 && dsCE.Tables[0].Rows.Count > 0)
                            {
                                cd = new MCostDetail(ctx, dsCE.Tables[0].Rows[0], trxName);
                                costDetailId = cd.GetM_CostDetail_ID();
                                dsCE.Dispose();
                                dsCE = null;
                            }
                            else
                            ...
                            if (MRPriceLifo == 0 && costingCheck.MMPolicy.Equals(MProductCategory.MMPOLICY_LiFo))
                            {
                                if (cl == MProductCategory.COSTINGLEVEL_Client ||
                                    cl == MProductCategory.COSTINGLEVEL_Warehouse ||
                                ...
                                else if (cl == MProductCategory.COSTINGLEVEL_BatchLot ||
                                         cl == MProductCategory.COSTINGLEVEL_OrgPlusBatch ||
                            ...
                            if (MRPriceFifo == 0 && costingCheck.MMPolicy.Equals(MProductCategory.MMPOLICY_FiFo))
                            {
                            ...
                            //query.Clear();
                            //query.Append($@"SELECT  ROUND( Amt/Qty, 10) as currentCostAmount  FROM M_CostDetail ced  
                            //                 WHERE ced.IsActive = 'Y' AND ced.M_Product_ID = { product.GetM_Product_ID() } 
                            //                 AND ced.C_AcctSchema_ID = { acctSchema.GetC_AcctSchema_ID() } AND  NVL(ced.M_AttributeSetInstance_ID , 0) IN (0, { M_ASI_ID }) 
                            //                 AND ced.M_InOutLine_ID =  { inoutline.GetM_InOutLine_ID() } AND NVL(ced.C_OrderLIne_ID , 0) IN ( 0 , {MatchPO_OrderLineId})
                            //                 AND NVL(ced.C_InvoiceLine_ID , 0) = 0 AND ced.AD_Client_ID = { inoutline.GetAD_Client_ID()} 
                            //                 ORDER BY ced.M_AttributeSetInstance_ID ASC, ced.C_OrderLIne_ID DESC");
                            //MRPriceLifo = Util.GetValueOfDecimal(DB.ExecuteScalar(query.ToString()));
                            #endregion
                                ...
                                else
                                {
                                    ...
                                    if (ds2 != null && ds2.Tables.Count > 0 && ds2.Tables[0].Rows.Count > 0)
                                    {
                                        ...
                                        for (int k = 0; k < ds2.Tables[0].Rows.Count; k++)
                                        {
                                            ...
                                            //24-Aug-2016
                                            Price = Decimal.Round(Decimal.Add(Price, Decimal.Divide(Decimal.Multiply(Price, costElement.GetSurchargePercentage()), 100)), acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero);
                                            //end
                                            Price = Decimal.Add(Price, Decimal.Multiply(MRPrice, tempQty));
                                            //end
                                            query.Clear();
                                            query.Append("UPDATE m_costqueue SET CurrentCostPrice = " + Decimal.Round((Price / Qty), acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero) + " WHERE M_CostQueue_ID = " + Util.GetValueOfInt(ds2.Tables[0].Rows[k]["M_CostQueue_ID"]));
                            ...
                            else
                            {
                                ...
                                query.Append($@"SELECT cd.M_CostQueue_ID, cd.Amt, cq.ActualQty, cq.CurrentQty, cq.M_CostElement_ID, ce.CostingMethod
                                                    ...
                                                    WHERE cd.M_InOutLine_ID = { inoutline.GetM_InOutLine_ID() }
                                                    AND cd.C_AcctSchema_ID = { acctSchema.GetC_AcctSchema_ID()}");
                                DataSet ds2 = DB.ExecuteDataset(query.ToString(), null, null);
                                if (ds2 != null && ds2.Tables.Count > 0 && ds2.Tables[0].Rows.Count > 0)
                                {
                                    decimal amt = Decimal.Divide(Price, Qty);
                                    amt = Decimal.Round(Decimal.Multiply(amt, Util.GetValueOfDecimal(ds2.Tables[0].Rows[0]["CurrentQty"])), acctSchema.GetCostingPrecision());
                                    if (Math.Sign(amt) != Math.Sign(Price))
                                    {
                                        amt = Decimal.Negate(amt);
                                    }
                                    cd = new MCostDetail(acctSchema, invoiceline.GetAD_Org_ID(), product.GetM_Product_ID(), M_ASI_ID,
                                                    0, amt, (Math.Sign(Qty) != Math.Sign(Util.GetValueOfDecimal(ds2.Tables[0].Rows[0]["CurrentQty"]))
                                                    ? Decimal.Negate(Util.GetValueOfDecimal(ds2.Tables[0].Rows[0]["CurrentQty"])) :
                                                    Util.GetValueOfDecimal(ds2.Tables[0].Rows[0]["CurrentQty"])), null, trxName);
                                }
                                else
                                {
                                    cd = new MCostDetail(acctSchema, invoiceline.GetAD_Org_ID(), product.GetM_Product_ID(), M_ASI_ID,
                                    ...
                                    if (handlingWindowName == "Invoice(Vendor)")
                                    {
                                        ...
                                        if (ds2 != null && ds2.Tables.Count > 0 && ds2.Tables[0].Rows.Count > 0)
                                        {
                                            for (int b = 0; b < ds2.Tables[0].Rows.Count; b++)
                                            {
                                                ...
                                                // VIS_045: 20-Mar-2025, Cost Queue Price updated only when Current Qty != 0 on Queue
                        ...
                        if (windowName == "Physical Inventory" || windowName == "Internal Use Inventory" ||
                            windowName == "Inventory Move" || windowName == "AssetDisposal")
                        {
                            #region Get Price
                            int C_Currency_ID = 0;
                            ...
                            {
                                M_Warehouse_Id = costingCheck.M_Warehouse_ID != 0 ? costingCheck.M_Warehouse_ID : movementline.GetParent().GetDTD001_MWarehouseSource_ID();
                                costingCheck.movementDate = costingCheck.movement != null ? costingCheck.movement.GetMovementDate() : movementline.GetParent().GetMovementDate();
                                costingCheck.isReversal = costingCheck.isReversal != null ? costingCheck.isReversal : movementline.GetParent().IsReversal();
                            }
                            else if (windowName == "AssetDisposal")
                            {
                                M_Warehouse_Id = Util.GetValueOfInt(po.Get_Value("M_Warehouse_ID"));
                                costingCheck.movementDate = Util.GetValueOfDateTime(po.Get_Value("DateAcct"));
                                costingCheck.isReversal = Util.GetValueOfInt(po.Get_Value("ReversalDoc_ID")) > 0 ? true : false;
                                costingCheck.M_Warehouse_ID = costingCheck.M_Warehouse_ID == 0 ? M_Warehouse_Id : costingCheck.M_Warehouse_ID;
                                costingCheck.AD_Org_ID = costingCheck.AD_Org_ID == 0 ? po.Get_ValueAsInt("AD_Org_ID") : costingCheck.AD_Org_ID;
                                costingCheck.M_ASI_ID = costingCheck.M_ASI_ID == 0 ? po.Get_ValueAsInt("M_AttributeSetInstance_ID") : costingCheck.M_ASI_ID;
                            }
                            ...
                            if (!((windowName == "Physical Inventory" || windowName == "Internal Use Inventory") &&
                                Util.GetValueOfDecimal(inventoryLine.Get_Value("PriceCost")) != 0))
                            {
                                price = MCost.GetproductCostBasedonAcctSchema(AD_Client_ID, AD_Org_ID, acctSchema.GetC_AcctSchema_ID(), product.GetM_Product_ID(),
                                            M_ASI_ID, trxName, M_Warehouse_Id);
                            }
                            ...
                            if (price == 0 && (windowName == "Physical Inventory"))
                            {
                                ...
                                query.Append(@"(SELECT pl.AD_Org_ID , pl.name , pl.m_pricelist_id  , plv.m_pricelist_version_id , pp.pricestd , pl.c_currency_id
                                           ...
                                           WHERE pp.AD_Org_ID IN (0, " + inventoryLine.GetAD_Org_ID() + @") AND pp.pricestd > 0 
                            ...
                            if (IsFreeProductNotVerified(isFreeProduct, Price, Price, costingCheck))
                            {
                                ...
                                {
                                    movement = movementline.GetParent();
                                ...
                                else if (windowName.Equals("Out") && po != null)
                                {
                                    conversionNotFound = Util.GetValueOfString(po.Get_Value("DocumentNo"));
                                }
                                _log.Info("CostingEngine: Price not available for window = " + windowName +
                            ...
                            // Currency Conversion, when price picked from Product price list
                        ...
                        if (windowName == "Production Execution" || windowName.Equals("PE-FinishGood"))
                        {
                            ...
                            if (costingCheck.movementDate != null && costingCheck.M_Warehouse_ID > 0)
                            {
                                M_Warehouse_Id = costingCheck.M_Warehouse_ID;
                                VAMFG_DateAcct = costingCheck.movementDate;
                            }
                            else
                            {
                                DataSet dsProductDetail = DB.ExecuteDataset(@"SELECT VAMFG_DateAcct , M_Warehouse_ID, ReversalDoc_ID FROM VAMFG_M_WrkOdrTransaction   
                                ...
                                if (dsProductDetail != null && dsProductDetail.Tables.Count > 0 && dsProductDetail.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    costingCheck.movementDate = VAMFG_DateAcct;
                                    costingCheck.isReversal = Util.GetValueOfInt(dsProductDetail.Tables[0].Rows[0]["ReversalDoc_ID"]) > 0 ? true : false;
                                }
                            }
                        ...
                        if ((windowName.Equals("Out") || windowName.Equals("In")) && po != null)
                        {
                            costingCheck.movementDate = costingCheck.movementDate == null ? Util.GetValueOfDateTime(po.Get_Value("DateAcct")) : costingCheck.movementDate;
                            costingCheck.isReversal = costingCheck.isReversal != null ? costingCheck.isReversal : (Util.GetValueOfInt(po.Get_Value("ReversalDoc_ID")) > 0 ? true : false);
                            costingCheck.M_Warehouse_ID = costingCheck.M_Warehouse_ID == 0 ? M_Warehouse_Id : costingCheck.M_Warehouse_ID;
                            costingCheck.AD_Org_ID = costingCheck.AD_Org_ID == 0 ? po.Get_ValueAsInt("AD_Org_ID") : costingCheck.AD_Org_ID;
                            costingCheck.M_ASI_ID = costingCheck.M_ASI_ID == 0 ? costingCheck.po.Get_ValueAsInt("M_AttributeSetInstance_ID") : costingCheck.M_ASI_ID;

                            if (windowName.Equals("In"))
                            {
                                price = Price;
                                if (acctSchema.GetC_Currency_ID() != Util.GetValueOfInt(ctx.GetContextAsInt("$C_Currency_ID")))
                                {
                                    price = MConversionRate.ConvertCostingPrecision(ctx, price, Util.GetValueOfInt(ctx.GetContext("$C_Currency_ID")),
                                            acctSchema.GetC_Currency_ID(), costingCheck.movementDate, 0, AD_Client_ID, AD_Org_ID2);
                                }
                                Price = price;
                                cmPrice = price;
                            }
                            ...
                            if (IsFreeProductNotVerified(isFreeProduct, Price, Price, costingCheck))
                            {
                                ...
                                _log.Info("CostingEngine: Price not available for window = " + windowName +
                                                " - TableName  = " + po.GetTableName() + ", ID = " + po.Get_ID());
                                costingCheck.errorMessage += "Price not available";
                                return false;
                            }
                        }
                        ...
                        if (windowName.Equals("ProvisionalInvoice"))
                        {
                            ...
                            // get object
                            if (costingCheck.provisionalInvoice == null)
                            {
                                DataSet dsProductDetail = DB.ExecuteDataset(@"SELECT * FROM C_ProvisionalInvoice  
                                 WHERE C_ProvisionalInvoice.C_ProvisionalInvoice_ID = " + po.Get_ValueAsInt("C_ProvisionalInvoice_ID"), null, trxName);
                                if (dsProductDetail != null && dsProductDetail.Tables.Count > 0 && dsProductDetail.Tables[0].Rows.Count > 0)
                                {
                                    costingCheck.provisionalInvoice = new MProvisionalInvoice(ctx, dsProductDetail.Tables[0].Rows[0], trxName);
                                }
                            }
                            if (costingCheck.provisionalInvoice == null)
                            {
                                _log.Info("CostingEngine: Provisional Invoice record not found.");
                                return false;
                            }
                            ...
                            if (acctSchema.GetC_Currency_ID() != costingCheck.provisionalInvoice.GetC_Currency_ID())
                            {
                                Price = MConversionRate.ConvertCostingPrecision(ctx, Price,
                                    ...
                                    dateAcct,
                                    costingCheck.provisionalInvoice.GetC_ConversionType_ID(),
                                    AD_Client_ID, AD_Org_ID2);
                            }
                            ...
                            if (Price == 0)
                            {
                                ...
                                conversionNotFound = costingCheck.provisionalInvoice.GetDocumentNo();
                                _log.Info("CostingEngine: Price not available for window = " + windowName +
                                                   " - Document No  = " + conversionNotFound);
                                return false;
                            }
                        ...
                        if (windowName == "Inventory Move")
                        {
                            ...
                            {
                                query.Clear();
                                query.Append(@"SELECT * FROM M_CostDetail WHERE IsActive = 'Y' 
                                                 AND M_MovementLine_ID = " + movementline.GetM_MovementLine_ID() +
                                              @" AND AD_Org_ID = " + AD_Org_ID);
                                if (costLevel == MProductCategory.COSTINGLEVEL_Warehouse ||
                                    costLevel == MProductCategory.COSTINGLEVEL_WarehousePlusBatch)
                                {
                                    query.Append(" AND M_Warehouse_ID = " + M_Warehouse_Id);
                                }
                                DataSet dsCostDetail = DB.ExecuteDataset(query.ToString(), null, trxName);
                                if (dsCostDetail != null && dsCostDetail.Tables.Count > 0 && dsCostDetail.Tables[0].Rows.Count > 0)
                                {
                                    cdSourceWarehouse = new MCostDetail(ctx, dsCostDetail.Tables[0].Rows[0], trxName);
                                    cdId = cdSourceWarehouse.GetM_CostDetail_ID();
                                }
                            }
                            ...
                            // maintain warehouse reference of Source Warehouse 
                            SourceM_Warehouse_Id = M_Warehouse_Id;

                            //  maintain reference for to Warehouse and to Organization
                        ...
                        if (((windowName == "Invoice(Vendor)" && product == null) ||
                            (windowName == "Invoice(Vendor)" && product != null && product.GetProductType() == "E") ||
                        ...
                        else if (windowName != "Match PO" && windowName != "Match IV")
                        {
                            ...
                            if (optionalstr == "process")
                            {
                                    ...
                                    windowName == "Invoice(Vendor)-Return")
                                {
                                    ...
                                    if (invoiceline.GetM_InOutLine_ID() > 0)
                                    {
                                        if ((windowName == "Invoice(Vendor)" || windowName == "Invoice(Vendor)-Return") &&
                                            matchInoutLine != null && matchInoutLine.GetM_InOutLine_ID() > 0)
                                        {
                                    ...
                                    {
                                        query.Append(" AND M_InoutLine_ID = " + Util.GetValueOfInt(DB.ExecuteScalar(@"SELECT m_inoutline_id 
                                        FROM m_inoutline WHERE isactive = 'Y' AND c_orderline_id = " + invoiceline.GetC_OrderLine_ID(), null, null)));
                                    }
                                }
                            ...
                            if (cd == null || cd.GetM_CostDetail_ID() <= 0)
                            {
                                if (cmPrice != 0)
                                {
                                    cd = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                                          inventoryLine, inoutline, movementline, invoiceline, po, 0, cmPrice, Qty, null, trxName, costingCheck, M_Warehouse_Id);
                                }
                                else
                                {
                                    cd = MCostDetail.CreateCostDetail(acctSchema, AD_Org_ID, product.GetM_Product_ID(), M_ASI_ID, windowName,
                                         inventoryLine, inoutline, movementline, invoiceline, po, 0, Price, Qty, null, trxName, costingCheck, M_Warehouse_Id);
                                }
                            }
                            #endregion
                        }
                        ...
                        if (costingCheck.materialCostingMethod.Equals(MCostElement.COSTINGMETHOD_Fifo) ||
                            costingCheck.materialCostingMethod.Equals(MCostElement.COSTINGMETHOD_Lifo) || optionalstr == "process")
                        {
                                ...
                                ))
                            {
                                ...
                                {
                                    costingElementId = costingCheck.Fifo_ID;
                                }
                                else
                                {
                            ...
                            backwardInOut:
                                if (windowName == "Physical Inventory" || windowName == "Internal Use Inventory")
                                {
                                    ...
                                    if (Qty > 0)
                                    {
                                        ...
                                        {
                                            result = MCostQueue.ReturnStockReAssignedToCostQueue(cd, windowName, costElement, Qty, true, cd.GetM_Warehouse_ID(), costingCheck, out backwardCompatabilitySupport);
                                            if (backwardCompatabilitySupport)
                                            {
                                                result = CreateCostQueue(ctx, acctSchema, product, M_ASI_ID, AD_Client_ID, inventoryLine.GetAD_Org_ID(), Price / Qty, Qty, windowName,
                                                      inventoryLine, inoutline, movementline, invoiceline, cd, trxName, costingCheck, out costQueuseIds);
                                            }
                                        }
                                        else
                                        {
                                        ...
                                        //if (costElement.GetCostingMethod().Equals(MCostElement.COSTINGMETHOD_Fifo))
                                        //{
                                        //    costingElementId = costingCheck.Lifo_ID;
                                        //}
                                        //else
                                    ...
                                    else
                                    {
                                        ...
                                        updateCostQueue(product, M_ASI_ID, acctSchema, inventoryLine.GetAD_Org_ID(),
                                            costElement, Decimal.Negate(Qty), costingCheck, M_Warehouse_Id, cd);
                                ...
                                if (windowName == "Out")
                                {
                                    ...
                                    if (Qty > 0)
                                    {
                                        ...
                                        if (!result)
                                        {
                                            ...
                                            _log.Severe($@"Error occured during CreateCostQueue for {po.GetTableName()}_ID = " + Util.GetValueOfInt(po.Get_ID()));
                                            return false;
                                        }
                                    }
                                    else
                                    {
                                        //1st entry either of FIFO of LIFO 
                                        updateCostQueue(product, M_ASI_ID, acctSchema, Util.GetValueOfInt(po.Get_Value("AD_Org_ID")),
                                            costElement, Decimal.Negate(Qty), costingCheck, M_Warehouse_Id, cd);
                                    }
                                    #endregion
                                }
                                else if (windowName == "In")
                                {
                                    #region Stock IN
                                    if (Qty > 0)
                                    {
                                        ...
                                        result = CreateCostQueue(ctx, acctSchema, product, M_ASI_ID, AD_Client_ID, Util.GetValueOfInt(po.Get_Value("AD_Org_ID")),
                                            Price / Qty, Qty, windowName, inventoryLine, inoutline, movementline, invoiceline, cd, trxName, costingCheck, out costQueuseIds);
                                        if (!result)
                                        {
                                            if (optionalstr != "window")
                                            {
                                            ...
                                            _log.Severe($@"Error occured during CreateCostQueue for {po.GetTableName()} = " + Util.GetValueOfInt(po.Get_ID()));
                                            return false;
                                        }
                                    }
                                    else
                                    ...
                                    backwardSupportPE:
                                        ...
                                        if (po.Get_ValueAsInt("ReversalDoc_ID") > 0 && !backwardCompatabilitySupport)
                                        {
                                            result = MCostQueue.ReturnStockReduceFromCostQueue(cd, windowName, costElement, Decimal.Negate(Qty),
                                                                                      true, cd.GetM_Warehouse_ID(), costingCheck, out backwardCompatabilitySupport);
                                            if (backwardCompatabilitySupport)
                                            {
                                    ...
                                    (inoutline.GetC_OrderLine_ID() == 0 && windowName.Equals("Return To Vendor")))) || backwardCompatabilitySupport)) // windowName == "Customer Return"
                                {
                                    ...
                                    {
                                        //1st entry either of FIFO of LIFO 
                                        updateCostQueue(product, M_ASI_ID, acctSchema, inoutline.GetAD_Org_ID(), costElement,
                                            Decimal.Negate(Qty), costingCheck, M_Warehouse_Id, cd);
                                    ...
                                    (windowName.Equals("Material Receipt") || windowName.Equals("Shipment")))))
                                {
                                    ...
                                    if (Qty > 0)
                                    {
                                        result = MCostQueue.ReturnStockReAssignedToCostQueue(cd, windowName, costElement, Qty,
                                                 inoutline.GetReversalDoc_ID() != 0 ? true : false, cd.GetM_Warehouse_ID(), costingCheck, out backwardCompatabilitySupport);
                                        if (backwardCompatabilitySupport)
                                        {
                                            goto backwardInOut;
                                        }
                                ...
                                else if (windowName == "Inventory Move")
                                {
                                    ...
                                    if (Qty > 0)
                                    {
                                        ...
                                        {
                                            result = MCostQueue.ReturnStockReAssignedToCostQueue(cd, windowName, costElement, Qty, true,
                                                                cd.GetM_Warehouse_ID(), costingCheck, out backwardCompatabilitySupport);
                                            if (backwardCompatabilitySupport)
                                            {
                                                result = CreateCostQueue(ctx, acctSchema, product, M_ASI_ID, AD_Client_ID, AD_Org_ID, Price / Qty,
                                                    Qty, windowName, inventoryLine, inoutline, movementline, invoiceline, cd, trxName,
                                                    costingCheck, out costQueuseIds);
                                        ...
                                        if (movementline.GetReversalDoc_ID() > 0)
                                        {
                                            result = MCostQueue.ReturnStockReduceFromCostQueue(cdSourceWarehouse, windowName, costElement, Qty, true,
                                                SourceM_Warehouse_Id, costingCheck, out backwardCompatabilitySupport);
                                            if (backwardCompatabilitySupport)
                                            {
                                                updateCostQueue(product, M_ASI_ID, acctSchema, movementline.GetAD_Org_ID(), costElement,
                                                    Qty, costingCheck, SourceM_Warehouse_Id, cdSourceWarehouse);
                                            }
                                        }
                                        else
                                        {
                                            updateCostQueue(product, M_ASI_ID, acctSchema, movementline.GetAD_Org_ID(),
                                                costElement, Qty, costingCheck, SourceM_Warehouse_Id, cdSourceWarehouse);
                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        if (movementline.GetReversalDoc_ID() > 0)
                                        {
                                            result = MCostQueue.ReturnStockReAssignedToCostQueue(cdSourceWarehouse, windowName, costElement, Decimal.Negate(Qty), true,
                                                cdSourceWarehouse.GetM_Warehouse_ID(), costingCheck, out backwardCompatabilitySupport);
                                            if (backwardCompatabilitySupport)
                                            {
                                               ...
                                               inventoryLine, inoutline, movementline, invoiceline, cdSourceWarehouse, trxName, costingCheck, out costQueuseIds);
                                            }
                                        }
                                        else
                                        {
                                        ...
                                        {
                                            result = MCostQueue.ReturnStockReduceFromCostQueue(cd, windowName, costElement, Decimal.Negate(Qty),
                                                true, M_Warehouse_Id, costingCheck, out backwardCompatabilitySupport);
                                            if (backwardCompatabilitySupport)
                                            {
                                                updateCostQueue(product, M_ASI_ID, acctSchema, AD_Org_ID, costElement,
                                                    Decimal.Negate(Qty), costingCheck, M_Warehouse_Id, cd);
                                            }
                                        }
                                        else
                                        ...
                                        //    result = MCostQueue.ReturnStockReAssignedToCostQueue(cdSourceWarehouse, windowName, costElement, Decimal.Negate(Qty),
                                        //        true, cdSourceWarehouse.GetM_Warehouse_ID(), costingCheck, out backwardCompatabilitySupport);
                                        //    if (!result)
                                        //    {
                                        //        if (optionalstr != "window")
                                        //        {
                                    ...
                                    if (invoiceline.GetC_OrderLine_ID() > 0 && matchInoutLine != null && matchInoutLine.GetM_InOutLine_ID() > 0)
                                    {
                                        ...
                                        if (ds1 != null && ds1.Tables.Count > 0 && ds1.Tables[0].Rows.Count > 0)
                                        {
                                            for (int k = 0; k < ds1.Tables[0].Rows.Count; k++)
                                            {
                                                ...
                                                //VIS_045: 19-Mar-2025, When Current Qty become ZERO then no need to update price on Cost Queue
                                                if (Util.GetValueOfDecimal(ds1.Tables[0].Rows[k]["CurrentQty"]) != 0)
                                                {
                                                    query.Clear();
                        ...
                        if (cd != null)
                        {
                            ...
                            else if (windowName.Equals("Out") || windowName.Equals("In"))
                            {
                                result = cd.UpdateProductCost(windowName, cd, acctSchema, product, M_ASI_ID, Util.GetValueOfInt(po.Get_Value("AD_Org_ID")), costingCheck, optionalStrCd: optionalstr);
                                if (!result)
                                {
                                    ...
                                    _log.Severe($@"Error occured during UpdateProductCost for {costingCheck.TableName} = "
                                        + Util.GetValueOfInt(po.Get_ID()));
                                    return false;
                                }
                            }
                            else if (windowName == "Production Execution" || windowName.Equals("PE-FinishGood"))
                            ...
                            else if (windowName == "Inventory Move")
                            {
                                    ...
                                    costingCheck.M_Warehouse_ID != costingCheck.M_WarehouseTo_ID)))
                                {
                                    ...
                                    cdSourceWarehouse.CreateCostForCombination(cdSourceWarehouse, acctSchema, product, M_ASI_ID, 0,
                                        windowName, costingCheck, optionalStrcc: optionalstr);
                                }
                            }
                            else if ((windowName == "Invoice(Vendor)" && !isLandedCostAllocation) || windowName == "Invoice(Vendor)-Return")
                            {
                            ...
                            else if (windowName.Equals("ProvisionalInvoice"))
                            {
                                ...
                                result = cd.UpdateProductCost(windowName, cd, acctSchema, product, M_ASI_ID,
                                    Util.GetValueOfInt(po.Get_Value("AD_Org_ID")), costingCheck, optionalStrCd: optionalstr);
                                if (!result)
                                {
                                    if (optionalstr != "window")
                                    {
                                    ...
                                    _log.Severe("Error occured during UpdateProductCost for C_ProvisonalInvoiceLine_ID = " + cd.GetC_ProvisionalInvoiceLine_ID());
                                    return false;
                                }
                            }
                        ...
                        if (windowName == "Match PO")
                        {
                            ...
                            if (cd.GetC_OrderLine_ID() == 0)
                            {
                                ...
                                else
                                {
                                    if (cl != "B")
                                    {
                                        MCostElementDetail.CreateCostElementDetail(ctx, AD_Client_ID, AD_Org_ID, product, 0,
                                                             acctSchema, costElementId, windowName, cd,
                                                             Decimal.Round(Decimal.Multiply(cost.GetCurrentCostPrice(), Qty),
                                                             acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero), Qty);
                                    }
                                    else
                                    {
                                        MCostElementDetail.CreateCostElementDetail(ctx, AD_Client_ID, AD_Org_ID, product, M_ASI_ID,
                                ...
                                else
                                {
                                    ...
                                    {
                                        MCostElementDetail.CreateCostElementDetail(ctx, AD_Client_ID, AD_Org_ID, product, 0,
                                                             acctSchema, costElementId, windowName, cd, Decimal.Round(Decimal.Multiply(cost.GetCurrentCostPrice(), Qty), acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero), Qty);
                                    }
                                    else
                                    {
                                        MCostElementDetail.CreateCostElementDetail(ctx, AD_Client_ID, AD_Org_ID, product, M_ASI_ID,
        ...
        /// <returns>qty</returns>
        public static decimal CheckQtyAvailablity(Ctx ctx, MAcctSchema acctSchema, int AD_Client_ID, int AD_Org_ID, MProduct product,
            int M_ASI_ID, int M_Warehouse_ID, CostingCheck costingCheck)
        {
            decimal qty = 0;
            try
            {
                string sql = @"SELECT SUM(CurrentQty) from m_costQueue where CurrentQty > 0 
                                AND M_Product_ID = " + product.GetM_Product_ID() + @"
                                AND M_AttributeSetInstance_ID = " + (costingCheck.M_ASI_ID != 0 ? costingCheck.M_ASI_ID : M_ASI_ID) + @"
                                AND AD_Org_ID = " + AD_Org_ID + @"
            ...
            try
            {
                ...
                if (dsInvoiceRecord != null && dsInvoiceRecord.Tables.Count > 0 && dsInvoiceRecord.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dsInvoiceRecord.Tables[0].Rows.Count; i++)
                    {
                        ...
                        // This section is used when system is calculating on Completion 
                        // 1 GRN having multiple Invoices 
                        ...
                        if (i == dsInvoiceRecord.Tables[0].Rows.Count - 1)
                        {
                            ...
                            {
                                // this will check invoice line id exist in dataset or not
                                // if not then then only execute this section
                            ...
                            PriceLifoOrFifo = Decimal.Round(PriceLifoOrFifo / Util.GetValueOfDecimal(dsInvoiceRecord.Tables[0].Rows[i]["mrqty"]), acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero);
                        }
                    }
                }
                else
                {
                    ...
                    if (invoice != null && invoice.GetDescription() != null && invoice.GetDescription().Contains("{->"))
                    {
                        ...
                        PriceLifoOrFifo = Decimal.Round((PriceLifoOrFifo / VOinoutline.GetMovementQty()), acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero);
                    }
                    else
                    {
        ...
        /// <summary>
        /// Is used to get Current cost price based on costing method either binded on Product Category or on Accounting schema from Product Costs
        /// </summary>
        /// <param name="ctx">context</param>
        /// <param name="acctSchema">accounting schema</param>
        /// <param name="product">product</param>
        ...
        /// <param name="M_Warehouse_ID">warehouse</param>
        /// <returns>PRICE</returns>
        private static Decimal CalculateCost(Ctx ctx, MAcctSchema acctSchema, MProduct product, int M_ASI_ID, int AD_Client_ID,
        ...
        {
            return CreateCostQueue(ctx, acctSchema, product, M_ASI_ID, AD_Client_ID, AD_Org_ID, Price, Qty,
                            windowName, inventoryLine, inoutline, movementline,
                           invoiceline, cd, trxName, null, out costQueueIds);
        }

        private static bool CreateCostQueue(Ctx ctx, MAcctSchema acctSchema, MProduct product, int M_ASI_ID, int AD_Client_ID, int AD_Org_ID, Decimal Price, Decimal Qty,
                           string windowName, MInventoryLine inventoryLine, MInOutLine inoutline, MMovementLine movementline,
                           MInvoiceLine invoiceline, MCostDetail cd, Trx trxName, CostingCheck costingCheck, out string costQueueIds)
        {
            costQueueIds = null;
            String sql = "";
            decimal priceLifo = 0;
                    ...
                    if (Price == 0 && !isFreeProduct)
                    {
                        ...
                        if (costingCheck.MMPolicy.Equals(MProductCategory.MMPOLICY_FiFo))
                        {
                            ...
                            if (priceFifo <= 0)
                            {
                                if (cl == MProductCategory.COSTINGLEVEL_BatchLot)
                                {
                                    priceFifo = GetLifoAndFifoCurrentCostFromCostQueue(AD_Client_ID, 0,
                                        product.GetM_Product_ID(), M_ASI_ID, acctSchema, 0, true, cd.Get_Trx(), cl);
                                }
                                else if (cl == MProductCategory.COSTINGLEVEL_Client)
                                {
                                    priceFifo = GetLifoAndFifoCurrentCostFromCostQueue(AD_Client_ID, 0,
                                        product.GetM_Product_ID(), 0, acctSchema, 0, true, cd.Get_Trx(), cl);
                                }
                                else if (cl == MProductCategory.COSTINGLEVEL_Organization)
                                {
                                    if (windowName == "Inventory Move")
                                ...
                                else if (cl == MProductCategory.COSTINGLEVEL_OrgPlusBatch || cl == MProductCategory.COSTINGLEVEL_Warehouse || cl == MProductCategory.COSTINGLEVEL_WarehousePlusBatch)
                                {
                                    if (windowName == "Inventory Move")
                                    {
                                        priceFifo = GetLifoAndFifoCurrentCostFromCostQueue(AD_Client_ID, movementline.GetAD_Org_ID(),
                                            ...
                                            ((cl == MProductCategory.COSTINGLEVEL_Warehouse || cl == MProductCategory.COSTINGLEVEL_WarehousePlusBatch) ?
                                            M_SourceWarehouse_ID : 0), true, cd.Get_Trx(), cl);
                                    }
                                    else
                                    {
                                        priceFifo = GetLifoAndFifoCurrentCostFromCostQueue(AD_Client_ID, AD_Org_ID,
                                           product.GetM_Product_ID(), (cl == MProductCategory.COSTINGLEVEL_Warehouse ? 0 : M_ASI_ID), acctSchema,
                                           ((cl == MProductCategory.COSTINGLEVEL_Warehouse || cl == MProductCategory.COSTINGLEVEL_WarehousePlusBatch) ?
                                           cd.GetM_Warehouse_ID() : 0), true, cd.Get_Trx(), cl);
                                    }
                                }
                                //priceFifo = Util.GetValueOfDecimal(DB.ExecuteScalar(sql));
                                priceFifo = Decimal.Round(priceFifo, acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero);
                            }
                            #endregion
                        }
                        #endregion
                        ...
                        #region lifo cost
                        if (costingCheck.MMPolicy.Equals(MProductCategory.MMPOLICY_LiFo))
                        {
                            if (cl == MProductCategory.COSTINGLEVEL_BatchLot)
                            {
                                sql = "SELECT CurrentCostPrice FROM M_Cost WHERE IsActive = 'Y' AND C_AcctSchema_ID = " + acctSchema.GetC_AcctSchema_ID() +
                            ...
                            if (priceLifo <= 0)
                            {
                                if (cl == MProductCategory.COSTINGLEVEL_BatchLot)
                                {
                                    priceLifo = GetLifoAndFifoCurrentCostFromCostQueue(AD_Client_ID, 0,
                                        product.GetM_Product_ID(), M_ASI_ID, acctSchema, 0, false, cd.Get_Trx(), cl);
                                }
                                else if (cl == MProductCategory.COSTINGLEVEL_Client)
                                {
                                    priceLifo = GetLifoAndFifoCurrentCostFromCostQueue(AD_Client_ID, 0,
                                        product.GetM_Product_ID(), 0, acctSchema, 0, false, cd.Get_Trx(), cl);
                                }
                                else if (cl == MProductCategory.COSTINGLEVEL_Organization)
                                {
                                    if (windowName == "Inventory Move")
                                ...
                                else if (cl == MProductCategory.COSTINGLEVEL_OrgPlusBatch || cl == MProductCategory.COSTINGLEVEL_Warehouse || cl == MProductCategory.COSTINGLEVEL_WarehousePlusBatch)
                                {
                                    if (windowName == "Inventory Move")
                                    {
                                        priceLifo = GetLifoAndFifoCurrentCostFromCostQueue(AD_Client_ID, movementline.GetAD_Org_ID(),
                                          product.GetM_Product_ID(), (cl == MProductCategory.COSTINGLEVEL_Warehouse ? 0 : M_ASI_ID), acctSchema,
                                          ((cl == MProductCategory.COSTINGLEVEL_Warehouse || cl == MProductCategory.COSTINGLEVEL_WarehousePlusBatch) ?
                                          M_SourceWarehouse_ID : 0), false, cd.Get_Trx(), cl);
                                    }
                                    else
                                    {
                                        priceLifo = GetLifoAndFifoCurrentCostFromCostQueue(AD_Client_ID, AD_Org_ID,
                                          product.GetM_Product_ID(), (cl == MProductCategory.COSTINGLEVEL_Warehouse ? 0 : M_ASI_ID), acctSchema,
                                          ((cl == MProductCategory.COSTINGLEVEL_Warehouse || cl == MProductCategory.COSTINGLEVEL_WarehousePlusBatch) ?
                                          cd.GetM_Warehouse_ID() : 0), false, cd.Get_Trx(), cl);
                                    }
                                }
                                priceLifo = Decimal.Round(priceLifo, acctSchema.GetCostingPrecision(), MidpointRounding.AwayFromZero);
                            }
                            #endregion
                        }
                        #endregion
            ...
            try
            {
                ...
                {
                    M_CostElement_ID = costingCheck.Lifo_ID;
                }
            ...
            catch (Exception ex)
            {
                ...
                _log.Info("Error occured Cost Queue not saved. Error Value :  " + (pp != null ? pp.GetValue() : "")
                    + " AND Error Name : " + (pp != null ? pp.GetName() : "") +
                           " And Exception message : " + ex.Message.ToString());
                costingCheck.errorMessage += "Exception at Cost Queue creation, " + ex.Message;
                return false;
            }
            return true;
        }
        ...
        {
            updateCostQueue(product, M_ASI_ID, mas, Org_ID, ce, movementQty, null, M_Warehouse_ID, cd);
        }

        private static void updateCostQueue(MProduct product, int M_ASI_ID, MAcctSchema mas,
                                             int Org_ID, MCostElement ce, decimal movementQty, CostingCheck costingCheck,
                                             int M_Warehouse_ID = 0, MCostDetail cd = null)
        {
            ...
            if (cQueue != null && cQueue.Length > 0)
            {
                ...
                for (int cq = 0; cq < cQueue.Length; cq++)
                {
                    ...
                    if (qty <= 0)
                    {
                        ...
                        {
                            // Create Cost Queue Transactional Record
                            MCostQueueTransaction.CreateCostQueueTransaction(cd.GetCtx(), queue.GetAD_Client_ID(), queue.GetAD_Org_ID(), queue.GetM_CostQueue_ID(), cd, Decimal.Negate(QueueQtyAffected), costingCheck);
                        }
        ...
        /// <param name="M_Warehouse_ID">Warehouse ID</param>
        /// <returns>True when success</returns>
        public static bool ReturnStockReAssignedToCostQueue(MCostDetail cd, String windowName, MCostElement costElement, Decimal Qty,
            bool IsReversedDocument, int M_Warehouse_ID, CostingCheck costingCheck, out bool backwardCompatabilitySupport)
        {
            String sql = "";
            backwardCompatabilitySupport = false;
            ...
            sql += " AND NVL(M_CostQueueTransaction.M_AttributeSetInstance_ID, 0) = " + costingCheck.M_ASI_ID;
            ...
            else if (windowName.Equals("Out") && !string.IsNullOrEmpty(costingCheck.TableName))
            {
                sql += $@" AND {costingCheck.TableName}_ID = (SELECT NVL(Reversaldoc_ID, 0) FROM {costingCheck.TableName}  
                        WHERE  {costingCheck.TableName}_ID = " + cd.Get_ValueAsInt(costingCheck.TableName + "_ID") + ")";
            }
                ...
                for (int i = 0; i < dsCostQueue.Tables[0].Rows.Count; i++)
                {
                    ...
                    else if (Math.Abs(Util.GetValueOfDecimal(dsCostQueue.Tables[0].Rows[i]["MovementQty"])) < Qty)
                    {
                        ...
                        if (!MCostQueueTransaction.CreateCostQueueTransaction(cd.GetCtx(), cd.GetAD_Client_ID(),
                            ...
                            Math.Abs(Util.GetValueOfDecimal(dsCostQueue.Tables[0].Rows[i]["MovementQty"])), costingCheck))
                        {
                            return false;
                        }
                    }
        ...
        /// <param name="M_Warehouse_ID">Warehouse Id</param>
        /// <returns>true when success</returns>
        public static bool ReturnStockReduceFromCostQueue(MCostDetail cd, String windowName, MCostElement costElement,
            Decimal Qty, bool IsReversedDocument, int M_Warehouse_ID, CostingCheck costingCheck, out bool backwardCompatabilitySupport)
        {
            String sql = "";
            backwardCompatabilitySupport = false;
            ...
            if (dsCostQueue != null && dsCostQueue.Tables.Count > 0 && dsCostQueue.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < dsCostQueue.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (Math.Abs(Util.GetValueOfDecimal(dsCostQueue.Tables[0].Rows[i]["MovementQty"])) >= Qty)
                    {
                        ...
                        if (!MCostQueueTransaction.CreateCostQueueTransaction(cd.GetCtx(), cd.GetAD_Client_ID(),
                            Util.GetValueOfInt(dsCostQueue.Tables[0].Rows[i]["AD_Org_ID"]),
                            Util.GetValueOfInt(dsCostQueue.Tables[0].Rows[i]["M_CostQueue_ID"]),
                            cd, Decimal.Negate(Qty), costingCheck))
                        {
                            return false;
                        }
                        break;
                    ...
                    else if (Math.Abs(Util.GetValueOfDecimal(dsCostQueue.Tables[0].Rows[i]["MovementQty"])) < Qty)
                    {
                        ...
                        if (!MCostQueueTransaction.CreateCostQueueTransaction(cd.GetCtx(), cd.GetAD_Client_ID(),
                            Util.GetValueOfInt(dsCostQueue.Tables[0].Rows[i]["AD_Org_ID"]), Util.GetValueOfInt(dsCostQueue.Tables[0].Rows[i]["M_CostQueue_ID"]), cd,
                            Decimal.Negate(Math.Abs(Util.GetValueOfDecimal(dsCostQueue.Tables[0].Rows[i]["MovementQty"]))), costingCheck))
                        {
                            return false;
                        }
                    }
