        ...
        protected override bool BeforeSave(bool newRecord)
        {
            try
            {
                ...
                else
                {
                    ...
                    //VAI066 15/12/2023 BugId-3460 Add 'W' and 'O' in the query to Handeled the Weighted AVerage Cost 
                    sql.Clear();
                    sql.Append(@"SELECT COUNT(*) FROM M_CostElementLine WHERE AD_Client_ID = " + GetAD_Client_ID()
                                  ...
                                  + " CostingMethod IN ('A' , 'F' , 'I' , 'L' , 'S' , 'i' , 'p', 'W','O')) AND M_CostElementLine_ID != " + GetM_CostElementLine_ID());
                    countRecord = Util.GetValueOfInt(DB.ExecuteScalar(sql.ToString(), null, null));
                    if (countRecord > 0)
                    {
