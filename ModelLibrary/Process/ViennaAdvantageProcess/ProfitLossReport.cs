...
{
    public class ProfitLossReport : SvrProcess
    {
        #region Variables
    ...
    t1.node_id,t1.ad_client_id,t1.ad_org_id,t1.value AS value,t1.name as name,nvl(max(t2.Currentamount),0) AS Currentamount,(nvl(max(t1.Total),0)-nvl(max(t2.Currentamount),0)) AS Previous,
         ...
         a.parent_id,max(c.issummary) as IsSummary,c.isintermediatecode, a.node_id,c.ad_client_id,c.ad_org_id,c.value, c.name, CASE WHEN c.AccountType='E' THEN (NVL((SUM(f.amtacctdr)),0)-NVL((SUM(f.amtacctcr)),0)) ELSE (NVL((SUM(f.amtacctcr)),0)-NVL((SUM(f.amtacctdr)),0)) END AS Total");
            ...
            _m_CaseSql.Append(@"(SELECT * From (Select distinct  c.AccountType,f.C_PROJECT_ID,
               a.parent_id,max(c.issummary) AS IsSummary,c.isintermediatecode, a.node_id,c.ad_client_id,c.ad_org_id,c.value, c.name, CASE WHEN c.AccountType='E'  THEN (NVL((SUM(f.amtacctdr)),0)-NVL((SUM(f.amtacctcr)),0)) ELSE (NVL((SUM(f.amtacctcr)),0)-NVL((SUM(f.amtacctdr)),0)) END AS Currentamount");
            _msql.Append(_m_CaseSql);
            _msql.Append(sql);
            ...
            Createsql(false);
            _msql.Append(m_parameterWhere);
            _msql.Append(" t2  ON (t1.value=t2.value) where t1.Total>0 group by t1.parent_id, t1.AccountType,  t1.C_PROJECT_ID,   t1.node_id,  t1.ad_client_id,  t1.ad_org_id,  t1.value     ,  t1.name  ,  t1.IsSummary,t1.isintermediatecode");
            //_msql.Append(" ");
            //_msql.Append(sql);
            //_msql.Append(sql);
        ...
        private void Createsql(bool totalQty)
        {
            ...
            {
                _LedgerType = "A";
                m_parameterWhere.Append(" ) WHERE c.ad_client_id=" + GetCtx().GetAD_Client_ID() + " and c.AccountType='E' or c.AccountType='R'  ");
            }
            else
            {
                if (_LedgerType == "N")
                ...
                if (IsIntermediate == "Y")
                {
                    m_parameterWhere.Append(" ) WHERE  c.issummary='" + _SummaryLevel + "' and c.AccountType='E' or c.AccountType='R' ");
                }
                if (IsIntermediate == "N")
                {
                    ...
                    m_parameterWhere.Append(" ) WHERE  c.issummary='" + _SummaryLevel + "' and c.AccountType='E' or c.AccountType='R'AND c.isintermediatecode='N' ");
                }
        ...
        {
            string _sqlview = @"CREATE OR REPLACE FORCE VIEW PROFITLOSS_V  as SELECT ad_client_id            ,  accounttype                   ,  ad_org_id                     ,  rownum AS ProfitLoss_V_ID     ,  C_PROJECT_ID                  ,  C_PERIOD_ID                   ,  ledgertype               AS LedgerType      ,  LEDGERNAME               AS LEDGER          ,  issummary                AS ShowSummaryLevel,  NVL(ABS(budgetval),0)    AS budgetval       ,  NVL(ABS(varinpercent),0) AS varinpercent    ,  NVL(ABS(crmonval),0)     AS crmonval        ,  NVL(ABS(tpmonvalue),0)   AS tpmonvalue      ,  NVL(ABS(totalvalue),0)   AS totalvalue      ,  ledgercode                                  ,  SUBSTR( ledgercode,1,1)  AS ledgercode1      ,  SUBSTR( ledgercode,2,2)  AS ledgercode2      ,  SUBSTR( ledgercode,4,2)  AS ledgercode3      ,  SUBSTR( ledgercode,6,2)  AS ledgercode4      ,  SUBSTR( ledgercode,8,2)  AS ledgercode5      ,  SUBSTR( ledgercode,10,2) AS ledgercode6      ,  SUBSTR( ledgercode,12,2) AS ledgercode7   FROM t_BALSHEET  WHERE accounttype ='E' OR accounttype      ='R' ORDER BY LEDGERCODE";
            DB.ExecuteQuery(_sqlview, null, null);
        }
        private void CreateViewAsTree()
        {
            string _sqlview = @"CREATE OR REPLACE FORCE VIEW PROFITLOSS_V AS  SELECT ad_client_id       ,  accounttype              ,  ad_org_id                ,  rownum AS ProfitLoss_V_ID,  C_PROJECT_ID             ,  C_PERIOD_ID              ,  ledgertype AS LedgerType ,  '.'  || LPAD (' ', LEVEL * 3)  || LEDGERNAME            AS LEDGER          ,  issummary                AS ShowSummaryLevel,  NVL(ABS(budgetval),0)    AS budgetval       ,  NVL(ABS(varinpercent),0) AS varinpercent    ,  NVL(ABS(crmonval),0)     AS crmonval        ,  NVL(ABS(tpmonvalue),0)   AS tpmonvalue      ,  NVL(ABS(totalvalue),0)   AS totalvalue      ,  ledgercode                                  ,  SUBSTR( ledgercode,1,1)  AS ledgercode1      ,  SUBSTR( ledgercode,2,2)  AS ledgercode2      ,  SUBSTR( ledgercode,4,2)  AS ledgercode3      ,  SUBSTR( ledgercode,6,2)  AS ledgercode4      ,  SUBSTR( ledgercode,8,2)  AS ledgercode5      ,  SUBSTR( ledgercode,10,2) AS ledgercode6      ,  SUBSTR( ledgercode,12,2) AS ledgercode7   FROM t_BALSHEET  WHERE accounttype ='E' OR accounttype      ='R' START WITH parent_id      =0 CONNECT BY parent_id = PRIOR node_id ORDER SIBLINGS BY LEDGERCODE";
            DB.ExecuteQuery(_sqlview, null, null);
        }
