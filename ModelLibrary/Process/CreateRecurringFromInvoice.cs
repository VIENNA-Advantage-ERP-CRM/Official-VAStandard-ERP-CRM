 ...
 * Class Name     : CreateRecurringFromInvoice
...
{
    public class CreateRecurringFromInvoice : SvrProcess
    {
        /// <summary>
        ...
        /// <summary>
        /// This method use to create Recurring Window record from Invoices window.
        /// Date           : 20230721
        /// Task ID        : 2260
        /// </summary>
        ...
        /// </summary>
        /// <param name="recordId">Invoice Record ID</param>
        /// <returns>Name of existing Recurring Window</returns>
        private string getExistingRecurringRecord(int recordId)
        {
            string recurringName = Util.GetValueOfString(DB.ExecuteScalar("SELECT Name FROM C_Recurring WHERE C_Invoice_ID=" + recordId));
        ...
        {
            string sql = "SELECT dt.Name ||'/'|| inv.DocumentNo AS RecurringName, inv.Ad_Org_Id AS OrgId , inv.Ad_Client_Id AS ClientID " +
                        " FROM C_Invoice inv " +
                        " INNER JOIN C_DocType dt on (inv.C_DocType_ID = dt.C_DocType_ID) WHERE inv.C_Invoice_ID =" + recordId;
